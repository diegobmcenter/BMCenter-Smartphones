...
export default function SmartphonesV9({
 ...
 return <div className="v9-page v9-inventory">
  ...
  <section className="v9-phone-grid">
   {filtered.map(x=>{
    ...
    return <article className="v9-phone-card" key={x.id}>
      <div className="v9-phone-media">
        <Smartphone size={34}/> 
        <button className={x.favorite?'active':''} onClick={()=>toggleFavorite(x)} title="Favorito"><Star size={15}/></button>
      </div>
      <div className="v9-phone-info">
        <div className="v9-phone-heading">
        ...
        <div className="v9-status-row">
          ...
          <div className="v9-published">{published.length?published.slice(0,3).map(p=><span key={p.id}>{String(p.name).slice(0,2).toUpperCase()}</span>):<em>Sem anúncio</em>}</div>
        </div>
        ...
        <footer>
          ...
          <div className="v9-menu-anchor">
            ...
            {actionPhone?.phone?.id===x.id&&<div className="v9-context">
