...
export default function AdsV9({
 ...
 return <div className="v9-publications">
  <div className="v9-publication-tools">
  ...
  <section className="v9-ad-grid">
   {filtered.map(({phone,ad})=>{
    ...
    return <article className="v9-ad-card" key={ad.id}>
      ...
      <div className="v9-ad-progress"><i><u style={{width:`${pct}%`}}/></i></div>
  ...
  {!!noAds.length&&<button className="v9-unpublished" onClick={()=>setShowNoAds(true)}><Smartphone size={17}/><span><b>{noAds.length} aparelho(s)</b> aguardando anúncio</span><strong>Ver aparelhos →</strong></button>}
 </div>
}
