...
export default function BatchV9({
 ...
 return <div className="v9-page v9-batch">
  <header className="v9-hero"><div><span>EDIÇÃO EM MASSA</span><h1>Ações em lote</h1><p>Selecione os aparelhos e aplique a alteração quando estiver pronto.</p></div><div className="v9-count-badge"><strong>{selected.length}</strong><span>selecionados</span></div></header>

  <section className="v9-batch-toolbar">
  ...
  <section className="v9-batch-actions">
  ...
  <section className="v9-batch-grid">{rows.map(p=><article className={selected.includes(p.id)?'selected':''} key={p.id} onClick={()=>toggle(p.id)}>
    <input type="checkbox" checked={selected.includes(p.id)} onChange={()=>toggle(p.id)} onClick={e=>e.stopPropagation()}/>
    <span className="v9-batch-device"><Smartphone size={17}/></span>
