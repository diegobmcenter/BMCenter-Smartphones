...
import{AlertTriangle,Camera,CheckCircle2,ChevronDown,ChevronRight,Clock3,ListTodo,Megaphone,PackageSearch,Play,RotateCcw,ScanSearch,ShoppingCart,SkipForward,Smartphone,Wrench}from'lucide-react';
...
export default function TodayV102({groups,attention=[],actions=[],waiting=[],completed=[],metrics={},onOpenPhone,onRunAction,onQuickAction}){
 ...
 const[queueOpen,setQueueOpen]=useState(false),[queueIndex,setQueueIndex]=useState(0),[resumeId,setResumeId]=useState(initialProgress?.actionId||'');
 useEffect(()=>{
  if(!actions.length){setQueueOpen(false);setQueueIndex(0);setResumeId('');clearQueueProgress();return}
 ...
 const startQueue=()=>{if(!actions.length)return;const index=resumeId?Math.max(0,actions.findIndex(item=>item.id===resumeId)):0;setQueueIndex(index<0?0:index);setQueueOpen(true)};
 const endQueue=()=>{setQueueOpen(false);setQueueIndex(0);setResumeId('');clearQueueProgress()};
 ...
 return <div className="v102-page v10523-today v10524-today">
  <header className="v102-hero v10523-hero"><div><span>HOJE</span><h1>Sua fila de trabalho.</h1><p>Faça primeiro o que já pode avançar. Exceções importantes ficam em Atenção e dependências externas em Aguardando.</p></div><button className="primary v10523-start" disabled={!actions.length} onClick={startQueue}><Play size={15}/> {resumeId?'Continuar minha fila':'Iniciar minha fila'}</button></header>
  ...
  {!queueOpen&&resumeId&&actions.length>0&&<button type="button" className="v10524-resume" onClick={startQueue}><Play size={13}/><span><b>Retomar de onde parei</b><small>{actions.find(item=>item.id===resumeId)?.title||'Próxima tarefa'} · {actions.findIndex(item=>item.id===resumeId)+1} de {actions.length}</small></span><ChevronRight size={14}/></button>}

  {queueOpen&&current&&<section className={`v10523-focus ${toneFor(current)}`}><header><div><span>TAREFA {queueIndex+1} DE {actions.length}</span><b>{current.cta||'Próxima ação'}</b></div><button onClick={endQueue}>Encerrar fila</button></header><div><i>{actionIcons[current.type]||<Smartphone/>}</i><section><h2>{current.title}</h2><p>{current.detail}</p></section><button className="primary" onClick={()=>executeAction(current)}>{current.cta||'Abrir ação'} <ChevronRight size={14}/></button><button onClick={skip}><SkipForward size={14}/> Pular</button></div></section>}
