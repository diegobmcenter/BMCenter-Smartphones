...
import {Archive,Check,ChevronDown,Clock3,Eye,Filter,MessageSquare,MoreHorizontal,Plus,RotateCcw,Search,Settings,Star,Trash2,UsersRound,WalletCards,X,TrendingUp} from 'lucide-react';
import{phoneSaleDisplayValue}from'../../saleAccounting.js';
import{syncPhonePublicationStatus}from'../../publicationStatus.js';
import{phoneCreationTimestamp}from'../../phoneCodes.js';
...
export default function SmartphonesV102({filtered,statuses,statusFilter,setStatusFilter,allTags,tagFilter,setTagFilter,query,setQuery,onlyFavorites,setOnlyFavorites,profiles,showProductCode,phoneTotalCost,money,toggleFavorite,changeStatus,setDetail,setEdit,setColumnEditor,setBatchCreate,blankPhone,items,actionPhone,setActionPhone,setSalePhone,setQuickReplyPhone,persist,updateFinancial,totalExpected,phoneDraft,batchDraft,continuePhoneDraft,continueBatchDraft,deletePhoneDraft,deleteBatchDraft}){
 ...
 const hasSoldRows=advancedFiltered.some(phone=>!!phone?.sale?.soldAt||phone?.status==='Vendido');
 const soldAtTime=phone=>phone?.sale?.soldAt?(Date.parse(`${phone.sale.soldAt}T12:00:00`)||0):0;
 const sortedAdvancedFiltered=[...advancedFiltered].sort((a,b)=>{
  if(sortMode==='oldest')return phoneCreationTimestamp(a)-phoneCreationTimestamp(b);
  if(sortMode==='sale_recent')return soldAtTime(b)-soldAtTime(a)||phoneCreationTimestamp(b)-phoneCreationTimestamp(a);
  if(sortMode==='sale_oldest')return (soldAtTime(a)||Number.MAX_SAFE_INTEGER)-(soldAtTime(b)||Number.MAX_SAFE_INTEGER)||phoneCreationTimestamp(a)-phoneCreationTimestamp(b);
  if(sortMode==='name')return `${a.brand||''} ${a.model||''}`.localeCompare(`${b.brand||''} ${b.model||''}`,'pt-BR',{numeric:true,sensitivity:'base'});
  if(sortMode==='price_asc')return Number(phoneSaleDisplayValue(a)||0)-Number(phoneSaleDisplayValue(b)||0);
  if(sortMode==='price_desc')return Number(phoneSaleDisplayValue(b)||0)-Number(phoneSaleDisplayValue(a)||0);
  if(sortMode==='status')return String(a.status||'').localeCompare(String(b.status||''),'pt-BR',{sensitivity:'base'});
  return phoneCreationTimestamp(b)-phoneCreationTimestamp(a);
 });
...
<section className="v102-toolbar v10313-main-toolbar"><label><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar modelo ou etiqueta"/></label><select className="v10478-tag-filter" value={tagFilter} onChange={e=>setTagFilter(e.target.value)}><option value="Todas">Todas as etiquetas</option>{allTags.map(t=><option key={t}>{t}</option>)}</select><button type="button" className={`v1041-status-trigger ${showStatusFilters||statusFilter.length?'active':''}`} onClick={()=>setShowStatusFilters(value=>!value)}><Check size={14}/><span>{statusFilter.length?`${statusFilter.length} status`:'Status'}</span><ChevronDown size={14}/></button><button className={`v10313-more-filters ${showExtraFilters||activeExtraCount?'active':''}`} onClick={()=>setShowExtraFilters(value=>!value)}><Filter size={14}/> Mais filtros{activeExtraCount>0&&<span>{activeExtraCount}</span>}</button><select className="v10540-sort-select" value={sortMode} onChange={e=>setSortMode(e.target.value)} aria-label="Ordenar aparelhos"><option value="recent">Cadastro: mais recentes</option><option value="oldest">Cadastro: mais antigos</option>{hasSoldRows&&<><option value="sale_recent">Venda: mais recentes</option><option value="sale_oldest">Venda: mais antigas</option></>}<option value="name">Aparelho A-Z</option><option value="price_asc">Menor preço</option><option value="price_desc">Maior preço</option><option value="status">Status</option></select><button className={`v10540-favorite-icon ${onlyFavorites?'active':''}`} title="Favoritos" aria-label="Favoritos" onClick={()=>setOnlyFavorites(v=>!v)}><Star size={15}/></button></section>
