...
{profilePhone&&<div className="v1040-profile-modal-backdrop" onMouseDown={e=>e.target===e.currentTarget&&setProfilePhone(null)}>
 <section className="v1040-profile-modal">
  ...
  <div className="v1040-profile-list">{profiles.filter(profile=>profile.active!==false).map(profile=>{const active=publishedProfilesFor(profilePhone).includes(profile.id);return <button key={profile.id} className={active?'active':''} onClick={()=>toggleProfile(profilePhone,profile.id)}><span>{active?<Check size={14}/>:<Plus size={14}/>}</span><div><b>{profile.name}</b><small>{active?'Publicado':'Não publicado'}</small></div></button>})}</div>
