...
<section className="v102-toolbar v10313-main-toolbar"><label><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar modelo ou etiqueta"/></label><MultiStatusFilter statuses={statuses} value={statusFilter} onChange={setStatusFilter}/><select value={tagFilter} onChange={e=>setTagFilter(e.target.value)}><option value="Todas">Todas as etiquetas</option>{allTags.map(t=><option key={t}>{t}</option>)}</select><button className={onlyFavorites?'active':''} onClick={()=>setOnlyFavorites(v=>!v)}><Star size={14}/> Favoritos</button><button className={`v10313-more-filters ${showExtraFilters||activeExtraCount?'active':''}`} onClick={()=>setShowExtraFilters(value=>!value)}><Filter size={14}/> Mais filtros{activeExtraCount>0&&<span>{activeExtraCount}</span>}</button></section>
