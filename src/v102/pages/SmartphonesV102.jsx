...
import {Archive,Camera,Check,ChevronDown,Clock3,Eye,FileText,Filter,MoreHorizontal,Plus,RotateCcw,Search,Settings,Star,Trash2,UsersRound,WalletCards,X,TrendingUp} from 'lucide-react';
...
export default function SmartphonesV102({filtered,statuses,statusFilter,setStatusFilter,allTags,tagFilter,setTagFilter,query,setQuery,onlyFavorites,setOnlyFavorites,profiles,showProductCode,phoneTotalCost,money,toggleFavorite,changeStatus,setDetail,setPhotoPhone,setEdit,setColumnEditor,setBatchCreate,blankPhone,items,actionPhone,setActionPhone,setSalePhone,persist,updateFinancial,totalExpected,phoneDraft,batchDraft,continuePhoneDraft,continueBatchDraft,deletePhoneDraft,deleteBatchDraft}){
 ...
 const profileMap=phone=>{
  ...
  try{(phone?.ads||[]).forEach(ad=>Object.entries(ad?.publications||{}).forEach(([id,pub])=>{if(pub?.status==='published'&&!source[id])source[id]={active:true,publishedAt:pub.date||'',updatedAt:pub.updatedAt||''}}))}catch{}
  return source;
 };
 const publishedProfilesFor=phone=>Object.entries(profileMap(phone)).filter(([,value])=>value?.active!==false).map(([id])=>id);
...
<section className="v102-toolbar v10313-main-toolbar"><label><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar modelo ou etiqueta"/></label><button type="button" className={`v1041-status-trigger ${showStatusFilters||statusFilter.length?'active':''}`} onClick={()=>setShowStatusFilters(value=>!value)}><Check size={14}/><span>{statusFilter.length?`${statusFilter.length} status`:'Status'}</span><ChevronDown size={14}/></button><select value={tagFilter} onChange={e=>setTagFilter(e.target.value)}><option value="Todas">Todas as etiquetas</option>{allTags.map(t=><option key={t}>{t}</option>)}</select><button className={onlyFavorites?'active':''} onClick={()=>setOnlyFavorites(v=>!v)}><Star size={14}/> Favoritos</button><button className={`v10313-more-filters ${showExtraFilters||activeExtraCount?'active':''}`} onClick={()=>setShowExtraFilters(value=>!value)}><Filter size={14}/> Mais filtros{activeExtraCount>0&&<span>{activeExtraCount}</span>}</button></section>
...
<section className="v102-device-list">{advancedFiltered.map(x=>{const cost=phoneTotalCost(x),profit=Number(x.expected||0)-cost;const published=publishedProfilesFor(x).length;return <article key={x.id} className="v102-device-row"><button className={`v102-star ${x.favorite?'active':''}`} onClick={()=>toggleFavorite(x)}><Star size={14}/></button><div className="v102-device-name"><h3>{x.brand} {x.model}</h3><p>{specs(x)}</p>{published>0&&<div className="v1041-published-inline">{publishedProfilesFor(x).slice(0,3).map(id=>{const profile=profiles.find(item=>item.id===id);return profile?<span key={id}>{profile.name}</span>:null})}{published>3&&<span className="more">+{published-3}</span>}</div>}{showProductCode()&&x.code&&<small>{x.code}</small>}</div><div className="v102-status"><select value={x.status} onChange={e=>changeStatus(x.id,e.target.value)}>{statuses.map(s=><option key={s}>{s}</option>)}</select><small>{published?'Publicado':'Sem anúncio'}</small></div><div className="v102-money v102-money-editable"><InlineMoney label="Custo" value={cost} onCommit={v=>updateFinancial(x.id,'cost',v)}/><InlineMoney label="Valor de venda" value={x.expected} onCommit={v=>updateFinancial(x.id,'expected',v)}/><InlineMoney label="Lucro" value={profit} tone={profit>=0?'good':'bad'} onCommit={v=>updateFinancial(x.id,'profit',v)}/></div><div className="v102-row-actions"><button onClick={()=>setDetail(x)} title="Abrir ficha"><Eye size={15}/></button><button className="v10423-photo-shortcut" onClick={()=>setPhotoPhone(x)} title="Fotos para anúncio"><Camera size={15}/></button><button onClick={()=>setEdit(x)} title="Editar"><FileText size={15}/></button><button className="v10423-scrap-shortcut" onClick={()=>{if(confirm(`Marcar ${x.brand||''} ${x.model||''} como Descarte/Sucata?\n\nEle sairá da lista normal e poderá ser recuperado pelo filtro “Descarte/Sucata”.`))changeStatus(x.id,'Descarte/Sucata')}} title="Marcar como Descarte/Sucata"><Archive size={15}/></button><div className="v102-menu-anchor"><button onClick={()=>setActionPhone(cur=>cur?.phone?.id===x.id?null:{phone:x})}><MoreHorizontal size={16}/></button>{actionPhone?.phone?.id===x.id&&<div className="v102-context"><button onClick={()=>{setProfilePhone(x);setActionPhone(null)}}><UsersRound size={15}/> Perfis publicados</button><button onClick={()=>{setSalePhone(x);setActionPhone(null)}}><WalletCards size={15}/> {x.sale?.soldAt?'Alterar Venda':'Registrar venda'}</button><button onClick={()=>{if(confirm(`Marcar ${x.brand||''} ${x.model||''} como Descarte/Sucata?`))changeStatus(x.id,'Descarte/Sucata');setActionPhone(null)}}><Archive size={15}/> Descarte/Sucata</button><button className="danger" onClick={()=>{if(confirm('Excluir aparelho?'))persist(items.filter(i=>i.id!==x.id));setActionPhone(null)}}><X size={15}/> Excluir</button></div>}</div></div></article>})}</section>
{profilePhone&&<div className="v1040-profile-modal-backdrop" onMouseDown={e=>e.target===e.currentTarget&&setProfilePhone(null)}>
 <section className="v1040-profile-modal">
  <header><div><h2>Perfis publicados</h2><p>{profilePhone.brand} {profilePhone.model}</p></div><button onClick={()=>setProfilePhone(null)}><X size={16}/></button></header>
  <div className="v1040-profile-list v1042-profile-list">{profiles.filter(profile=>profile.active!==false).map(profile=>{const active=publishedProfilesFor(profilePhone).includes(profile.id),entry=profileMap(profilePhone)[profile.id]||{},date=entry.publishedAt||new Date().toISOString().slice(0,10);return <article key={profile.id} className={active?'active':''}><button className="v1042-profile-toggle" onClick={()=>toggleProfile(profilePhone,profile.id)}><span>{active?<Check size={14}/>:<Plus size={14}/>}</span><div><b>{profile.name}</b><small>{active?'Publicado':'Não publicado'}</small></div></button>{active&&<label className="v1042-publication-date"><span>Data</span><input type="date" value={date} max={new Date().toISOString().slice(0,10)} onChange={e=>updateProfileDate(profilePhone,profile.id,e.target.value)}/></label>}</article>})}</div>
  {!profiles.filter(profile=>profile.active!==false).length&&<p className="v1040-empty-profiles">Nenhum perfil ativo cadastrado.</p>}
