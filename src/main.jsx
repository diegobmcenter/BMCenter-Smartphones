...
function PhonePhotoStudio({phone,onPhoneChange,standalone=false}){
 ...
 async function processAll({force=false,sceneOverride=null,sourcePhone=null}={}){
  ...
  for(let index=0;index<sourcePhotos.length;index++){
   ...
   setProgress(`IA local preparando ${index+1} de ${sourcePhotos.length}...`);
   ...
   try{
    ...
    await putPhotoAsset({id:processedAssetId,phoneId:current.id,role:'processed',dataUrl:resultDataUrl,createdAt:new Date().toISOString(),sceneId:chosenScene.id,name:`${current.code||'aparelho'}-${index+1}.jpg`,engine:'local-bg-removal'});
 ...
 return <section className={`photo-studio ${standalone?'standalone':''}`}>
  <header className="photo-studio-head"><div><span><WandSparkles size={15}/></span><div><h3>Fotos para anúncio</h3><p>IA local gratuita · aparelho original preservado · cenário consistente · sem créditos de API.</p></div></div><div className="photo-studio-head-right"><div className="photo-metadata-badge"><ShieldCheck size={14}/><span><b>Sem metadados</b><small>EXIF · GPS · XMP · IPTC</small></span></div><div className="photo-count"><b>{ready}</b><span>prontas</span><em>/ {total}</em></div></div></header>
