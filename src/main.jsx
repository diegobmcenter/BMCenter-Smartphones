...
import{effectivePartCost,normalizePartsOrder,normalizePartsOrders,syncOrdersIntoPhones,migrateLegacyPartsOrders,orderStatusLabel,createBulkPartsOrder}from'./partsOrders.js';
...
import BatchV102 from './v102/pages/BatchV102.jsx';import ActivityV102 from './v102/pages/ActivityV102.jsx';import ReportsV10 from './v10/pages/ReportsV10.jsx';import{cloudConfigured,getCloudSession,signInCloud,signUpCloud,signOutCloud,initializeCloudState,queueCloudSave,subscribeCloudState,getCloudStatus,clearCloudState,pushCloudStateNow,createCloudBackup,listCloudBackups,restoreCloudBackup,deleteCloudBackup}from'./cloud.js';import'./styles.css';import'./v10.css';import'./v102.css';import'./v1023.css';import'./v1024.css';import'./v1025.css';import'./v1026.css';import'./v1027.css';import'./v1028.css';import'./v1029.css';import'./v1030.css';import'./v1031.css';import'./v1033.css';import'./v1034.css';import'./v1038.css';import'./v1039.css';import'./v10311.css';import'./v10312.css';import'./v10313.css';import'./v10314.css';import'./v10315.css';import'./v1040.css';import'./v1041.css';import'./v1042.css';import'./v1043.css';import'./v1044.css';import'./v1046.css';import'./v1047.css';import'./v1048.css';import'./v1049.css';import'./v10410.css';import'./v10413.css';import'./v10414.css';import'./v10415.css';import'./v10416.css';import'./v10417.css';import'./v10418.css';import'./v10423.css';import'./v10424.css';import'./v10447.css';import'./v10448.css';import'./v10449.css';
...
function Parts(){
  ...
  const[expandedReceivedOrders,setExpandedReceivedOrders]=useState({});
  const[bulkOrderOpen,setBulkOrderOpen]=useState(false);
  const[bulkSearch,setBulkSearch]=useState('');
  const[bulkStatus,setBulkStatus]=useState('Todos');
  const[bulkSelected,setBulkSelected]=useState([]);
  const[bulkPrices,setBulkPrices]=useState({});
  const[bulkDraft,setBulkDraft]=useState(()=>({supplier:'',partName:'Película',unitPrice:'',freight:'',orderDate:new Date().toISOString().slice(0,10),expectedDate:'',notes:'',receivedNow:false}));
  ...
  function openBulkOrder(){
    setBulkOrderOpen(true);setBulkSearch('');setBulkStatus('Todos');setBulkSelected([]);setBulkPrices({});
    setBulkDraft({supplier:activeSuppliers[0]?.name||'',partName:'Película',unitPrice:'',freight:'',orderDate:new Date().toISOString().slice(0,10),expectedDate:'',notes:'',receivedNow:false})
  }
  function toggleBulkPhone(phoneId){setBulkSelected(current=>current.includes(phoneId)?current.filter(id=>id!==phoneId):[...current,phoneId])}
  function selectBulkFiltered(){const ids=bulkVisiblePhones.map(phone=>phone.id);setBulkSelected(current=>[...new Set([...current,...ids])])}
  function unselectBulkFiltered(){const ids=new Set(bulkVisiblePhones.map(phone=>phone.id));setBulkSelected(current=>current.filter(id=>!ids.has(id)))}
  function saveBulkOrder(){
    if(!bulkDraft.supplier.trim())return flash('Informe o fornecedor do pedido em massa.');
    if(!bulkDraft.partName.trim())return flash('Informe o nome da peça.');
    if(!bulkSelected.length)return flash('Selecione pelo menos um aparelho.');
    const missing=bulkSelected.filter(id=>String(bulkPrices[id]??bulkDraft.unitPrice).trim()==='');
    if(missing.length)return flash('Informe o valor unitário da peça.');
    try{
      const pricesByPhone=Object.fromEntries(bulkSelected.map(id=>[id,toNumber(bulkPrices[id]??bulkDraft.unitPrice)]));
      const result=createBulkPartsOrder({phones,phoneIds:bulkSelected,partName:bulkDraft.partName,supplier:bulkDraft.supplier,unitPrice:toNumber(bulkDraft.unitPrice),pricesByPhone,freight:toNumber(bulkDraft.freight),orderDate:bulkDraft.orderDate,expectedDate:bulkDraft.expectedDate,notes:bulkDraft.notes,receivedNow:bulkDraft.receivedNow});
      if(!result.order)return flash(result.skipped.length?'Os aparelhos selecionados já possuem esta peça em pedido ativo.':'Nenhum item pôde ser criado.');
      persistOrders([...orders,result.order],result.phones);setBulkOrderOpen(false);setPartsView(bulkDraft.receivedNow?'received':'orders');
      const total=result.order.items.length,skipped=result.skipped.length;
      flash(`${total} peça(s) adicionada(s) em massa${skipped?` · ${skipped} ignorada(s)`:''}.`)
    }catch(error){flash(error?.message||'Não foi possível criar o pedido em massa.')}
  }
  ...
  function OrderCard({order,receivedHistory=false}){
    ...
    const expanded=!receivedHistory||!!expandedReceivedOrders[order.id];
    const toggleHistory=()=>receivedHistory&&setExpandedReceivedOrders(current=>({...current,[order.id]:!current[order.id]}));
    return <article className={`parts-v48-order-card status-${order.status} ${receivedHistory?'parts-v49-received-card':''} ${expanded?'expanded':'collapsed'}`}>
      <header className={receivedHistory?'parts-v49-received-summary':''} onClick={toggleHistory} role={receivedHistory?'button':undefined} tabIndex={receivedHistory?0:undefined} onKeyDown={e=>{if(receivedHistory&&(e.key==='Enter'||e.key===' ')){e.preventDefault();toggleHistory()}}}><div className="parts-v48-order-title"><span><Store size={14}/></span><div><small>{orderStatusLabel(order.status).toUpperCase()}</small><h3>{order.supplier}</h3><p>{order.items.length} peça(s){order.orderDate?` · Pedido ${formatDate(order.orderDate)}`:''}</p></div></div><div className="parts-v48-order-total"><small>Total do pedido</small><strong>{money(order.total)}</strong><span>Frete {money(order.freight)}</span></div>{receivedHistory&&<span className="parts-v49-expand-indicator" title={expanded?'Fechar detalhes':'Ver detalhes'}><ChevronDown size={16}/></span>}</header>
  ...
  {bulkOrderOpen&&<div className="parts-v47-backdrop" onMouseDown={e=>e.target===e.currentTarget&&setBulkOrderOpen(false)}><section className="parts-v49-bulk-dialog"><header><div><span>PEDIDO EM MASSA · SEM COTAÇÃO</span><h2>Adicionar a mesma peça em vários aparelhos</h2><p>Selecione os aparelhos, informe o fornecedor e o valor. Ao concluir, cada peça entra individualmente no custo do respectivo smartphone.</p></div><button className="icon-only" onClick={()=>setBulkOrderOpen(false)}><X size={15}/></button></header><div className="parts-v49-bulk-main"><section className="parts-v49-bulk-fields"><label>Fornecedor<input list="parts-v49-bulk-suppliers" value={bulkDraft.supplier} onChange={e=>setBulkDraft({...bulkDraft,supplier:e.target.value})} placeholder="Fornecedor"/><datalist id="parts-v49-bulk-suppliers">{activeSuppliers.map(s=><option key={s.id||s.name} value={s.name}/>)}</datalist></label><label>Peça<input value={bulkDraft.partName} onChange={e=>setBulkDraft({...bulkDraft,partName:e.target.value})} placeholder="Ex.: Película"/></label><label>Valor unitário<div className="money-prefix"><span>R$</span><input inputMode="decimal" value={bulkDraft.unitPrice} onChange={e=>setBulkDraft({...bulkDraft,unitPrice:e.target.value.replace(/[^0-9,.-]/g,'')})} placeholder="0,00"/></div></label><label>Frete total<div className="money-prefix"><span>R$</span><input inputMode="decimal" value={bulkDraft.freight} onChange={e=>setBulkDraft({...bulkDraft,freight:e.target.value.replace(/[^0-9,.-]/g,'')})} placeholder="0,00"/></div></label><label>Data do pedido<input type="date" value={bulkDraft.orderDate} onChange={e=>setBulkDraft({...bulkDraft,orderDate:e.target.value})}/></label><label>Previsão<input type="date" value={bulkDraft.expectedDate} onChange={e=>setBulkDraft({...bulkDraft,expectedDate:e.target.value})}/></label><label className="wide">Observações<input value={bulkDraft.notes} onChange={e=>setBulkDraft({...bulkDraft,notes:e.target.value})} placeholder="Opcional"/></label><label className="parts-v49-bulk-received"><input type="checkbox" checked={bulkDraft.receivedNow} onChange={e=>setBulkDraft({...bulkDraft,receivedNow:e.target.checked})}/><span>Pedido já foi recebido</span></label></section><section className="parts-v49-bulk-select"><header><div><b>Selecionar aparelhos</b><small>{bulkSelected.length} selecionado(s) · {bulkVisiblePhones.length} nesta filtragem</small></div><div><button onClick={selectBulkFiltered}>Selecionar filtrados</button><button onClick={unselectBulkFiltered}>Desmarcar filtrados</button></div></header><div className="parts-v49-bulk-filters"><label><Search size={13}/><input value={bulkSearch} onChange={e=>setBulkSearch(e.target.value)} placeholder="Buscar modelo, código ou cor..."/></label><select value={bulkStatus} onChange={e=>setBulkStatus(e.target.value)}><option value="Todos">Todos os status</option>{statuses.filter(status=>!['Vendido','Descarte/Sucata'].includes(status)).map(status=><option key={status}>{status}</option>)}</select></div><div className="parts-v49-bulk-list">{bulkVisiblePhones.map(phone=>{const checked=bulkSelected.includes(phone.id);return <article className={checked?'selected':''} key={phone.id}><label className="parts-v49-bulk-check"><input type="checkbox" checked={checked} onChange={()=>toggleBulkPhone(phone.id)}/><div><b>{phoneDisplayName(phone,{includeCode:false})}</b><small>{phone.code} · {phone.status}</small></div></label>{checked&&<label className="parts-v49-bulk-row-price"><span>Valor desta peça</span><div className="money-prefix"><span>R$</span><input inputMode="decimal" value={bulkPrices[phone.id]??bulkDraft.unitPrice} onChange={e=>setBulkPrices({...bulkPrices,[phone.id]:e.target.value.replace(/[^0-9,.-]/g,'')})}/></div></label>}</article>})}{!bulkVisiblePhones.length&&<div className="parts-v47-empty"><Search size={18}/><div><b>Nenhum aparelho neste filtro</b><span>Altere a busca ou o status.</span></div></div>}</div></section></div><footer><div><span>{bulkSelected.length} aparelho(s)</span><b>Peças {money(bulkSelected.reduce((sum,id)=>sum+toNumber(bulkPrices[id]??bulkDraft.unitPrice),0))} · Frete {money(toNumber(bulkDraft.freight))}</b></div><div><button onClick={()=>setBulkOrderOpen(false)}>Cancelar</button><button className="primary" onClick={saveBulkOrder}><CheckSquare size={13}/> Concluir pedido em massa</button></div></footer></section></div>}
