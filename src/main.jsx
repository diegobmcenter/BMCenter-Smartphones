...
function PhonePhotoStudio({phone,onPhoneChange,standalone=false}){
 ...
 async function processAll({force=false,sceneOverride=null,sourcePhone=null}={}){
  ...
  for(let index=0;index<sourcePhotos.length;index++){
   ...
   try{
    ...
    const resultDataUrl=await preparePhotoLocally(original,{scene:chosenScene,intensity:settings.intensity,keepScale:settings.keepScale,onProgress:message=>setProgress(`Foto ${index+1}/${sourcePhotos.length} · ${message}`)});
    ...
    await putPhotoAsset({id:processedAssetId,phoneId:current.id,role:'processed',dataUrl:resultDataUrl,createdAt:new Date().toISOString(),sceneId:chosenScene.id,name:`${current.code||'aparelho'}-${index+1}.jpg`,engine:'local-theme-no-ai'});
 ...
 return <section className={`photo-studio ${standalone?'standalone':''}`}>
  <header className="photo-studio-head"><div><span><WandSparkles size={15}/></span><div><h3>Fotos para anúncio</h3><p>60 temas locais · zero IA · zero API · original preservado em alta qualidade.</p></div></div><div className="photo-studio-head-right"><div className="photo-metadata-badge"><ShieldCheck size={14}/><span><b>Sem metadados</b><small>EXIF · GPS · XMP · IPTC</small></span></div><div className="photo-count"><b>{ready}</b><span>prontas</span><em>/ {total}</em></div></div></header>
