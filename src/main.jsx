...
function ReportsPage(){
 ...
 const supplierSpend={};phones.forEach(p=>(p.parts||[]).forEach(part=>{const quotes=part.quotes||[],selected=quotes.find(q=>q.id===part.selectedQuoteId)||[...quotes].sort((a,b)=>Number(a.price)-Number(b.price))[0],supplier=part.purchaseSupplier||selected?.supplier,value=part.orderId?effectivePartCost(part):Number(selected?.price||0),referenceDate=part.orderedAt||part.receivedAt||p.date||p.lastActivityAt;if(supplier&&dateInRange(referenceDate)){supplierSpend[supplier]=(supplierSpend[supplier]||0)+value}}));
