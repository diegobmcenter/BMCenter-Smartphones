...
function PhonePhotoStudio({phone,onPhoneChange,standalone=false}){
 ...
 const settings={style:'Automático',intensity:'Natural',keepScale:true,aiEngine:'isnet',localFolderName:'',localFolderLinked:false,...(workPhone?.photoStudioSettings||{})};
 ...
 async function processAll({force=false,sceneOverride=null,sourcePhone=null}={}){
  ...
  for(let index=0;index<sourcePhotos.length;index++){
   ...
   setProgress(`Tema local preparando ${index+1} de ${sourcePhotos.length}...`);
   ...
   try{
    ...
    const resultDataUrl=await preparePhotoLocally(original,{scene:chosenScene,intensity:settings.intensity,keepScale:settings.keepScale,engine:settings.aiEngine||'isnet',onProgress:message=>setProgress(`Foto ${index+1}/${sourcePhotos.length} · ${message}`)});
    ...
    await putPhotoAsset({id:processedAssetId,phoneId:current.id,role:'processed',dataUrl:resultDataUrl,createdAt:new Date().toISOString(),sceneId:chosenScene.id,name:`${current.code||'aparelho'}-${index+1}.jpg`,engine:`local-${settings.aiEngine||'isnet'}`});
 ...
 return <section className={`photo-studio ${standalone?'standalone':''}`}>
  <header className="photo-studio-head"><div><span><WandSparkles size={15}/></span><div><h3>Fotos para anúncio</h3><p>2 IAs locais gratuitas · fundo realmente substituído · pixels originais do aparelho preservados.</p></div></div><div className="photo-studio-head-right"><div className="photo-metadata-badge"><ShieldCheck size={14}/><span><b>Sem metadados</b><small>EXIF · GPS · XMP · IPTC</small></span></div><div className="photo-count"><b>{ready}</b><span>prontas</span><em>/ {total}</em></div></div></header>
  <div className="photo-studio-controls">
   ...
   <label>Motor IA<select value={settings.aiEngine||'isnet'} onChange={e=>updateSettings({aiEngine:e.target.value})}><option value="isnet">ISNet · Rápido</option><option value="birefnet">BiRefNet Lite · Qualidade</option></select></label>
  ...
  <div className="photo-drive photo-local-folder">
   ...
   <div className="photo-local-flow"><span>1</span><b>Crie/vincule a subpasta</b><ChevronRight size={13}/><span>2</span><b>Tire a foto no BMCenter ou Open Camera</b><ChevronRight size={13}/><span>3</span><b>Fotos ficam ligadas ao aparelho</b></div>
   <div className="photo-storage-note"><ShieldCheck size={14}/><span><b>Cada captura fica vinculada ao aparelho.</b> Se a subpasta local estiver vinculada, “Tirar foto” também grava uma cópia sanitizada diretamente nela. O DriveSync pode continuar sincronizando essa pasta com o Google Drive.</span></div>
  </div>
 </section>
}
