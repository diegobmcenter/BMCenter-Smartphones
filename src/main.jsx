...
function Parts(){
  ...
  const overviewOrderMatches=order=>{
    ...
    const text=`${order.supplier||''} ${order.notes||''} ${(order.items||[]).map(item=>`${item.partName||''} ${item.phoneLabel||''}`).join(' ')} ${linkedPhones.map(phone=>`${phoneDisplayName(phone)} ${phone.code||''} ${phone.status||''}`).join(' ')}`.toLowerCase();
  ...
  const returnRows=orders.flatMap(order=>(order.items||[]).filter(item=>item.returnStatus).map(item=>({order,item,phone:phoneById.get(item.phoneId)||null})));
  ...
  const overviewDetailRows=overviewDetail==='open'?overviewOpenRows:overviewDetail==='waiting'?overviewWaitingRows:overviewDetail==='received'?overviewReceivedRows:overviewDetail==='returns'?overviewReturnRows:[];
  ...
  function undoDraftOrderPreparation(order,itemId=null){
    ...
    const remainingOrders=remainingItems.length
    ...
    if(!remainingItems.length)setExpandedOrderCards(current=>{const next={...current};delete next[order.id];return next});
    setPartsView('orders');
  ...
  function undoOrderProgress(order,itemId=null){
    ...
    if(step==='prepare')return undoDraftOrderPreparation(order,itemId);
    if(!itemId&&!confirm(step==='receive'?'Desfazer o recebimento do pedido completo? As peças voltarão para Pedidos, mas continuarão como compradas.':'Desfazer a confirmação do pedido completo? As peças voltarão ao estado anterior à compra e sairão do custo realizado.'))return;
    const stamp=new Date().toISOString();let result=null;let affected=[];
    const next=orders.map(current=>{
      if(current.id!==order.id)return current;
      result=undoPartsOrderStep(current,itemId,step);
      if(!result.changed)return current;
      const changedTargets=(current.items||[]).filter(item=>(!itemId||item.id===itemId)&&(step==='receive'?!!item.receivedAt:!!item.confirmedAt&&!item.receivedAt));
      affected=changedTargets.map(item=>`${item.phoneId}::${item.partId}`);
      return normalizePartsOrder({...result.order,updatedAt:stamp})
    });
    if(!result?.changed)return flash('Nada foi alterado.');
    const message=step==='receive'?'Recebimento desfeito':'Confirmação do pedido desfeita';
    const source=withTimeline(phones,affected,message);persistOrders(next,source);setPartsView('orders');
    flash(step==='receive'?'Recebimento desfeito. O item voltou para Pedidos.':'Confirmação desfeita. O item voltou à etapa anterior à compra.')
  }
  ...
  function OrderCard({order,receivedHistory=false}){
    const allConfirmed=order.items.length&&order.items.every(item=>item.confirmedAt),allReceived=order.items.length&&order.items.every(item=>item.receivedAt),anyConfirmed=order.items.some(item=>item.confirmedAt),anyReceived=order.items.some(item=>item.receivedAt);
  ...
  return <><div className="parts-command-page parts-v10447 parts-v10448 parts-v10450">
    ...
    {overviewDetail&&<section className="parts-v10518-overview-detail"><header><div><small>RESUMO DETALHADO</small><h3>{overviewDetail==='open'?`${overviewCounts.open} peça(s) em aberto`:overviewDetail==='waiting'?`${waitingItems} peça(s) aguardando recebimento`:overviewDetail==='received'?`${receivedItems} peça(s) recebida(s)`:`${overviewCounts.returns} devolução(ões) pendente(s)`}</h3></div><button type="button" className="icon-only" onClick={()=>setOverviewDetail('')}><X size={14}/></button></header><div className="parts-v10518-overview-list">{overviewDetailRows.map((row,index)=>{const phone=row.phone||null,part=row.part||row.item||{},order=row.order||null;const key=overviewDetail==='open'?`${phone?.id||'phone'}-${part.id||index}`:`${order?.id||'order'}-${part.id||index}`;const supplier=order?.supplier||row.chosen?.supplier||row.cheapest?.supplier||part.purchaseSupplier||'Sem fornecedor';const status=overviewDetail==='open'?(row.quotes?.length?'Com cotação':'Sem cotação'):overviewDetail==='waiting'?'Aguardando recebimento':overviewDetail==='received'?'Recebida':part.returnStatus==='pending'?'Para devolver':'Financeiro pendente';return <article key={key}><div className="parts-v10518-overview-identity"><b>{part.name||part.partName||'Peça'}</b><span>{phone?phoneDisplayName(phone,{includeCode:false}):part.phoneLabel||'Aparelho'}{phone?.code?` · ${phone.code}`:''}</span></div><div className="parts-v10518-overview-order"><small>{supplier}</small><b>{status}</b></div><button type="button" onClick={()=>{setPartsView(overviewDetail==='open'?'needs':overviewDetail==='waiting'?'orders':overviewDetail==='received'?'received':'returns');setQuery(phone?.code||part.phoneLabel||part.name||part.partName||'');setOverviewDetail('')}}>Abrir</button></article>})}{!overviewDetailRows.length&&<div className="parts-v47-empty"><Package size={18}/><div><b>Nenhum registro neste filtro</b><span>Altere os filtros gerais acima.</span></div></div>}</div></section>}
