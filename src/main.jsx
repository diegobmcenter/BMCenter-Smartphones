import React,{useEffect,useMemo,useRef,useState}from'react';import{createRoot}from'react-dom/client';import{Smartphone,Users,ShoppingCart,LayoutDashboard,Plus,LogOut,X,Store,ClipboardCheck,History,FileText,Download,Upload,ShieldCheck,Check,KanbanSquare,BarChart3,Search,CalendarDays,WalletCards,Tags,Package,Clock3,AlertTriangle,TrendingUp,Settings,Bell,ListTodo,Eye,Pencil,MoreVertical,ChevronLeft,ChevronRight,ChevronDown,Star,CheckSquare,DatabaseZap,RefreshCw,RotateCcw,Activity,Archive,Bookmark,UploadCloud,MessageSquare,Paperclip,Target,Gauge,CalendarClock,Copy,Trash2,ExternalLink,Save}from'lucide-react';
import{QRCodeSVG}from'qrcode.react';
import{effectivePartCost,partsOperationalCounters,partsPeriodReportMetrics,returnRefundTotal,returnRecoveredAmount,returnPartRefundDraft,returnShippingNetCost,allocateFreight,normalizePartsOrder,normalizePartsOrders,syncOrdersIntoPhones,migrateLegacyPartsOrders,recoverLegacyPartOrderStatusMutations,isPartProcurementComplete,isPartOpenForProcurement,orderStatusLabel,createBulkPartsOrder,createMultiBulkPartsOrder,removePartsOrderLinks,bulkPhoneProductsTotal,undoPartsOrderStep}from'./partsOrders.js';
...
function sanitizePhoneForLeanMode(phone){
 ...
 const normalized={...clean,nfc:phone?.nfc===true?true:phone?.nfc===false?false:null,unlockCredentials:normalizeUnlockCredentials(phone)};
 return syncPhonePublicationStatus(normalized,normalizeMarketplaceProfiles(normalized))
}
...
function PageContent({page,navigate}){
 try{
  return page==='dashboard'?<Dashboard/>:
   page==='today'?<TodayPage navigate={navigate}/>:
   page==='phones'?<Phones/>:
   page==='ads'?<Ads/>:
...
function Phones(){
 ...
 return <>
  <SmartphonesV102
    ...
    setSalePhone={setSalePhone} setQuickReplyPhone={setQuickReplyPhone} persist={persist} updateFinancial={updateFinancial}
...
function Parts(){
  ...
  const[standaloneReturns,setStandaloneReturns]=useState(()=>{const value=load(RKEY);return Array.isArray(value)?value:[]});
  const[returnShipments,setReturnShipments]=useState(()=>{const value=load(RSHIPKEY);return Array.isArray(value)?value:[]});
  ...
  const[standaloneReturnEditor,setStandaloneReturnEditor]=useState(null);
  const[returnShipmentEditor,setReturnShipmentEditor]=useState(null);
  ...
  const[rowSort,setRowSort]=useState('device');
  const[quoteSort,setQuoteSort]=useState('best_price');
  const[overviewSearch,setOverviewSearch]=useState('');
  const[overviewPeriod,setOverviewPeriod]=useState('30d');
  const[overviewType,setOverviewType]=useState('Todos');
  const[overviewOrigin,setOverviewOrigin]=useState('Todos');
  const[overviewSupplier,setOverviewSupplier]=useState('Todos');
  const[orderFreightFilter,setOrderFreightFilter]=useState('Todos');
  const[orderSort,setOrderSort]=useState('recent');
  const[receivedSituationFilter,setReceivedSituationFilter]=useState('Todos');
  const[returnShipmentFilter,setReturnShipmentFilter]=useState('Todos');
  const[returnSort,setReturnSort]=useState('recent');
  ...
  const[bulkOrderMode,setBulkOrderMode]=useState(()=>initialBulkSession?.mode==='single'?'single':'bulk');
  ...
  const[bulkProducts,setBulkProducts]=useState(()=>Array.isArray(initialBulkSession?.products)?initialBulkSession.products.map(product=>({type:'linked',quantity:'1',reference:'',...product})):[]);
  ...
  const[bulkStagedOrders,setBulkStagedOrders]=useState(()=>Array.isArray(initialBulkSession?.stagedOrders)?initialBulkSession.stagedOrders.map(batch=>({...batch,products:(batch.products||[]).map(product=>({type:'linked',quantity:'1',reference:'',...product}))})):[]);
  ...
  useRemoteStorageBridge(RKEY,setStandaloneReturns,value=>Array.isArray(value)?value:[]);
  useRemoteStorageBridge(RSHIPKEY,setReturnShipments,value=>Array.isArray(value)?value:[]);
  ...
  useEffect(()=>{if(!bulkOrderOpen)return;saveDeviceSessionDraft(BULK_PARTS_DEVICE_DRAFT_KEY,{open:true,mode:bulkOrderMode,draft:bulkDraft,products:bulkProducts,activeProductId:bulkActiveProductId,stagedOrders:bulkStagedOrders,search:bulkSearch,status:bulkStatus})},[bulkOrderOpen,bulkOrderMode,bulkDraft,bulkProducts,bulkActiveProductId,bulkStagedOrders,bulkSearch,bulkStatus]);
  useEffect(()=>{
  ...
  const auxiliaryPartsOverlayOpen=showRules||quoteModal||!!directBuy||!!orderEditor||!!refundEditor||!!standaloneReturnEditor||!!returnShipmentEditor;
  useEffect(()=>{
  ...
  const normalizeNeedle=value=>String(value??'').toLowerCase();
  const startOfDayValue=value=>{if(!value)return null;const date=new Date(value);if(Number.isNaN(date.getTime()))return null;date.setHours(0,0,0,0);return date.getTime()};
  const periodMatches=(value,period=overviewPeriod)=>{if(period==='Todos'||period==='all')return true;const target=startOfDayValue(value);if(target===null)return false;const today=new Date();today.setHours(0,0,0,0);if(period==='today')return target===today.getTime();const threshold=new Date(today);if(period==='7d')threshold.setDate(threshold.getDate()-6);else if(period==='30d')threshold.setDate(threshold.getDate()-29);else if(period==='90d')threshold.setDate(threshold.getDate()-89);else return true;return target>=threshold.getTime()};
  const topTypeMatches=value=>overviewType==='Todos'||value===overviewType;
  const topOriginMatches=value=>overviewOrigin==='Todos'||value===overviewOrigin;
  const globalSearchMatches=text=>!overviewSearch.trim()||normalizeNeedle(text).includes(normalizeNeedle(overviewSearch));
  const topSupplierMatches=supplier=>overviewSupplier==='Todos'||supplier===overviewSupplier;
  const itemUnits=item=>Math.max(1,Math.floor(Number(item?.quantity||1)||1));
  const orderVisualType=order=>{const linked=(order?.items||[]).length>0,external=(order?.externalItems||[]).some(item=>itemUnits(item)>0);if(linked&&external)return'misto';if(external)return'avulso';return'bmcenter'};
  const orderOrigin=order=>order?.source==='bulk'||order?.bulkVersion?'bulk':order?.source==='manual'?'manual':'direct';
  const rowOrigin=row=>row?.type==='standalone'?'manual':orderOrigin(row.order);
  const rowType=row=>row?.type==='linked'?'bmcenter':'avulso';
  const shipmentType=shipment=>{const refs=shipment?.items||[];const hasLinked=refs.some(item=>item.type==='linked');const hasExternal=refs.some(item=>item.type!=='linked');if(hasLinked&&hasExternal)return'misto';if(hasExternal)return'avulso';return'bmcenter'};
  const shipmentOrigin=()=> 'shipment';
  const orderPrimaryDate=order=>order?.status==='received'?(order.receivedAt||order.updatedAt||order.orderDate||order.createdAt):(order.orderDate||order.createdAt||order.updatedAt);
  const returnRowDate=row=>row?.type==='standalone'?(row.item.returnedAt||row.item.resolvedAt||row.item.returnDate||row.item.createdAt):(row.item.returnedToSupplierAt||row.item.returnMarkedAt||row.item.receivedAt||row.order?.receivedAt||row.order?.updatedAt);
  const returnShipmentDate=shipment=>shipment?.shipmentDate||shipment?.updatedAt||shipment?.createdAt;
  const orderHasPendingReturn=order=>[...(order.items||[]),...(order.externalItems||[])].some(item=>!!item.returnStatus);
  const orderHasExternalItems=order=>(order.externalItems||[]).some(item=>itemUnits(item)>0);
  const receivedSituationMatchesOrder=order=>receivedSituationFilter==='Todos'||(receivedSituationFilter==='pending_return'?orderHasPendingReturn(order):receivedSituationFilter==='finalized'?!orderHasPendingReturn(order):receivedSituationFilter==='mixed'?orderVisualType(order)==='misto':receivedSituationFilter==='external'?orderHasExternalItems(order):true);
  const orderFreightMatches=order=>orderFreightFilter==='Todos'||(orderFreightFilter==='with_freight'?Number(order?.freight||0)>0:Number(order?.freight||0)<=0);
  const sortLocale=(a,b)=>String(a||'').localeCompare(String(b||''),'pt-BR',{sensitivity:'base'});
  ...
  const persistStandaloneReturns=next=>{setStandaloneReturns(next);save(RKEY,next)};
  const persistReturnShipments=next=>{setReturnShipments(next);save(RSHIPKEY,next)};
  ...
  const bulkLinkedCount=bulkProducts.filter(product=>product.type!=='external').reduce((sum,product)=>sum+(product.phoneIds||[]).length,0);
  const bulkExternalCount=bulkProducts.filter(product=>product.type==='external').reduce((sum,product)=>sum+Math.max(1,Number(product.quantity||1)||1),0);
  const bulkLineCount=bulkLinkedCount+bulkExternalCount;
  const bulkLinkedSubtotal=bulkProducts.filter(product=>product.type!=='external').reduce((sum,product)=>sum+(product.phoneIds||[]).reduce((inner,id)=>inner+toNumber(product.pricesByPhone?.[id]??product.unitPrice),0),0);
  const bulkExternalSubtotal=bulkProducts.filter(product=>product.type==='external').reduce((sum,product)=>sum+toNumber(product.unitPrice)*Math.max(1,Number(product.quantity||1)||1),0);
  const bulkSubtotal=bulkLinkedSubtotal+bulkExternalSubtotal;
  const bulkCurrentHasSelection=bulkProducts.some(product=>product.type==='external'?String(product.name||'').trim():((product.phoneIds||[]).length>0));
  const stagedBulkLineCount=bulkStagedOrders.reduce((sum,batch)=>sum+batch.products.reduce((inner,product)=>inner+(product.type==='external'?Math.max(1,Number(product.quantity||1)||1):(product.phoneIds||[]).length),0),0);
  ...
  const overviewNeedle=normalizeNeedle(overviewSearch.trim());
  const rowSearchNeedle=normalizeNeedle(query.trim());
  const rowSearch=row=>`${phoneDisplayName(row.phone)} ${row.phone.code||''} ${row.phone.status||''} ${row.part.name} ${(row.quotes||[]).map(q=>q.supplier).join(' ')}`.toLowerCase().includes(rowSearchNeedle);
  ...
  const sortNeedRows=list=>[...list].sort((a,b)=>rowSort==='part'?sortLocale(a.part.name,b.part.name)||sortLocale(phoneDisplayName(a.phone),phoneDisplayName(b.phone)):rowSort==='status'?sortLocale(a.phone.status,b.phone.status)||sortLocale(phoneDisplayName(a.phone),phoneDisplayName(b.phone)):sortLocale(phoneDisplayName(a.phone),phoneDisplayName(b.phone))||sortLocale(a.part.name,b.part.name));
  const sortQuoteRows=list=>[...list].sort((a,b)=>quoteSort==='supplier'?sortLocale((a.chosen||a.cheapest||a.quotes[0]||{}).supplier,(b.chosen||b.cheapest||b.quotes[0]||{}).supplier)||sortLocale(a.part.name,b.part.name):quoteSort==='part'?sortLocale(a.part.name,b.part.name)||sortLocale(phoneDisplayName(a.phone),phoneDisplayName(b.phone)):Number(a.cheapest?.price||0)-Number(b.cheapest?.price||0)||sortLocale(a.part.name,b.part.name));
  const overviewPhoneMatches=phone=>true;
  const overviewRowMatches=row=>{const text=`${phoneDisplayName(row.phone)} ${row.phone.code||''} ${row.phone.status||''} ${row.part.name} ${(row.quotes||[]).map(q=>q.supplier).join(' ')}`.toLowerCase();const supplierMatch=topSupplierMatches(row.part.purchaseSupplier)||(row.quotes||[]).some(q=>topSupplierMatches(q.supplier));return overviewPhoneMatches(row.phone)&&supplierMatch&&topTypeMatches('bmcenter')&&topOriginMatches('Todos')&&globalSearchMatches(text)};
  const visibleRows=quoteableRows.filter(row=>overviewRowMatches(row)&&rowSearch(row)&&rowCommonFilter(row));
  const visibleNeedsRows=sortNeedRows(visibleRows.filter(row=>needsSituationFilter==='Todos'||(needsSituationFilter==='Sem cotação'?!row.quotes.length:needsSituationFilter==='Com cotação'?!!row.quotes.length:true)));
  const visibleQuoteRows=sortQuoteRows(visibleRows.filter(row=>row.quotes.length&&(quoteSupplierFilter==='Todos'||row.quotes.some(q=>q.supplier===quoteSupplierFilter))));
  ...
  const clearRowFilters=()=>{setPartFilter('Todos');setPhoneStatusPartsFilter('Todos');setNeedsSituationFilter('Todos');setQuoteSupplierFilter('Todos');setRowSort('device');setQuoteSort('best_price')};
  const phoneById=new Map(phones.map(phone=>[phone.id,phone]));
  const overviewOrderMatches=order=>{const linkedPhones=(order.items||[]).map(item=>phoneById.get(item.phoneId)).filter(Boolean);const text=`${order.supplier||''} ${order.notes||''} ${(order.items||[]).map(item=>`${item.partName||''} ${item.phoneLabel||''}`).join(' ')} ${(order.externalItems||[]).map(item=>`${item.partName||''} ${item.reference||''}`).join(' ')} ${linkedPhones.map(phone=>`${phoneDisplayName(phone)} ${phone.code||''} ${phone.status||''}`).join(' ')}`.toLowerCase();return topSupplierMatches(order.supplier)&&topTypeMatches(orderVisualType(order))&&topOriginMatches(orderOrigin(order))&&periodMatches(orderPrimaryDate(order))&&globalSearchMatches(text)};
  const overviewItemMatches=(order,item,phone,extra={})=>{const text=`${order.supplier||''} ${item.partName||''} ${item.phoneLabel||''} ${phone?phoneDisplayName(phone):''} ${phone?.code||''} ${phone?.status||''} ${item.reference||''} ${item.reason||''}`.toLowerCase();return topSupplierMatches(order.supplier)&&topTypeMatches(extra.type||rowType({type:phone?'linked':'external'}))&&topOriginMatches(extra.origin||orderOrigin(order))&&periodMatches(extra.date||orderPrimaryDate(order))&&globalSearchMatches(text)};
  const waitingOrders=orders.filter(order=>order.status!=='received'),receivedOrders=orders.filter(order=>order.status==='received');
  const returnRows=orders.flatMap(order=>(order.items||[]).filter(item=>item.returnStatus).map(item=>({order,item,phone:phoneById.get(item.phoneId)||null,type:'linked'})));
  const externalReturnRows=orders.flatMap(order=>(order.externalItems||[]).filter(item=>item.returnStatus).map(item=>({order,item,phone:null,type:'external'})));
  const standaloneReturnRows=standaloneReturns.filter(Boolean).map(entry=>{const quantity=Math.max(1,Number(entry.quantity||1)||1),price=toNumber(entry.unitPrice)*quantity,freight=toNumber(entry.freight);return{order:{id:`standalone-${entry.id}`,supplier:entry.supplier||'Fornecedor não informado',source:'manual'},item:{...entry,partName:entry.partName||'Item avulso',returnStatus:entry.status||'pending',price,freightShare:freight,effectiveCost:price+freight},phone:null,type:'standalone',standalone:entry}});
  ...
  const activeExternalReturns=externalReturnRows.filter(row=>row.item.returnStatus==='pending');
  const activeStandaloneReturns=standaloneReturnRows.filter(row=>row.item.returnStatus==='pending');
  const operationalCounts=partsOperationalCounters(phones,orders);
  const overviewOpenRows=quoteableRows.filter(overviewRowMatches);
  const overviewWaitingRows=orders.flatMap(order=>(order.items||[]).filter(item=>item.confirmedAt&&!item.receivedAt&&!item.returnStatus).map(item=>({order,item,phone:phoneById.get(item.phoneId)||null}))).filter(row=>overviewItemMatches(row.order,row.item,row.phone,{type:'bmcenter',origin:orderOrigin(row.order),date:row.item.confirmedAt||orderPrimaryDate(row.order)}));
  const overviewReceivedRows=orders.flatMap(order=>[...(order.items||[]).filter(item=>item.receivedAt&&!item.returnStatus).map(item=>({order,item,phone:phoneById.get(item.phoneId)||null})),...(order.externalItems||[]).filter(item=>item.receivedAt&&!item.returnStatus).map(item=>({order,item,phone:null}))]).filter(row=>overviewItemMatches(row.order,row.item,row.phone,{type:row.phone?'bmcenter':'avulso',origin:orderOrigin(row.order),date:row.item.receivedAt||orderPrimaryDate(row.order)}));
  const overviewReturnRows=[...pendingReturns,...pendingReturnFinancials].filter(row=>overviewItemMatches(row.order,row.item,row.phone,{type:'bmcenter',origin:orderOrigin(row.order),date:returnRowDate(row)}));
  const overviewExternalReturnRows=activeExternalReturns.filter(row=>overviewItemMatches(row.order,row.item,row.phone,{type:'avulso',origin:orderOrigin(row.order),date:returnRowDate(row)}));
  const overviewStandaloneReturnRows=activeStandaloneReturns.filter(row=>overviewItemMatches(row.order,row.item,row.phone,{type:'avulso',origin:'manual',date:returnRowDate(row)}));
  const overviewCounts={open:overviewOpenRows.length,quotes:overviewOpenRows.filter(row=>row.quotes.length).length,orders:waitingOrders.filter(overviewOrderMatches).length,received:receivedOrders.filter(overviewOrderMatches).length,returns:overviewReturnRows.length+overviewExternalReturnRows.length+overviewStandaloneReturnRows.length};
  const waitingItems=overviewWaitingRows.length;
  const receivedItems=overviewReceivedRows.length;
  const returnAttentionCount=overviewReturnRows.length+overviewExternalReturnRows.length+overviewStandaloneReturnRows.length;
  const overviewSuppliers=[...new Set([...quoteSuppliers,...orders.map(order=>order.supplier),...standaloneReturns.map(item=>item.supplier)].filter(Boolean))].sort((a,b)=>a.localeCompare(b,'pt-BR',{sensitivity:'base'}));
  const overviewDetailRows=overviewDetail==='open'?overviewOpenRows:overviewDetail==='waiting'?overviewWaitingRows:overviewDetail==='received'?overviewReceivedRows:overviewDetail==='returns'?[...overviewReturnRows,...overviewExternalReturnRows,...overviewStandaloneReturnRows]:[];
  const overviewHasFilters=!!overviewSearch.trim()||overviewPeriod!=='30d'||overviewType!=='Todos'||overviewOrigin!=='Todos'||overviewSupplier!=='Todos';
  const clearOverviewFilters=()=>{setOverviewSearch('');setOverviewPeriod('30d');setOverviewType('Todos');setOverviewOrigin('Todos');setOverviewSupplier('Todos')};
  const activeOverviewChips=[overviewPeriod!=='30d'&&{key:'period',label:`Período: ${overviewPeriod==='today'?'Hoje':overviewPeriod==='7d'?'7 dias':overviewPeriod==='30d'?'30 dias':overviewPeriod==='90d'?'90 dias':'Tudo'}`,clear:()=>setOverviewPeriod('30d')},overviewSupplier!=='Todos'&&{key:'supplier',label:`Fornecedor: ${overviewSupplier}`,clear:()=>setOverviewSupplier('Todos')},overviewType!=='Todos'&&{key:'type',label:`Tipo: ${overviewType==='bmcenter'?'Peças BMCenter':overviewType==='avulso'?'Itens avulsos':'Mistos'}`,clear:()=>setOverviewType('Todos')},overviewOrigin!=='Todos'&&{key:'origin',label:`Origem: ${overviewOrigin==='manual'?'Manual':overviewOrigin==='bulk'?'Pedido em massa':overviewOrigin==='direct'?'Compra direta':'Remessa de devolução'}`,clear:()=>setOverviewOrigin('Todos')},overviewSearch.trim()&&{key:'search',label:`Busca: ${overviewSearch.trim()}`,clear:()=>setOverviewSearch('')}].filter(Boolean);
  const openOverview=type=>setOverviewDetail(current=>current===type?'':type);
  const sortOrders=list=>[...list].sort((a,b)=>orderSort==='oldest'?(startOfDayValue(orderPrimaryDate(a))||0)-(startOfDayValue(orderPrimaryDate(b))||0):orderSort==='supplier'?sortLocale(a.supplier,b.supplier):orderSort==='total'?Number(b.total||0)-Number(a.total||0):(startOfDayValue(orderPrimaryDate(b))||0)-(startOfDayValue(orderPrimaryDate(a))||0));
  const orderMatches=order=>{const text=`${order.supplier} ${order.notes||''} ${(order.items||[]).map(item=>`${item.partName} ${item.phoneLabel}`).join(' ')} ${(order.externalItems||[]).map(item=>`${item.partName} ${item.reference||''}`).join(' ')}`.toLowerCase();return overviewOrderMatches(order)&&text.includes(rowSearchNeedle)&&(orderStatusFilter==='Todos'||order.status===orderStatusFilter)&&orderFreightMatches(order)};
  const visibleWaitingOrders=sortOrders(waitingOrders.filter(orderMatches));
  const visibleReceivedOrders=sortOrders(receivedOrders.filter(order=>orderMatches(order)&&receivedSituationMatchesOrder(order)));
  const sortReturnRows=list=>[...list].sort((a,b)=>returnSort==='supplier'?sortLocale(a.order?.supplier,b.order?.supplier):returnSort==='value'?Number(b.item?.effectiveCost||b.item?.price||0)-Number(a.item?.effectiveCost||a.item?.price||0):(startOfDayValue(returnRowDate(b))||0)-(startOfDayValue(returnRowDate(a))||0));
  const returnMatches=row=>{const text=`${row.order.supplier} ${row.item.partName} ${row.item.phoneLabel||''} ${row.phone?.code||''} ${row.phone?.brand||''} ${row.phone?.model||''}`.toLowerCase();const financialPending=row.item.returnStatus==='returned'&&!['received','supplier_credit'].includes(row.item.returnFinancialStatus);const filterMatch=returnFilter==='all'||(returnFilter==='financial'?financialPending:row.item.returnStatus===returnFilter);const shipmentMatch=returnShipmentFilter==='Todos'||(returnShipmentFilter==='with_shipment'?!!row.item.returnShipmentId:!row.item.returnShipmentId);return overviewItemMatches(row.order,row.item,row.phone,{type:'bmcenter',origin:orderOrigin(row.order),date:returnRowDate(row)})&&text.includes(rowSearchNeedle)&&filterMatch&&shipmentMatch};
  const externalReturnMatches=row=>{const text=`${row.order.supplier} ${row.item.partName} ${row.item.reference||''} ${row.item.reason||''}`.toLowerCase();const financialPending=false;const filterMatch=returnFilter==='all'||(returnFilter==='financial'?financialPending:row.item.returnStatus===returnFilter);const shipmentMatch=returnShipmentFilter==='Todos'||(returnShipmentFilter==='with_shipment'?!!row.item.returnShipmentId:!row.item.returnShipmentId);const origin=row.type==='standalone'?'manual':orderOrigin(row.order);return overviewItemMatches(row.order,row.item,row.phone,{type:'avulso',origin,date:returnRowDate(row)})&&text.includes(rowSearchNeedle)&&filterMatch&&shipmentMatch};
  const visibleReturnRows=sortReturnRows(returnRows.filter(returnMatches)),visibleExternalReturnRows=sortReturnRows(externalReturnRows.filter(externalReturnMatches)),visibleStandaloneReturnRows=sortReturnRows(standaloneReturnRows.filter(externalReturnMatches));
  const returnSeparatedBySupplier=Object.values([...returnRows.filter(row=>row.item.returnStatus==='pending').map(row=>({...row,separatedType:'linked'})),...externalReturnRows.filter(row=>row.item.returnStatus==='pending').map(row=>({...row,separatedType:'external'})),...standaloneReturnRows.filter(row=>row.item.returnStatus==='pending').map(row=>({...row,separatedType:'external'}))].filter(row=>((returnShipmentFilter==='Todos')||(returnShipmentFilter==='with_shipment'?!!row.item.returnShipmentId:!row.item.returnShipmentId))&&`${row.order.supplier} ${row.item.partName} ${row.item.phoneLabel||''} ${row.item.reference||''} ${row.item.reason||''}`.toLowerCase().includes(rowSearchNeedle)&&topSupplierMatches(row.order.supplier)&&topTypeMatches(row.separatedType==='linked'?'bmcenter':'avulso')&&topOriginMatches(row.type==='standalone'?'manual':orderOrigin(row.order))&&periodMatches(returnRowDate(row))).reduce((acc,row)=>{const name=row.order.supplier||'Fornecedor não informado';acc[name]??={supplier:name,count:0,linkedValue:0,externalValue:0,total:0};const value=Number(row.item.effectiveCost||row.item.price||0),qty=row.separatedType==='external'?Number(row.item.quantity||1):1;acc[name].count+=qty;acc[name][row.separatedType==='external'?'externalValue':'linkedValue']+=value;acc[name].total+=value;return acc},{})).sort((a,b)=>b.total-a.total);
  const visibleReturnShipments=[...returnShipments].filter(shipment=>{const text=`${shipment.supplier||''} ${shipment.notes||''} ${(shipment.items||[]).map(item=>`${item.partName||''} ${item.reference||''}`).join(' ')}`.toLowerCase();return ['returned','all'].includes(returnFilter)&&topSupplierMatches(shipment.supplier)&&topTypeMatches(shipmentType(shipment))&&topOriginMatches(shipmentOrigin())&&periodMatches(returnShipmentDate(shipment))&&text.includes(rowSearchNeedle)}).sort((a,b)=>returnSort==='supplier'?sortLocale(a.supplier,b.supplier):(startOfDayValue(returnShipmentDate(b))||0)-(startOfDayValue(returnShipmentDate(a))||0));
  const orderSuppliers=[...new Set([...orders.map(order=>order.supplier),...standaloneReturns.map(item=>item.supplier)].filter(Boolean))].sort((a,b)=>a.localeCompare(b,'pt-BR'));
  const orderTypeLabel=order=>orderVisualType(order)==='misto'?'Misto':orderVisualType(order)==='avulso'?'Avulso':'BMCenter';
  const orderOriginLabel=order=>orderOrigin(order)==='bulk'?'Em massa':orderOrigin(order)==='manual'?'Manual':'Compra direta';
  const viewFocusSummary=partsView==='needs'?{eyebrow:'Fila operacional',title:`${visibleNeedsRows.length} peça(s) em aberto`,text:'Use esta aba para descobrir o que falta cotar e comprar primeiro.'}:partsView==='quotes'?{eyebrow:'Comparação de fornecedores',title:`${visibleQuoteRows.length} peça(s) cotadas`,text:'Aqui você escolhe o melhor fornecedor e transforma a cotação em pedido.'}:partsView==='orders'?{eyebrow:'Pedidos aguardando ação',title:`${visibleWaitingOrders.length} pedido(s) em andamento`,text:'Confirme, receba e acompanhe pedidos ainda não concluídos.'}:partsView==='received'?{eyebrow:'Histórico operacional recente',title:`${visibleReceivedOrders.length} pedido(s) recebidos`,text:'Recebidos agora respeita o período do topo, com padrão inicial nos últimos 30 dias.'}:{eyebrow:'Pós-compra e retorno',title:`${visibleReturnRows.length+visibleExternalReturnRows.length+visibleStandaloneReturnRows.length} devolução(ões) filtradas`,text:'Gerencie itens para devolver, remessas e pendências financeiras sem misturar com compras ativas.'};
  ...
  const freightFor=(name,items)=>{if(!items.length)return 0;const setting=settingFor(name),subtotal=items.reduce((sum,item)=>sum+Number(item.price||0),0),unitCount=items.reduce((sum,item)=>sum+Math.max(1,Math.floor(Number(item.quantity||1)||1)),0);if(setting.freightPaid)return 0;if(toNumber(setting.freeAbove)>0&&subtotal>=toNumber(setting.freeAbove))return 0;if(toNumber(setting.freeItems)>0&&unitCount>=toNumber(setting.freeItems))return 0;return toNumber(setting.freight)};
  ...
  function editQuote(row,quote){
    if(!quote)return flash('Nenhuma cotação disponível para editar.');
    if(row.part.orderId&&linkedOrderIds.has(row.part.orderId))return flash('Cotação usada em pedido deve ser alterada pelo próprio pedido.');
    setDirectBuy({phone:row.phone,part:row.part,mode:'quote',editingQuoteId:quote.id});
    setDirectDraft({supplier:quote.supplier||'',price:quote.price??'',freight:'',notes:quote.notes||'',receivedNow:false,orderDate:new Date().toISOString().slice(0,10)})
  }
  function undoQuoteStage(row,quote=null){
    if(row.part.orderId&&linkedOrderIds.has(row.part.orderId))return flash('Esta peça já está vinculada a um pedido. Desfaça o pedido primeiro.');
    const quotes=row.part.quotes||[];
    if(!quotes.length)return flash('Esta peça não possui cotação para desfazer.');
    const individual=!!quote;
    const message=individual?`Desfazer a cotação de ${quote.supplier}?${quotes.length===1?' A peça voltará para Em aberto.':''}`:`Desfazer todas as ${quotes.length} cotação(ões) de ${row.part.name}? A peça voltará para Em aberto.`;
    if(!confirm(message))return;
    const stamp=new Date().toISOString();
    const next=phones.map(phone=>{
      if(phone.id!==row.phone.id)return phone;
      const parts=(phone.parts||[]).map(part=>{
        if(part.id!==row.part.id)return part;
        const remaining=individual?quotes.filter(item=>item.id!==quote.id):[];
        const selectedQuoteId=remaining.some(item=>item.id===part.selectedQuoteId)?part.selectedQuoteId:'';
        return{...part,quotes:remaining,selectedQuoteId,status:'Cotando'}
      });
      const action=individual?`Cotação desfeita: ${row.part.name} · ${quote.supplier}`:`Cotações desfeitas: ${row.part.name}`;
      return{...phone,parts,lastActivityAt:stamp,timeline:[...(phone.timeline||[]),{id:crypto.randomUUID(),date:stamp,message:action}]}
    });
    savePhonesOnly(next);
    flash(individual?(quotes.length===1?'Cotação desfeita. A peça voltou para Em aberto.':'Cotação desfeita.'):'Cotações desfeitas. A peça voltou para Em aberto.')
  }
  ...
  function newBulkProduct(name='Película',type='linked'){
    return{id:crypto.randomUUID(),type:type==='external'?'external':'linked',name,unitPrice:'',quantity:'1',reference:'',phoneIds:[],pricesByPhone:{}}
  }
  ...
  function resetCurrentBulkOrder(initialName='Película'){
    const first=newBulkProduct(initialName);
  ...
  function openBulkOrder(mode='bulk'){
    const resolved=mode==='single'?'single':'bulk';
    setBulkOrderMode(resolved);setBulkOrderOpen(true);setBulkStagedOrders([]);resetCurrentBulkOrder(resolved==='single'?'':'Película')
  }
  ...
  function addBulkProduct({focusName=false,type='linked'}={}){const product=newBulkProduct('',type);setBulkProducts(current=>[...current,product]);setBulkActiveProductId(product.id);if(focusName)focusBulkProductName(product.id);return product.id}
  function handleBulkUnitPriceTab(event,product,index){
    ...
    if(String(product.name||'').trim()||String(product.unitPrice||'').trim())addBulkProduct({focusName:true,type:product.type==='external'?'external':'linked'})
  }
  ...
  function toggleBulkPhone(phoneId){if(!activeBulkProduct||activeBulkProduct.type==='external')return;const selected=activeBulkProduct.phoneIds||[];updateBulkProduct(activeBulkProduct.id,{phoneIds:selected.includes(phoneId)?selected.filter(id=>id!==phoneId):[...selected,phoneId]})}
  function selectBulkFiltered(){if(!activeBulkProduct||activeBulkProduct.type==='external')return;const ids=bulkVisiblePhones.map(phone=>phone.id);updateBulkProduct(activeBulkProduct.id,{phoneIds:[...new Set([...(activeBulkProduct.phoneIds||[]),...ids])]})}
  function unselectBulkFiltered(){if(!activeBulkProduct||activeBulkProduct.type==='external')return;const ids=new Set(bulkVisiblePhones.map(phone=>phone.id));updateBulkProduct(activeBulkProduct.id,{phoneIds:(activeBulkProduct.phoneIds||[]).filter(id=>!ids.has(id))})}
  ...
  function buildBulkSnapshot(draft=bulkDraft,products=bulkProducts,id=''){
    ...
    const normalizedProducts=products.map(product=>({...product,type:product.type==='external'?'external':'linked'}));
    const selectedProducts=normalizedProducts.filter(product=>String(product.name||'').trim()&&(product.type==='external'||(product.phoneIds||[]).length));
    if(!selectedProducts.length)throw new Error('Adicione pelo menos uma peça do BMCenter ou um item avulso.');
    const linkedProducts=selectedProducts.filter(product=>product.type!=='external');
    const duplicateNames=linkedProducts.map(product=>product.name.trim().toLocaleLowerCase('pt-BR')).filter((name,index,list)=>list.indexOf(name)!==index);
    if(duplicateNames.length)throw new Error('Há peças BMCenter com o mesmo nome neste pedido. Una os aparelhos no mesmo produto.');
    const missingPrice=selectedProducts.some(product=>product.type==='external'?String(product.unitPrice??'').trim()==='':(product.phoneIds||[]).some(phoneId=>String(product.pricesByPhone?.[phoneId]??product.unitPrice).trim()===''));
    if(missingPrice)throw new Error('Informe o valor de cada produto antes de adicionar o pedido.');
    const invalidExternal=selectedProducts.some(product=>product.type==='external'&&Math.max(0,Number(product.quantity||0)||0)<1);
    if(invalidExternal)throw new Error('Informe a quantidade dos itens avulsos.');
    const cleanProducts=selectedProducts.map(product=>product.type==='external'?{...product,name:product.name.trim(),reference:String(product.reference||'').trim(),quantity:Math.max(1,Math.floor(Number(product.quantity||1)||1)),phoneIds:[],pricesByPhone:{}}:{...product,name:product.name.trim(),phoneIds:[...(product.phoneIds||[])],pricesByPhone:{...(product.pricesByPhone||{})}});
    const subtotal=cleanProducts.reduce((sum,product)=>product.type==='external'?sum+toNumber(product.unitPrice)*Math.max(1,Number(product.quantity||1)||1):sum+(product.phoneIds||[]).reduce((inner,phoneId)=>inner+toNumber(product.pricesByPhone?.[phoneId]??product.unitPrice),0),0);
  ...
  function saveBulkOrder(){
    try{
      ...
      for(const snapshot of snapshots){
        ...
        const createdOrder=bulkOrderMode==='single'?{...result.order,source:'manual'}:result.order;
        nextOrders.push(createdOrder);sourcePhones=syncOrdersIntoPhones(result.phones,nextOrders);createdOrders++;createdItems+=createdOrder.items.length+(createdOrder.externalItems||[]).reduce((sum,item)=>sum+Number(item.quantity||1),0);if(snapshot.draft.receivedNow)receivedOrdersCount++
      }
      ...
      flash(bulkOrderMode==='single'?`Pedido criado · ${createdItems} item(ns)${skippedItems?` · ${skippedItems} ignorado(s)`:''}. Use “Desfazer” no cartão do pedido se precisar reverter.`:`${createdOrders} pedido(s) criado(s) · ${createdItems} peça(s)${skippedItems?` · ${skippedItems} ignorada(s)`:''}.`)
  ...
  function openDirectPurchase(row,mode='buy'){
    ...
    setDirectBuy({phone:row.phone,part:row.part,mode,editingQuoteId:''});
  ...
  function changeOrderItems(orderId,itemId,action){
    ...
    const next=orders.map(order=>{if(order.id!==orderId)return order;
      ...
      const externalItems=(order.externalItems||[]).map(item=>{if(itemId&&item.id!==itemId)return item;if(action==='confirm')return{...item,confirmedAt:item.confirmedAt||stamp};if(action==='receive')return{...item,confirmedAt:item.confirmedAt||stamp,receivedAt:item.receivedAt||stamp};return item});
      return normalizePartsOrder({...order,items,externalItems,orderDate:order.orderDate||(action==='confirm'||action==='receive'?today:''),receivedAt:action==='receive'&&!itemId?stamp:order.receivedAt,updatedAt:stamp})});
  ...
  function undoDraftOrderPreparation(order,itemId=null){
    const targets=(order?.items||[]).filter(item=>!itemId||item.id===itemId);
    if(!targets.length)return flash('Nenhuma peça encontrada para desfazer.');
    ...
    const hasExternal=(order.externalItems||[]).length>0;
    const remainingOrders=remainingItems.length||hasExternal
      ?orders.map(current=>current.id===order.id?normalizePartsOrder({...current,items:remainingItems,updatedAt:stamp}):current)
      :orders.filter(current=>current.id!==order.id);
    const source=withTimeline(phones,affected,itemId?'Preparação da peça desfeita':'Preparação do pedido desfeita');
    const cleared=removePartsOrderLinks(source,{...order,items:targets},remainingOrders);
    persistOrders(remainingOrders,cleared);
    if(!remainingItems.length&&!hasExternal)setExpandedOrderCards(current=>{const next={...current};delete next[order.id];return next});
    setPartsView('orders');
    flash(itemId?'Peça removida do pedido e devolvida à etapa anterior.':'Preparação do pedido desfeita.')
  }
  function undoOrderProgress(order,itemId=null){
    const linkedTargets=(order?.items||[]).filter(item=>!itemId||item.id===itemId),externalTargets=(order?.externalItems||[]).filter(item=>!itemId||item.id===itemId),targets=[...linkedTargets,...externalTargets];
    if(!targets.length)return flash('Nenhuma peça encontrada para desfazer.');
    if(targets.some(item=>item.returnStatus))return flash('Cancele ou conclua a devolução antes de desfazer este recebimento.');
    ...
    if(step==='prepare'){
      if(itemId&&externalTargets.length){
        ...
        return flash('Item avulso removido deste pedido.')
      }
      if(!itemId&&(order.externalItems||[]).length){
        if(!confirm('Desfazer a preparação do pedido completo? Itens BMCenter serão desvinculados e itens avulsos serão removidos deste pedido.'))return;
        const remainingOrders=orders.filter(current=>current.id!==order.id),cleared=removePartsOrderLinks(phones,order,remainingOrders);
        persistOrders(remainingOrders,cleared);setExpandedOrderCards(current=>{const next={...current};delete next[order.id];return next});setPartsView('orders');return flash('Preparação do pedido desfeita.')
      }
    ...
    const next=orders.map(current=>{
      ...
      let linked=current.items||[];
      if(linkedTargets.length){const result=undoPartsOrderStep(current,itemId,step);linked=result.order.items;const changedTargets=(current.items||[]).filter(item=>(!itemId||item.id===itemId)&&(step==='receive'?!!item.receivedAt:!!item.confirmedAt&&!item.receivedAt));affected=changedTargets.map(item=>`${item.phoneId}::${item.partId}`)}
      const externalItems=(current.externalItems||[]).map(item=>{if(itemId&&item.id!==itemId)return item;if(step==='receive'&&item.receivedAt)return{...item,receivedAt:''};if(step==='confirm'&&item.confirmedAt&&!item.receivedAt)return{...item,confirmedAt:''};return item});
      return normalizePartsOrder({...current,items:linked,externalItems,updatedAt:stamp})
    });
    const message=step==='receive'?'Recebimento desfeito':'Confirmação do pedido desfeita',source=withTimeline(phones,affected,message);persistOrders(next,source);setPartsView('orders');flash(step==='receive'?'Recebimento desfeito. O item voltou para Pedidos.':'Confirmação desfeita. O item voltou à etapa anterior à compra.')
  }
  ...
  function changeExternalReturnState(orderId,itemId,state){
    ...
    const next=orders.map(order=>{if(order.id!==orderId)return order;const externalItems=(order.externalItems||[]).map(item=>{
      ...
      if(state==='pending')return{...item,returnStatus:'pending',returnMarkedAt:item.returnMarkedAt||stamp,returnedToSupplierAt:'',resolvedAt:''};
      if(state==='returned')return{...item,returnStatus:'returned',returnMarkedAt:item.returnMarkedAt||stamp,returnedToSupplierAt:item.returnedToSupplierAt||stamp,resolvedAt:''};
      if(state==='resolved')return{...item,returnStatus:'resolved',resolvedAt:stamp};
      return{...item,returnStatus:'',returnMarkedAt:'',returnedToSupplierAt:'',resolvedAt:''}
    });return normalizePartsOrder({...order,externalItems,updatedAt:stamp})});
    persistOrders(next,phones);
    if(state==='pending'){setPartsView('returns');setReturnFilter('pending');flash('Item avulso separado para devolução.')}else if(state==='returned')flash('Item avulso marcado como devolvido.');else if(state==='resolved')flash('Lembrete da devolução avulsa resolvido.');else flash('Devolução avulsa cancelada.')
  }
  function standaloneReturnClean(item={}){const{undoSnapshot,...clean}=item;return clean}
  function emptyStandaloneReturn(){return{id:'',supplier:'',partName:'',reference:'',quantity:'1',unitPrice:'',freight:'',reason:'',notes:'',returnDate:new Date().toISOString().slice(0,10),status:'pending'}}
  function openStandaloneReturn(item=null){
    const base=item?{...standaloneReturnClean(item),quantity:String(item.quantity||1),unitPrice:String(item.unitPrice??''),freight:String(item.freight??'')} : emptyStandaloneReturn();
    setStandaloneReturnEditor(base);setPartsView('returns');setReturnFilter('pending')
  }
  function saveStandaloneReturn(){
    if(!standaloneReturnEditor)return;
    const supplier=String(standaloneReturnEditor.supplier||'').trim(),partName=String(standaloneReturnEditor.partName||'').trim();
    if(!supplier)return flash('Selecione ou informe o fornecedor da devolução.');
    if(!partName)return flash('Informe o item que será devolvido.');
  ...
  function changeStandaloneReturnState(id,state){
    ...
    else if(state==='returned'){changed.status='returned';changed.returnedAt=current.returnedAt||stamp;changed.resolvedAt=''}
    else if(state==='resolved'){changed.status='resolved';changed.resolvedAt=stamp}
    persistStandaloneReturns(standaloneReturns.map(item=>item.id===id?changed:item));
    if(state==='pending'){setReturnFilter('pending');flash('Devolução avulsa reaberta.')}else if(state==='returned'){setReturnFilter('returned');flash('Devolução avulsa marcada como devolvida. Use “Desfazer” para reverter.')}else{setReturnFilter('resolved');flash('Devolução avulsa resolvida. Use “Desfazer” para reverter.')}
  }
  ...
  const returnShipmentRowKey=row=>row.type==='standalone'?`standalone:${row.standalone?.id||row.item.id}`:`${row.type}:${row.order.id}:${row.item.id}`;
  const returnShipmentQty=row=>row.type==='linked'?1:Math.max(1,Math.floor(Number(row.item.quantity||1)||1));
  const pendingReturnRowsForSupplier=supplier=>[...returnRows,...externalReturnRows,...standaloneReturnRows].filter(row=>row.item.returnStatus==='pending'&&row.order.supplier===supplier);
  const shipmentClean=shipment=>{const{undoSnapshot,...clean}=shipment||{};return clean};
  function currentShipmentItem(ref){
    if(ref.type==='standalone')return standaloneReturns.find(item=>item.id===ref.standaloneId)||null;
    const order=orders.find(item=>item.id===ref.orderId);
    return ref.type==='linked'?(order?.items||[]).find(item=>item.id===ref.itemId)||null:(order?.externalItems||[]).find(item=>item.id===ref.itemId)||null
  }
  function openReturnShipment(supplier,initialRow=null){
    const rows=pendingReturnRowsForSupplier(supplier);
    if(!rows.length)return flash('Não há itens separados para devolver a este fornecedor.');
    const selectedKeys=initialRow?[returnShipmentRowKey(initialRow)]:rows.map(returnShipmentRowKey);
    setReturnShipmentEditor({id:'',supplier,shipmentDate:new Date().toISOString().slice(0,10),freight:'',freightRefund:'',notes:'',selectedKeys});
    setPartsView('returns');setReturnFilter('pending')
  }
  function editReturnShipment(shipment){
    setReturnShipmentEditor({id:shipment.id,supplier:shipment.supplier,shipmentDate:shipment.shipmentDate||new Date().toISOString().slice(0,10),freight:String(shipment.freight??''),freightRefund:String(shipment.freightRefund??''),notes:shipment.notes||'',selectedKeys:(shipment.items||[]).map(ref=>ref.key)})
  }
  function restoreShipmentSnapshots(snapshotItems=[]){
    const linkedByOrder=new Map(),externalByOrder=new Map(),standaloneById=new Map();
    snapshotItems.forEach(ref=>{if(!ref?.before)return;if(ref.type==='linked'){if(!linkedByOrder.has(ref.orderId))linkedByOrder.set(ref.orderId,new Map());linkedByOrder.get(ref.orderId).set(ref.itemId,ref.before)}else if(ref.type==='external'){if(!externalByOrder.has(ref.orderId))externalByOrder.set(ref.orderId,new Map());externalByOrder.get(ref.orderId).set(ref.itemId,ref.before)}else if(ref.type==='standalone')standaloneById.set(ref.standaloneId,ref.before)});
    const nextOrders=orders.map(order=>normalizePartsOrder({...order,items:(order.items||[]).map(item=>linkedByOrder.get(order.id)?.get(item.id)||item),externalItems:(order.externalItems||[]).map(item=>externalByOrder.get(order.id)?.get(item.id)||item)}));
    const nextStandalone=standaloneReturns.map(item=>standaloneById.get(item.id)||item);
    persistOrders(nextOrders,phones);persistStandaloneReturns(nextStandalone)
  }
  function saveReturnShipment(){
    if(!returnShipmentEditor)return;
    const supplier=String(returnShipmentEditor.supplier||'').trim(),freight=Math.max(0,toNumber(returnShipmentEditor.freight)),freightRefund=Math.max(0,toNumber(returnShipmentEditor.freightRefund));
    if(!supplier)return flash('Informe o fornecedor da remessa.');
    if(!returnShipmentEditor.shipmentDate)return flash('Informe a data do envio da devolução.');
    if(freightRefund>freight+.001)return flash('O frete recuperado não pode ser maior que o frete pago na devolução.');
    const existing=returnShipmentEditor.id?returnShipments.find(item=>item.id===returnShipmentEditor.id):null;
    let rows=[];
    if(existing){
      rows=(existing.items||[]).map(ref=>{const item=currentShipmentItem(ref);if(!item)return null;const order=ref.type==='standalone'?{id:`standalone-${ref.standaloneId}`,supplier}:orders.find(order=>order.id===ref.orderId);return{type:ref.type,order,item,standalone:ref.type==='standalone'?item:null}}).filter(Boolean)
    }else{
      const selected=new Set(returnShipmentEditor.selectedKeys||[]);
    ...
    const stamp=new Date().toISOString(),shipmentId=existing?.id||crypto.randomUUID();
    const refs=rows.map(row=>{const key=returnShipmentRowKey(row),base={key,type:row.type,orderId:row.type==='standalone'?'':row.order.id,itemId:row.type==='standalone'?'':row.item.id,standaloneId:row.type==='standalone'?(row.standalone?.id||row.item.id):'',quantity:returnShipmentQty(row),partName:row.item.partName||'Item',reference:row.item.reference||row.item.phoneLabel||'',freightShare:freightAllocation.get(key)||0,freightRefundShare:refundAllocation.get(key)||0};return existing?{...(existing.items||[]).find(item=>item.key===key),...base}:{...base,before:{...(row.type==='standalone'?(row.standalone||row.item):row.item)}}});
    const editSnapshots=existing?refs.map(ref=>({...ref,before:{...currentShipmentItem(ref)}})):null;
    const linkedUpdates=new Map(),externalUpdates=new Map(),standaloneUpdates=new Map();
    refs.forEach(ref=>{
      const current=currentShipmentItem(ref);if(!current)return;
      const common={returnShipmentId:shipmentId,returnShippingDate:returnShipmentEditor.shipmentDate,returnShippingFreightShare:ref.freightShare,returnShippingFreightRefundShare:ref.freightRefundShare};
      if(ref.type==='linked'){
        if(!linkedUpdates.has(ref.orderId))linkedUpdates.set(ref.orderId,new Map());
        linkedUpdates.get(ref.orderId).set(ref.itemId,{...current,...common,returnStatus:'returned',returnMarkedAt:current.returnMarkedAt||stamp,returnedToSupplierAt:current.returnedToSupplierAt||stamp,returnFinancialStatus:current.returnFinancialStatus||'pending',returnPartRefund:current.returnFinancialStatus?Number(current.returnPartRefund||0):Number(current.price||0),returnFreightRefund:Number(current.returnFreightRefund||0),returnRefundMethod:current.returnRefundMethod||'',returnRefundDate:current.returnRefundDate||'',returnFinancialUpdatedAt:current.returnFinancialUpdatedAt||stamp})
      }else if(ref.type==='external'){
        if(!externalUpdates.has(ref.orderId))externalUpdates.set(ref.orderId,new Map());
        externalUpdates.get(ref.orderId).set(ref.itemId,{...current,...common,returnStatus:'returned',returnMarkedAt:current.returnMarkedAt||stamp,returnedToSupplierAt:current.returnedToSupplierAt||stamp,resolvedAt:''})
      }else standaloneUpdates.set(ref.standaloneId,{...current,...common,status:'returned',returnedAt:current.returnedAt||stamp,resolvedAt:''})
    });
    const nextOrders=orders.map(order=>normalizePartsOrder({...order,items:(order.items||[]).map(item=>linkedUpdates.get(order.id)?.get(item.id)||item),externalItems:(order.externalItems||[]).map(item=>externalUpdates.get(order.id)?.get(item.id)||item),updatedAt:stamp}));
    const nextStandalone=standaloneReturns.map(item=>standaloneUpdates.get(item.id)||item);
    const shipment={id:shipmentId,supplier,shipmentDate:returnShipmentEditor.shipmentDate,freight,freightRefund,notes:String(returnShipmentEditor.notes||'').trim(),items:refs,unitCount:refs.reduce((sum,ref)=>sum+Number(ref.quantity||1),0),createdAt:existing?.createdAt||stamp,updatedAt:stamp,undoSnapshot:existing?{shipment:shipmentClean(existing),items:editSnapshots}:null};
    persistOrders(nextOrders,phones);persistStandaloneReturns(nextStandalone);persistReturnShipments(existing?returnShipments.map(item=>item.id===shipmentId?shipment:item):[shipment,...returnShipments]);setReturnShipmentEditor(null);setReturnFilter('returned');flash(existing?'Remessa atualizada. Use “Desfazer” para reverter a última alteração.':'Remessa de devolução registrada. O frete foi rateado pela quantidade de unidades.')
  }
  function undoReturnShipment(id){
    const shipment=returnShipments.find(item=>item.id===id);if(!shipment)return;
    if(shipment.undoSnapshot){
      restoreShipmentSnapshots(shipment.undoSnapshot.items||[]);
      persistReturnShipments(returnShipments.map(item=>item.id===id?{...shipment.undoSnapshot.shipment,undoSnapshot:null}:item));
      setReturnFilter('returned');flash('Última alteração da remessa desfeita.');return
    }
  ...
  function saveOrderEdit(){
    ...
    const stamp=new Date().toISOString();
    const edited=normalizePartsOrder({...current,...orderEditor,supplier:String(orderEditor.supplier||'').trim()||'Fornecedor não definido',freight:toNumber(orderEditor.freight),items:orderEditor.items.map(item=>({...item,price:toNumber(item.price)})),externalItems:(orderEditor.externalItems||[]).map(item=>({...item,quantity:Math.max(1,Math.floor(Number(item.quantity||1)||1)),unitPrice:toNumber(item.unitPrice)})),updatedAt:stamp});
  ...
  function copyOrder(order){
    const linkedLines=(order.items||[]).map(item=>`• ${item.partName} — ${item.phoneLabel||'Aparelho'} — ${money(item.price)}`);
    const externalLines=(order.externalItems||[]).map(item=>`• ${item.partName} — ${item.quantity||1} un. × ${money(item.unitPrice)}${item.reference?` — ${item.reference}`:''}`);
    const blocks=[];if(linkedLines.length)blocks.push(`Peças BMCenter:
${linkedLines.join('\n')}`);if(externalLines.length)blocks.push(`Itens avulsos:
${externalLines.join('\n')}`);
    const text=`Olá! Quero confirmar este pedido:

${blocks.join('\n\n')}

Peças BMCenter: ${money(order.linkedSubtotal||0)}
Itens avulsos: ${money(order.externalSubtotal||0)}
...
Total: ${money(order.total)}`;
  ...
  function OrderCard({order,receivedHistory=false}){
    const externalItems=order.externalItems||[],allItems=[...(order.items||[]),...externalItems];
    const allConfirmed=allItems.length&&allItems.every(item=>item.confirmedAt),allReceived=allItems.length&&allItems.every(item=>item.receivedAt),anyConfirmed=allItems.some(item=>item.confirmedAt),anyReceived=allItems.some(item=>item.receivedAt);
    const externalQty=externalItems.reduce((sum,item)=>sum+Number(item.quantity||1),0),expanded=!!expandedOrderCards[order.id];
    const toggle=()=>setExpandedOrderCards(current=>({...current,[order.id]:!current[order.id]}));
    const headerHint=receivedHistory?'Histórico recebido':allReceived?'Tudo recebido':anyReceived?'Recebimento parcial':allConfirmed?'Pronto para receber':anyConfirmed?'Confirmação parcial':'Aguardando confirmação';
    const badgeTone=receivedHistory?'received':orderVisualType(order);
    return <article className={`parts-v48-order-card status-${order.status} parts-v50-order-card parts-v10532-order-card parts-v10532-tone-${badgeTone} ${receivedHistory?'parts-v49-received-card':''} ${expanded?'expanded':'collapsed'} ${externalItems.length?'parts-v10528-mixed-order':''}`}>
      <header className="parts-v50-order-summary"><div className="parts-v48-order-title"><span><Store size={14}/></span><div><small>{orderStatusLabel(order.status).toUpperCase()}</small><h3>{order.supplier}</h3><p>{order.items.length?`${order.items.length} BMCenter`:''}{order.items.length&&externalQty?' · ':''}{externalQty?`${externalQty} avulso(s)`:''}{order.orderDate?` · Pedido ${formatDate(order.orderDate)}`:''}</p><div className="parts-v10532-order-badges"><span className="parts-v10532-badge type">{orderTypeLabel(order)}</span><span className="parts-v10532-badge origin">{orderOriginLabel(order)}</span>{receivedHistory&&<span className="parts-v10532-badge history">Recebido</span>}</div></div></div><div className="parts-v48-order-total parts-v10532-order-total"><small>{headerHint}</small><strong>{money(order.total)}</strong><span>Frete {money(order.freight)}</span></div><div className="parts-v10514-summary-actions">{!expanded&&<button type="button" className="parts-v10514-undo-summary" title={receivedHistory&&allReceived?'Desfazer recebimento do pedido completo':anyReceived?'Desfazer último recebimento do pedido':anyConfirmed?'Desfazer confirmação do pedido':'Desfazer preparação do pedido'} onClick={()=>undoOrderProgress(order)}><RotateCcw size={13}/><span>Desfazer</span></button>}<button type="button" className="parts-v50-expand-button" title={expanded?'Fechar detalhes':'Ver detalhes'} onClick={toggle}><ChevronDown size={16}/></button></div></header>
      {expanded&&<><div className="parts-v48-order-lines">{order.items.map(item=>{const phone=phones.find(p=>p.id===item.phoneId);return <div className={item.receivedAt?'received':item.confirmedAt?'confirmed':''} key={item.id}><span className="state">{item.receivedAt?'✓':item.confirmedAt?'●':'○'}</span><div className="identity"><b>{item.partName}</b><small>{phone?phoneDisplayName(phone,{includeCode:false}):item.phoneLabel||'Aparelho'}{phone?.code?` · ${phone.code}`:''}</small></div><div className="cost"><small>Peça</small><b>{money(item.price)}</b></div><div className="cost"><small>Frete rateado</small><b>{money(item.freightShare)}</b></div><div className="cost effective"><small>Custo efetivo</small><b>{money(item.effectiveCost)}</b></div><div className="parts-v48-line-actions">{!item.confirmedAt&&<><button onClick={()=>changeOrderItems(order.id,item.id,'confirm')}><CheckSquare size={12}/> Confirmar</button><button className="parts-v10514-undo-button" onClick={()=>undoOrderProgress(order,item.id)}><RotateCcw size={12}/> Desfazer</button></>}{item.confirmedAt&&!item.receivedAt&&<><button className="primary" onClick={()=>changeOrderItems(order.id,item.id,'receive')}><Package size={12}/> Receber</button><button className="parts-v10514-undo-button" onClick={()=>undoOrderProgress(order,item.id)}><RotateCcw size={12}/> Desfazer</button></>}{item.receivedAt&&!item.returnStatus&&<><span className="received-label">Recebida</span><button className="parts-v10514-undo-button" onClick={()=>undoOrderProgress(order,item.id)}><RotateCcw size={12}/> Desfazer</button><button className="parts-v79-return-button" onClick={()=>changeReturnState(order.id,item.id,'pending')}><RotateCcw size={12}/> Devolver</button></>}{item.returnStatus==='pending'&&<><span className="parts-v79-return-label pending">Para devolver</span><button onClick={()=>changeReturnState(order.id,item.id,'cancel')}>Cancelar</button><button className="primary" onClick={()=>openReturnShipment(order.supplier,{order,item,phone,type:'linked'})}><Package size={12}/> Registrar envio</button></>}{item.returnStatus==='returned'&&<><span className="parts-v79-return-label returned">Devolvida</span><span className={`parts-v84-finance-mini ${['received','supplier_credit'].includes(item.returnFinancialStatus)?'settled':'pending'}`}>{['received','supplier_credit'].includes(item.returnFinancialStatus)?`Recuperado ${money(returnRecoveredAmount(item))}`:'Financeiro pendente'}</span></>}</div></div>})}</div>
      {!!externalItems.length&&<section className="parts-v10528-external-order-lines"><header><div><b>Itens avulsos</b><small>Somente controle deste pedido · não entram nos custos dos aparelhos</small></div><strong>{money(order.externalSubtotal||0)}</strong></header><div>{externalItems.map(item=><article className={`${item.receivedAt?'received':item.confirmedAt?'confirmed':''} ${item.returnStatus?`return-${item.returnStatus}`:''}`} key={item.id}><span className="state">{item.returnStatus==='resolved'?'✓':item.returnStatus==='returned'?'↩':item.returnStatus==='pending'?'!':item.receivedAt?'✓':item.confirmedAt?'●':'○'}</span><div className="identity"><b>{item.partName}</b><small>{item.reference||'Sem referência'} · {item.quantity||1} un. × {money(item.unitPrice)}</small></div><div className="cost"><small>Itens</small><b>{money(item.price)}</b></div><div className="cost"><small>Frete rateado</small><b>{money(item.freightShare)}</b></div><div className="cost effective"><small>Total</small><b>{money(item.effectiveCost)}</b></div><div className="parts-v48-line-actions">{!item.confirmedAt&&<><button onClick={()=>changeOrderItems(order.id,item.id,'confirm')}><CheckSquare size={12}/> Confirmar</button><button className="parts-v10514-undo-button" onClick={()=>undoOrderProgress(order,item.id)}><RotateCcw size={12}/> Remover</button></>}{item.confirmedAt&&!item.receivedAt&&<><button className="primary" onClick={()=>changeOrderItems(order.id,item.id,'receive')}><Package size={12}/> Receber</button><button className="parts-v10514-undo-button" onClick={()=>undoOrderProgress(order,item.id)}><RotateCcw size={12}/> Desfazer</button></>}{item.receivedAt&&!item.returnStatus&&<><span className="received-label">Recebido</span><button className="parts-v79-return-button" onClick={()=>changeExternalReturnState(order.id,item.id,'pending')}><RotateCcw size={12}/> Devolver</button></>}{item.returnStatus==='pending'&&<><span className="parts-v79-return-label pending">Separado</span><button onClick={()=>changeExternalReturnState(order.id,item.id,'cancel')}>Cancelar</button><button className="primary" onClick={()=>openReturnShipment(order.supplier,{order,item,phone:null,type:'external'})}><Package size={12}/> Registrar envio</button></>}{item.returnStatus==='returned'&&<><span className="parts-v79-return-label returned">Devolvido</span><button className="primary" onClick={()=>changeExternalReturnState(order.id,item.id,'resolved')}>Resolver lembrete</button>{item.returnShipmentId?<button className="parts-v10514-undo-button" onClick={()=>undoReturnShipment(item.returnShipmentId)}><RotateCcw size={12}/> Desfazer remessa</button>:<button onClick={()=>changeExternalReturnState(order.id,item.id,'pending')}><RotateCcw size={12}/> Reabrir</button>}</>}{item.returnStatus==='resolved'&&<><span className="parts-v10528-resolved-label">Resolvido</span>{item.returnShipmentId?<button className="parts-v10514-undo-button" onClick={()=>undoReturnShipment(item.returnShipmentId)}><RotateCcw size={12}/> Desfazer remessa</button>:<button onClick={()=>changeExternalReturnState(order.id,item.id,'pending')}><RotateCcw size={12}/> Reabrir</button>}</>}</div></article>)}</div></section>}
      <footer><div><span>BMCenter <b>{money(order.linkedSubtotal||0)}</b></span>{externalItems.length>0&&<span>Avulsos <b>{money(order.externalSubtotal||0)}</b></span>}<span>Frete <b>{money(order.freight)}</b></span>{order.returnedRecovered>0&&<><span className="parts-v84-recovered">Recuperado <b>{money(order.returnedRecovered)}</b></span><span>Custo BMCenter líquido <b>{money(order.netCost)}</b></span></>}</div><div><button onClick={()=>copyOrder(order)}><Copy size={12}/> Copiar</button><button onClick={()=>setOrderEditor(JSON.parse(JSON.stringify(order)))}><FileText size={12}/> Editar pedido</button>{isBulkOrder(order)&&<button className="danger" onClick={()=>deleteOrder(order)}><Trash2 size={12}/> Excluir pedido</button>}{anyReceived?<button className="parts-v10514-undo-button" onClick={()=>undoOrderProgress(order)}><RotateCcw size={12}/> Desfazer recebimentos</button>:anyConfirmed?<button className="parts-v10514-undo-button" onClick={()=>undoOrderProgress(order)}><RotateCcw size={12}/> Desfazer confirmação</button>:<button className="parts-v10514-undo-button" onClick={()=>undoOrderProgress(order)}><RotateCcw size={12}/> Desfazer pedido</button>}{!allConfirmed&&<button className="primary" onClick={()=>changeOrderItems(order.id,null,'confirm')}><CheckSquare size={12}/> Confirmar tudo</button>}{allConfirmed&&!allReceived&&<button className="primary success-button" onClick={()=>changeOrderItems(order.id,null,'receive')}><Package size={12}/> Receber tudo</button>}</div></footer></>}
    </article>
  }
  ...
  function QuoteCard({row}){
    ...
    const summaryQuote=row.quotes.find(q=>q.id===selectedId)||row.cheapest||sorted[0]||null;
    return <article className={`parts-v48-order-card parts-v50-quote-card ${expanded?'expanded':'collapsed'}`}>
      <header className="parts-v50-order-summary"><div className="parts-v48-order-title"><span><Tags size={14}/></span><div><small>COTAÇÕES</small><h3>{row.part.name}</h3><p>{phoneDisplayName(row.phone,{includeCode:false})} · {row.phone.code}</p></div></div><div className="parts-v48-order-total"><small>Menor preço</small><strong>{money(row.cheapest?.price||0)}</strong><span>{row.quotes.length} fornecedor(es)</span></div><div className="parts-v10514-summary-actions parts-v10516-quote-summary-actions"><button type="button" className="parts-v10516-edit-summary" title="Editar cotação" onClick={()=>editQuote(row,summaryQuote)}><Pencil size={13}/><span>Editar</span></button><button type="button" className="parts-v10514-undo-summary" title="Desfazer cotações e voltar para Em aberto" onClick={()=>undoQuoteStage(row)}><RotateCcw size={13}/><span>Desfazer</span></button><button type="button" className="parts-v50-expand-button" title={expanded?'Fechar cotações':'Ver cotações'} onClick={()=>setExpandedQuoteRows(current=>({...current,[key]:!current[key]}))}><ChevronDown size={16}/></button></div></header>
      {expanded&&<><div className="parts-v50-quote-lines">{sorted.map(q=><div className={selectedId===q.id?'selected':''} key={q.id}><span className="state">{selectedId===q.id?'✓':'○'}</span><div className="identity"><b>{q.supplier}</b><small>{q.notes||'Cotação cadastrada'}</small></div><div className="cost effective"><small>Valor</small><b>{money(q.price)}</b></div><div className="parts-v48-line-actions parts-v10516-quote-line-actions"><button className={selectedId===q.id?'primary':''} onClick={()=>chooseQuote(row,q.id)}>{selectedId===q.id?'Selecionada':'Selecionar'}</button><button onClick={()=>editQuote(row,q)}><Pencil size={12}/> Editar</button><button className="parts-v10514-undo-button" onClick={()=>undoQuoteStage(row,q)}><RotateCcw size={12}/> Desfazer</button></div></div>)}</div><footer><div><span>{row.quotes.length} cotação(ões)</span>{rec&&<span>Recomendação <b>{rec.supplier}</b></span>}</div><div><button className="primary" onClick={()=>{if(row.cheapest)chooseQuote(row,row.cheapest.id)}}><CheckSquare size={12}/> Menor preço</button><button onClick={()=>openDirectPurchase(row,'quote')}><Plus size={12}/> Nova cotação</button></div></footer></>}
    </article>
  }

  return <><div className="parts-command-page parts-v10447 parts-v10448 parts-v10450">
    <header className="parts-v47-head parts-v10518-head"><div className="parts-v10535-head-main"><span>CENTRAL DE PEÇAS</span><h1>Pedidos e peças</h1><p>Pedidos de peças dos smartphones e itens avulsos do mesmo fornecedor, sem misturar custos.</p><div className="parts-v10535-inline-filters"><select value={overviewPeriod} onChange={e=>setOverviewPeriod(e.target.value)}><option value="today">Hoje</option><option value="7d">7 dias</option><option value="30d">30 dias</option><option value="90d">90 dias</option><option value="all">Tudo</option></select><select value={overviewSupplier} onChange={e=>setOverviewSupplier(e.target.value)}><option value="Todos">Todos os fornecedores</option>{overviewSuppliers.map(name=><option key={name}>{name}</option>)}</select><select value={overviewType} onChange={e=>setOverviewType(e.target.value)}><option value="Todos">Todos os tipos</option><option value="bmcenter">Peças BMCenter</option><option value="avulso">Itens avulsos</option><option value="misto">Mistos</option></select><select value={overviewOrigin} onChange={e=>setOverviewOrigin(e.target.value)}><option value="Todos">Todas as origens</option><option value="manual">Manual</option><option value="bulk">Pedido em massa</option><option value="direct">Compra direta</option><option value="shipment">Remessa de devolução</option></select><button type="button" className="parts-v10518-clear parts-v10536-icon-clear" title="Limpar filtros globais" disabled={!overviewHasFilters} onClick={clearOverviewFilters}><RotateCcw size={13}/></button></div>{activeOverviewChips.length>0&&<div className="parts-v10531-filter-chips parts-v10535-filter-chips">{activeOverviewChips.map(chip=><button type="button" key={chip.key} className="parts-v10531-filter-chip" onClick={chip.clear}><span>{chip.label}</span><X size={12}/></button>)}</div>}</div><div className="parts-v47-summary parts-v10518-summary"><button type="button" className={overviewDetail==='open'?'active':''} onClick={()=>openOverview('open')}><b>{overviewCounts.open}</b> em aberto</button><button type="button" className={overviewDetail==='waiting'?'active':''} onClick={()=>openOverview('waiting')}><b>{waitingItems}</b> aguardando</button><button type="button" className={overviewDetail==='received'?'active':''} onClick={()=>openOverview('received')}><b>{receivedItems}</b> recebidas</button><button type="button" className={`${returnAttentionCount?'attention ':''}${overviewDetail==='returns'?'active':''}`} onClick={()=>openOverview('returns')}><b>{overviewCounts.returns}</b> devoluções pendentes</button></div><div className="parts-v47-head-actions"><button className="primary" onClick={()=>openBulkOrder('single')}><Plus size={14}/> Novo pedido</button><button onClick={()=>openBulkOrder('bulk')}><ShoppingCart size={14}/> Pedido em massa</button><button onClick={()=>{setShowAdd(true);setTimeout(()=>deviceSearchRef.current?.focus(),50)}}><Plus size={14}/> Adicionar à lista</button><button onClick={()=>startQuickQuote()}><Tags size={13}/> Lançar cotação</button><button className="icon-only" title="Configurações de frete" onClick={()=>setShowRules(true)}><Settings size={14}/></button></div></header>
    {overviewDetail&&<section className="parts-v10518-overview-detail"><header><div><small>RESUMO DETALHADO</small><h3>{overviewDetail==='open'?`${overviewCounts.open} peça(s) em aberto`:overviewDetail==='waiting'?`${waitingItems} peça(s) aguardando recebimento`:overviewDetail==='received'?`${receivedItems} peça(s) recebida(s)`:`${overviewCounts.returns} devolução(ões) pendente(s)`}</h3></div><button type="button" className="icon-only" onClick={()=>setOverviewDetail('')}><X size={14}/></button></header><div className="parts-v10518-overview-list">{overviewDetailRows.map((row,index)=>{const phone=row.phone||null,part=row.part||row.item||{},order=row.order||null,isExternal=['external','standalone'].includes(row.type),isStandalone=row.type==='standalone';const key=overviewDetail==='open'?`${phone?.id||'phone'}-${part.id||index}`:`${order?.id||'order'}-${part.id||index}`;const supplier=order?.supplier||row.chosen?.supplier||row.cheapest?.supplier||part.purchaseSupplier||'Sem fornecedor';const status=overviewDetail==='open'?(row.quotes?.length?'Com cotação':'Sem cotação'):overviewDetail==='waiting'?'Aguardando recebimento':overviewDetail==='received'?'Recebida':isStandalone?(part.returnStatus==='pending'?'Avulsa independente':'Avulsa devolvida'):isExternal?(part.returnStatus==='pending'?'Avulso separado':'Avulso devolvido'):part.returnStatus==='pending'?'Para devolver':'Financeiro pendente';return <article key={key} className={isExternal?'parts-v10528-overview-external':''}><div className="parts-v10518-overview-identity"><b>{part.name||part.partName||'Peça'}</b><span>{isExternal?(part.reference||'Item avulso'):(phone?phoneDisplayName(phone,{includeCode:false}):part.phoneLabel||'Aparelho')}{!isExternal&&phone?.code?` · ${phone.code}`:''}</span></div><div className="parts-v10518-overview-order"><small>{supplier}</small><b>{status}</b></div><button type="button" onClick={()=>{setPartsView(overviewDetail==='open'?'needs':overviewDetail==='waiting'?'orders':overviewDetail==='received'?'received':'returns');setQuery(isExternal?(part.reference||part.partName||''):(phone?.code||part.phoneLabel||part.name||part.partName||''));setOverviewDetail('')}}>Abrir</button></article>})}{!overviewDetailRows.length&&<div className="parts-v47-empty"><Package size={18}/><div><b>Nenhum registro neste filtro</b><span>Altere os filtros gerais acima.</span></div></div>}</div></section>}
    ...
    <section className="parts-v47-workspace parts-v50-workspace"><nav className="parts-v47-tabs"><button className={partsView==='needs'?'active':''} onClick={()=>setPartsView('needs')}><Package size={14}/> Em aberto <b>{overviewCounts.open}</b></button><button className={partsView==='quotes'?'active':''} onClick={()=>setPartsView('quotes')}><Tags size={14}/> Cotações <b>{overviewCounts.quotes}</b></button><button className={partsView==='orders'?'active':''} onClick={()=>setPartsView('orders')}><ShoppingCart size={14}/> Pedidos <b>{overviewCounts.orders}</b></button><button className={partsView==='received'?'active':''} onClick={()=>setPartsView('received')}><CheckSquare size={14}/> Recebidos <b>{overviewCounts.received}</b></button><button className={partsView==='returns'?'active parts-v79-return-tab':''} onClick={()=>setPartsView('returns')}><RotateCcw size={14}/> Devoluções <b>{overviewCounts.returns}</b></button></nav>
      <div className="parts-v50-filters parts-v10531-context-filters"><label className="parts-v47-search"><Search size={14}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Busca contextual da aba..."/></label>{partsView==='needs'&&<><select value={partFilter} onChange={e=>setPartFilter(e.target.value)}><option value="Todos">Todas as peças</option>{partNames.map(name=><option key={name}>{name}</option>)}</select><select value={phoneStatusPartsFilter} onChange={e=>setPhoneStatusPartsFilter(e.target.value)}><option value="Todos">Todos os status dos aparelhos</option>{statuses.filter(status=>!['Vendido','Descarte/Sucata'].includes(status)).map(status=><option key={status}>{status}</option>)}</select><select value={needsSituationFilter} onChange={e=>setNeedsSituationFilter(e.target.value)}><option value="Todos">Toda situação</option><option>Sem cotação</option><option>Com cotação</option></select><select value={rowSort} onChange={e=>setRowSort(e.target.value)}><option value="device">Ordenar por aparelho</option><option value="part">Ordenar por peça</option><option value="status">Ordenar por status</option></select><button className="parts-v50-clear-filters" title="Limpar filtros" onClick={clearRowFilters}><RotateCcw size={13}/></button></>}{partsView==='quotes'&&<><select value={partFilter} onChange={e=>setPartFilter(e.target.value)}><option value="Todos">Todas as peças</option>{partNames.map(name=><option key={name}>{name}</option>)}</select><select value={quoteSupplierFilter} onChange={e=>setQuoteSupplierFilter(e.target.value)}><option value="Todos">Todos os fornecedores da cotação</option>{quoteSuppliers.map(name=><option key={name}>{name}</option>)}</select><select value={quoteSort} onChange={e=>setQuoteSort(e.target.value)}><option value="best_price">Menor preço</option><option value="supplier">Fornecedor</option><option value="part">Peça</option></select><button className="parts-v50-clear-filters" title="Limpar filtros" onClick={clearRowFilters}><RotateCcw size={13}/></button></>}{partsView==='orders'&&<><select value={orderStatusFilter} onChange={e=>setOrderStatusFilter(e.target.value)}><option value="Todos">Toda situação do pedido</option><option value="draft">Rascunho</option><option value="partial_ordered">Pedido parcial</option><option value="ordered">Pedido realizado</option><option value="partial_received">Recebimento parcial</option></select><select value={orderFreightFilter} onChange={e=>setOrderFreightFilter(e.target.value)}><option value="Todos">Frete: todos</option><option value="with_freight">Com frete</option><option value="without_freight">Sem frete</option></select><select value={orderSort} onChange={e=>setOrderSort(e.target.value)}><option value="recent">Mais recente</option><option value="oldest">Mais antigo</option><option value="supplier">Fornecedor</option><option value="total">Maior valor</option></select><button className="parts-v50-clear-filters" title="Restaurar filtros da aba" onClick={()=>{setOrderStatusFilter('Todos');setOrderFreightFilter('Todos');setOrderSort('recent');setQuery('')}}><RotateCcw size={13}/></button></>}{partsView==='received'&&<><select value={receivedSituationFilter} onChange={e=>setReceivedSituationFilter(e.target.value)}><option value="Todos">Toda situação</option><option value="pending_return">Com devolução pendente</option><option value="finalized">Somente finalizados</option><option value="mixed">Pedidos mistos</option><option value="external">Com itens avulsos</option></select><select value={orderFreightFilter} onChange={e=>setOrderFreightFilter(e.target.value)}><option value="Todos">Frete: todos</option><option value="with_freight">Com frete</option><option value="without_freight">Sem frete</option></select><select value={orderSort} onChange={e=>setOrderSort(e.target.value)}><option value="recent">Recebimento mais recente</option><option value="oldest">Recebimento mais antigo</option><option value="supplier">Fornecedor</option><option value="total">Maior valor</option></select><button className="parts-v50-clear-filters" title="Restaurar filtros da aba" onClick={()=>{setReceivedSituationFilter('Todos');setOrderFreightFilter('Todos');setOrderSort('recent');setQuery('')}}><RotateCcw size={13}/></button></>}{partsView==='returns'&&<><select value={returnFilter} onChange={e=>setReturnFilter(e.target.value)}><option value="pending">Para devolver</option><option value="financial">Financeiro pendente</option><option value="returned">Já devolvidas</option><option value="resolved">Avulsas resolvidas</option><option value="all">Todas</option></select><select value={returnShipmentFilter} onChange={e=>setReturnShipmentFilter(e.target.value)}><option value="Todos">Remessa: todas</option><option value="with_shipment">Com remessa</option><option value="without_shipment">Sem remessa</option></select><select value={returnSort} onChange={e=>setReturnSort(e.target.value)}><option value="recent">Mais recente</option><option value="supplier">Fornecedor</option><option value="value">Maior valor</option></select><button className="parts-v50-clear-filters" title="Restaurar filtros da aba" onClick={()=>{setReturnFilter('pending');setReturnShipmentFilter('Todos');setReturnSort('recent');setQuery('')}}><RotateCcw size={13}/></button></>}</div>

      <section className="parts-v10532-focus-bar"><div><small>{viewFocusSummary.eyebrow}</small><b>{viewFocusSummary.title}</b><span>{viewFocusSummary.text}</span></div></section>
      ...
      {partsView==='returns'&&<div className="parts-v79-returns parts-v84-returns parts-v10528-returns">
        <section className="parts-v10529-return-command"><div><span><Bookmark size={17}/></span><div><b>Devolução avulsa independente</b><small>Registre uma devolução ao fornecedor mesmo quando não existe pedido anterior no BMCenter.</small></div></div><button type="button" className="primary" onClick={()=>openStandaloneReturn()}><Plus size={13}/> Nova devolução avulsa</button></section>
        {!!returnSeparatedBySupplier.length&&<section className="parts-v10528-return-summary"><header><div><small>SEPARADO PARA DEVOLVER</small><b>Valor físico por fornecedor</b></div><span>{money(returnSeparatedBySupplier.reduce((sum,row)=>sum+row.total,0))}</span></header><div>{returnSeparatedBySupplier.map(row=><article key={row.supplier}><div><b>{row.supplier}</b><small>{row.count} item(ns) separados</small></div><strong>{money(row.total)}</strong><span>BMCenter {money(row.linkedValue)} · Avulsos {money(row.externalValue)}</span><button type="button" className="primary parts-v10530-ship-button" onClick={()=>openReturnShipment(row.supplier)}><Package size={12}/> Registrar envio</button></article>)}</div></section>}
        {!!visibleReturnShipments.length&&<section className="parts-v10530-shipment-history"><header><div><small>REMESSAS DE DEVOLUÇÃO</small><b>Frete de devolução por quantidade</b></div><span>{visibleReturnShipments.length}</span></header><div>{visibleReturnShipments.map(shipment=>{const units=Math.max(1,Number(shipment.unitCount||0)||1),linked=(shipment.items||[]).filter(item=>item.type==='linked').reduce((sum,item)=>sum+Number(item.quantity||1),0),external=units-linked,net=Math.max(0,Number(shipment.freight||0)-Number(shipment.freightRefund||0));return <article key={shipment.id}><div className="parts-v10530-shipment-title"><span><Package size={14}/></span><div><b>{shipment.supplier}</b><small>{formatDate(shipment.shipmentDate)} · {units} unidade(s) · {linked} BMCenter{external?` · ${external} avulsa(s)`:''}</small></div></div><div><small>Frete pago</small><b>{money(shipment.freight)}</b></div><div><small>Por unidade</small><b>{money(Number(shipment.freight||0)/units)}</b></div><div><small>Recuperado</small><b>{money(shipment.freightRefund)}</b></div><div><small>Frete líquido</small><strong>{money(net)}</strong></div><div className="parts-v10530-shipment-actions"><button onClick={()=>editReturnShipment(shipment)}><Pencil size={12}/> Editar frete</button><button className="parts-v10514-undo-button" onClick={()=>undoReturnShipment(shipment.id)}><RotateCcw size={12}/> Desfazer</button></div></article>})}</div></section>}
        {!!visibleReturnRows.length&&<section className="parts-v10528-return-group"><header><div><b>Devoluções dos aparelhos BMCenter</b><small>Continuam com o fluxo financeiro e de custo normal do sistema.</small></div><span>{visibleReturnRows.length}</span></header><div>{visibleReturnRows.map(row=>{const recovered=returnRecoveredAmount(row.item),refund=returnRefundTotal(row.item),financialSettled=['received','supplier_credit'].includes(row.item.returnFinancialStatus),financialPending=row.item.returnStatus==='returned'&&!financialSettled;return <article className={`parts-v79-return-card parts-v84-return-card ${row.item.returnStatus} ${financialPending?'financial-pending':''}`} key={`${row.order.id}-${row.item.id}`}><span className="parts-v79-return-icon"><RotateCcw size={15}/></span><div className="parts-v79-return-identity"><small>{row.order.supplier}</small><b>{row.item.partName}</b><span>{row.phone?phoneDisplayName(row.phone,{includeCode:false}):row.item.phoneLabel||'Aparelho'}{row.phone?.code?` · ${row.phone.code}`:''}</span></div><div className="parts-v79-return-meta"><small>Custo original</small><b>{money(row.item.effectiveCost||row.item.price)}</b></div><div className="parts-v79-return-meta parts-v84-return-finance"><small>{financialSettled?'Recuperado':financialPending?'A recuperar':'Financeiro'}</small><b>{row.item.returnStatus==='pending'?'—':money(financialSettled?recovered:refund)}</b></div><div className="parts-v79-return-meta parts-v84-return-net"><small>Custo líquido</small><b>{money(effectivePartCost(row.item))}</b>{row.item.returnShippingFreightShare>0&&<small>Frete devolução líquido {money(returnShippingNetCost(row.item))}</small>}</div><div className="parts-v79-return-actions">{row.item.returnStatus==='pending'?<><span>Para devolver</span><button onClick={()=>changeReturnState(row.order.id,row.item.id,'cancel')}>Cancelar</button><button className="primary" onClick={()=>openReturnShipment(row.order.supplier,row)}><Package size={12}/> Registrar envio</button></>:<><span className={financialSettled?'returned':'parts-v84-finance-pending'}>{financialSettled?(row.item.returnFinancialStatus==='supplier_credit'?`Crédito ${money(recovered)}`:`Reembolso ${money(recovered)}`):`Financeiro pendente${refund?` ${money(refund)}`:''}`}</span><button className={financialPending?'primary':''} onClick={()=>openReturnSettlement(row)}>{financialPending?'Resolver financeiro':'Editar financeiro'}</button>{row.item.returnShipmentId?<button className="parts-v10514-undo-button" onClick={()=>undoReturnShipment(row.item.returnShipmentId)}><RotateCcw size={12}/> Desfazer remessa</button>:<button onClick={()=>changeReturnState(row.order.id,row.item.id,'pending')}><RotateCcw size={12}/> Reabrir</button>}</>}</div></article>})}</div></section>}
        {!!visibleExternalReturnRows.length&&<section className="parts-v10528-return-group external"><header><div><b>Devoluções avulsas</b><small>Somente lembrete operacional. Não altera aparelho, margem, estoque ou relatório financeiro.</small></div><span>{visibleExternalReturnRows.reduce((sum,row)=>sum+Number(row.item.quantity||1),0)}</span></header><div>{visibleExternalReturnRows.map(row=><article className={`parts-v10528-external-return-card ${row.item.returnStatus}`} key={`${row.order.id}-${row.item.id}`}><span className="parts-v79-return-icon"><Bookmark size={15}/></span><div className="parts-v79-return-identity"><small>{row.order.supplier} · AVULSO</small><b>{row.item.partName}</b><span>{row.item.reference||'Sem referência'} · {row.item.quantity||1} un. × {money(row.item.unitPrice)}</span></div><div className="parts-v79-return-meta"><small>Valor dos itens</small><b>{money(row.item.price)}</b></div><div className="parts-v79-return-meta"><small>Frete rateado</small><b>{money(row.item.freightShare)}</b></div><div className="parts-v79-return-meta parts-v10528-external-total"><small>Frete devolução</small><b>{money(returnShippingNetCost(row.item))}</b><small>{row.item.returnShippingFreightShare?`Pago ${money(row.item.returnShippingFreightShare)}`:'Ainda não enviado'}</small></div><div className="parts-v79-return-actions">{row.item.returnStatus==='pending'?<><span>Separado</span><button onClick={()=>changeExternalReturnState(row.order.id,row.item.id,'cancel')}>Cancelar</button><button className="primary" onClick={()=>openReturnShipment(row.order.supplier,row)}><Package size={12}/> Registrar envio</button></>:row.item.returnStatus==='returned'?<><span className="returned">Devolvido</span><button className="primary" onClick={()=>changeExternalReturnState(row.order.id,row.item.id,'resolved')}>Resolver lembrete</button>{row.item.returnShipmentId?<button className="parts-v10514-undo-button" onClick={()=>undoReturnShipment(row.item.returnShipmentId)}><RotateCcw size={12}/> Desfazer remessa</button>:<button onClick={()=>changeExternalReturnState(row.order.id,row.item.id,'pending')}><RotateCcw size={12}/> Reabrir</button>}</>:<><span className="parts-v10528-resolved-label">Resolvido</span>{row.item.returnShipmentId?<button className="parts-v10514-undo-button" onClick={()=>undoReturnShipment(row.item.returnShipmentId)}><RotateCcw size={12}/> Desfazer remessa</button>:<button onClick={()=>changeExternalReturnState(row.order.id,row.item.id,'pending')}><RotateCcw size={12}/> Reabrir</button>}</>}</div></article>)}</div></section>}
        {!!visibleStandaloneReturnRows.length&&<section className="parts-v10528-return-group external parts-v10529-standalone-group"><header><div><b>Devoluções avulsas independentes</b><small>Cadastradas diretamente, sem depender de pedido anterior. Permanecem fora dos custos dos smartphones.</small></div><span>{visibleStandaloneReturnRows.reduce((sum,row)=>sum+Number(row.item.quantity||1),0)}</span></header><div>{visibleStandaloneReturnRows.map(row=>{const item=row.standalone||row.item,total=toNumber(item.unitPrice)*Math.max(1,Number(item.quantity||1)||1)+toNumber(item.freight);return <article className={`parts-v10528-external-return-card parts-v10529-standalone-card ${item.status||'pending'}`} key={item.id}><span className="parts-v79-return-icon"><Bookmark size={15}/></span><div className="parts-v79-return-identity"><small>{item.supplier} · AVULSA INDEPENDENTE</small><b>{item.partName}</b><span>{item.reference||item.reason||'Sem referência'} · {item.quantity||1} un. × {money(item.unitPrice)}</span></div><div className="parts-v79-return-meta"><small>Valor dos itens</small><b>{money(toNumber(item.unitPrice)*Math.max(1,Number(item.quantity||1)||1))}</b></div><div className="parts-v79-return-meta"><small>Frete da compra</small><b>{money(item.freight)}</b></div><div className="parts-v79-return-meta parts-v10528-external-total"><small>{item.status==='pending'?'Programada':item.status==='returned'?'Devolvida':'Resolvida'}</small><b>{money(total)}</b><small>{formatDate(item.returnDate)}</small></div><div className="parts-v79-return-actions parts-v10529-standalone-actions">{item.status==='pending'?<><span>Para devolver</span><button onClick={()=>openStandaloneReturn(item)}><Pencil size={12}/> Editar</button><button className="primary" onClick={()=>openReturnShipment(item.supplier,row)}><Package size={12}/> Registrar envio</button></>:item.status==='returned'?<><span className="returned">Devolvido</span><button className="primary" onClick={()=>changeStandaloneReturnState(item.id,'resolved')}>Resolver</button></>:<span className="parts-v10528-resolved-label">Resolvido</span>}{item.returnShipmentId?<button className="parts-v10514-undo-button" onClick={()=>undoReturnShipment(item.returnShipmentId)}><RotateCcw size={12}/> Desfazer remessa</button>:<button className="parts-v10514-undo-button" onClick={()=>undoStandaloneReturn(item.id)}><RotateCcw size={12}/> Desfazer</button>}</div></article>})}</div></section>}
        {!visibleReturnRows.length&&!visibleExternalReturnRows.length&&!visibleStandaloneReturnRows.length&&<div className="parts-v47-empty"><RotateCcw size={20}/><div><b>Nenhum item neste filtro</b><span>Use “Devolver” em um pedido recebido ou clique em “Nova devolução avulsa” para cadastrar uma devolução independente.</span></div></div>}
      </div>}
    </section>
  </div>
  ...
  {returnShipmentEditor&&<div className="parts-v47-backdrop" onMouseDown={e=>e.target===e.currentTarget&&setReturnShipmentEditor(null)}>{(()=>{const existing=returnShipmentEditor.id?returnShipments.find(item=>item.id===returnShipmentEditor.id):null;const rows=existing?(existing.items||[]).map(ref=>{const item=currentShipmentItem(ref);if(!item)return null;return{type:ref.type,order:{id:ref.orderId,supplier:returnShipmentEditor.supplier},item,standalone:ref.type==='standalone'?item:null}}).filter(Boolean):pendingReturnRowsForSupplier(returnShipmentEditor.supplier);const selected=new Set(returnShipmentEditor.selectedKeys||[]),selectedRows=existing?rows:rows.filter(row=>selected.has(returnShipmentRowKey(row))),units=selectedRows.reduce((sum,row)=>sum+returnShipmentQty(row),0),freight=toNumber(returnShipmentEditor.freight),refund=toNumber(returnShipmentEditor.freightRefund);return <section className="parts-v10530-shipment-dialog"><header><div><span>REMESSA DE DEVOLUÇÃO</span><h2>{existing?'Editar frete da remessa':'Registrar envio ao fornecedor'}</h2><p>O frete é dividido somente pela quantidade de unidades enviadas. O valor de cada produto não interfere no rateio.</p></div><button className="icon-only" onClick={()=>setReturnShipmentEditor(null)}><X size={15}/></button></header><div className="parts-v10530-shipment-supplier"><div><small>FORNECEDOR</small><b>{returnShipmentEditor.supplier}</b></div><div><small>UNIDADES SELECIONADAS</small><b>{units}</b></div></div><div className="parts-v10530-shipment-items">{rows.map(row=>{const key=returnShipmentRowKey(row),checked=existing||selected.has(key),qty=returnShipmentQty(row),kind=row.type==='linked'?'BMCenter':row.type==='external'?'Avulso do pedido':'Avulso independente';return <label className={checked?'selected':''} key={key}><input type="checkbox" disabled={!!existing} checked={checked} onChange={()=>{const keys=returnShipmentEditor.selectedKeys||[];setReturnShipmentEditor({...returnShipmentEditor,selectedKeys:checked?keys.filter(item=>item!==key):[...keys,key]})}}/><div><b>{row.item.partName}</b><small>{kind} · {qty} un.{row.item.reference?` · ${row.item.reference}`:row.item.phoneLabel?` · ${row.item.phoneLabel}`:''}</small></div><span>{qty} un.</span></label>})}</div><div className="parts-v10530-shipment-fields"><label>Data do envio<input type="date" value={returnShipmentEditor.shipmentDate||''} onChange={e=>setReturnShipmentEditor({...returnShipmentEditor,shipmentDate:e.target.value})}/></label><label>Frete total da devolução<div className="money-prefix"><span>R$</span><input inputMode="decimal" value={returnShipmentEditor.freight??''} onChange={e=>setReturnShipmentEditor({...returnShipmentEditor,freight:e.target.value.replace(/[^0-9,.-]/g,'')})} placeholder="0,00"/></div></label><label>Frete recuperado do fornecedor<div className="money-prefix"><span>R$</span><input inputMode="decimal" value={returnShipmentEditor.freightRefund??''} onChange={e=>setReturnShipmentEditor({...returnShipmentEditor,freightRefund:e.target.value.replace(/[^0-9,.-]/g,'')})} placeholder="0,00"/></div></label><label className="wide">Observações<input value={returnShipmentEditor.notes||''} onChange={e=>setReturnShipmentEditor({...returnShipmentEditor,notes:e.target.value})} placeholder="Código de postagem, transportadora, observações..."/></label></div><div className="parts-v10530-shipment-calc"><div><small>Frete por unidade</small><strong>{money(units?freight/units:0)}</strong></div><div><small>Frete líquido da remessa</small><strong>{money(Math.max(0,freight-refund))}</strong></div><p>Exemplo: frete de R$ 15 com 5 unidades = R$ 3 por unidade, mesmo que os produtos tenham valores diferentes.</p></div><footer><button onClick={()=>setReturnShipmentEditor(null)}>Cancelar</button><button className="primary" onClick={saveReturnShipment}><Package size={13}/> {existing?'Salvar e recalcular':'Registrar envio'}</button></footer></section>})()}</div>}

  {standaloneReturnEditor&&<div className="parts-v47-backdrop" onMouseDown={e=>e.target===e.currentTarget&&setStandaloneReturnEditor(null)}><section className="parts-v10529-return-dialog"><header><div><span>DEVOLUÇÃO AVULSA INDEPENDENTE</span><h2>{standaloneReturnEditor.id?'Editar devolução avulsa':'Nova devolução avulsa'}</h2><p>Use quando a devolução não nasceu de um pedido registrado no BMCenter. Este lançamento não altera custo, margem ou estoque dos smartphones.</p></div><button className="icon-only" onClick={()=>setStandaloneReturnEditor(null)}><X size={15}/></button></header><div className="parts-v10529-return-grid"><label>Fornecedor<input list="parts-v10529-return-suppliers" value={standaloneReturnEditor.supplier||''} onChange={e=>setStandaloneReturnEditor({...standaloneReturnEditor,supplier:e.target.value})} placeholder="Selecione ou digite..."/><datalist id="parts-v10529-return-suppliers">{activeSuppliers.map(s=><option key={s.id||s.name} value={s.name}/>)}</datalist></label><label>Item / peça<input value={standaloneReturnEditor.partName||''} onChange={e=>setStandaloneReturnEditor({...standaloneReturnEditor,partName:e.target.value})} placeholder="Ex.: Tela Galaxy A14"/></label><label>Referência<input value={standaloneReturnEditor.reference||''} onChange={e=>setStandaloneReturnEditor({...standaloneReturnEditor,reference:e.target.value})} placeholder="Pedido externo, cliente, NF..."/></label><label>Quantidade<input inputMode="numeric" value={standaloneReturnEditor.quantity||'1'} onChange={e=>setStandaloneReturnEditor({...standaloneReturnEditor,quantity:e.target.value.replace(/\D/g,'')})}/></label><label>Valor unitário<div className="money-prefix"><span>R$</span><input inputMode="decimal" value={standaloneReturnEditor.unitPrice??''} onChange={e=>setStandaloneReturnEditor({...standaloneReturnEditor,unitPrice:e.target.value.replace(/[^0-9,.-]/g,'')})} placeholder="0,00"/></div></label><label>Frete original da compra<div className="money-prefix"><span>R$</span><input inputMode="decimal" value={standaloneReturnEditor.freight??''} onChange={e=>setStandaloneReturnEditor({...standaloneReturnEditor,freight:e.target.value.replace(/[^0-9,.-]/g,'')})} placeholder="0,00"/></div></label><label>Data da devolução<input type="date" value={standaloneReturnEditor.returnDate||''} onChange={e=>setStandaloneReturnEditor({...standaloneReturnEditor,returnDate:e.target.value})}/></label><label>Motivo<input list="parts-v10529-return-reasons" value={standaloneReturnEditor.reason||''} onChange={e=>setStandaloneReturnEditor({...standaloneReturnEditor,reason:e.target.value})} placeholder="Ex.: Produto com defeito"/><datalist id="parts-v10529-return-reasons"><option value="Produto com defeito"/><option value="Produto incorreto"/><option value="Garantia do fornecedor"/><option value="Cancelamento da compra"/><option value="Outro"/></datalist></label><label className="wide">Observações<textarea value={standaloneReturnEditor.notes||''} onChange={e=>setStandaloneReturnEditor({...standaloneReturnEditor,notes:e.target.value})} placeholder="Detalhes, combinado com o fornecedor, código de postagem..."/></label></div><div className="parts-v10529-return-total"><div><ShieldCheck size={16}/><span>Operação isolada dos aparelhos BMCenter. Após salvar, o cartão terá sempre a ação <b>Desfazer</b>.</span></div><strong>{money(toNumber(standaloneReturnEditor.unitPrice)*Math.max(1,Number(standaloneReturnEditor.quantity||1)||1)+toNumber(standaloneReturnEditor.freight))}</strong></div><footer><button onClick={()=>setStandaloneReturnEditor(null)}>Cancelar</button><button className="primary" onClick={saveStandaloneReturn}><Save size={13}/> {standaloneReturnEditor.id?'Salvar alteração':'Criar devolução avulsa'}</button></footer></section></div>}

  {bulkOrderOpen&&<div className="parts-v47-backdrop" onMouseDown={e=>e.target===e.currentTarget&&closeBulkOrder()}><section ref={bulkDialogRef} className="parts-v50-bulk-dialog parts-v61-bulk-dialog parts-v62-bulk-dialog parts-v10528-bulk-dialog"><header><div><span>{bulkOrderMode==='single'?'NOVO PEDIDO · PEDIDO MISTO':'PEDIDO EM MASSA · PEDIDO MISTO'}</span><h2>{bulkOrderMode==='single'?'Monte um pedido normal com peças BMCenter e itens avulsos':'Peças BMCenter e itens avulsos no mesmo fornecedor'}</h2><p>{bulkOrderMode==='single'?'Crie um único pedido manual. Você pode combinar peças vinculadas aos smartphones e itens avulsos sem misturar custos.':'Itens avulsos ficam somente neste pedido e nas devoluções; não alteram custo, margem, estoque ou dados dos aparelhos.'}</p></div><button className="icon-only" onClick={closeBulkOrder}><X size={15}/></button></header>{bulkOrderMode==='bulk'&&<section className="parts-v61-bulk-batches"><header><div><b>Pedidos deste lançamento</b><small>{bulkStagedOrders.length?`${bulkStagedOrders.length} pedido(s) já preparado(s) · você está editando o próximo`:'Preencha o pedido abaixo. Se houver outro pedido, adicione-o antes de concluir.'}</small></div><button type="button" className="parts-v61-stage-button" onClick={stageCurrentBulkOrder}><Plus size={12}/> Salvar este e adicionar outro pedido</button></header>{!!bulkStagedOrders.length&&<div className="parts-v61-batch-list">{bulkStagedOrders.map((batch,index)=>{const linked=batch.products.filter(product=>product.type!=='external').reduce((sum,product)=>sum+(product.phoneIds||[]).length,0),external=batch.products.filter(product=>product.type==='external').reduce((sum,product)=>sum+Number(product.quantity||1),0),itemCount=linked+external,total=Number(batch.subtotal||0)+toNumber(batch.draft.freight);return <article key={batch.id}><span>{String(index+1).padStart(2,'0')}</span><div><b>{batch.draft.supplier}</b><small>{formatDate(batch.draft.orderDate)} · {itemCount} item(ns){external?` · ${external} avulso(s)`:''} · frete {money(toNumber(batch.draft.freight))}</small></div><strong>{money(total)}</strong><button type="button" onClick={()=>editStagedBulkOrder(batch.id)}>Editar</button><button type="button" className="danger icon-only" title="Remover deste lançamento" onClick={()=>removeStagedBulkOrder(batch.id)}><Trash2 size={12}/></button></article>})}</div>}</section>}<div className="parts-v50-bulk-main"><aside className="parts-v50-bulk-left"><section className="parts-v50-bulk-meta parts-v62-bulk-meta"><label className="parts-v62-meta-supplier">Fornecedor<select value={bulkDraft.supplier} onChange={e=>setBulkDraft({...bulkDraft,supplier:e.target.value})}><option value="">Selecione o fornecedor...</option>{activeSuppliers.map(s=><option key={s.id||s.name} value={s.name}>{s.name}</option>)}</select></label><label className="parts-v62-meta-freight">Frete<div className="money-prefix"><span>R$</span><input inputMode="decimal" value={bulkDraft.freight} onChange={e=>setBulkDraft({...bulkDraft,freight:e.target.value.replace(/[^0-9,.-]/g,'')})} placeholder="0,00"/></div></label><label className="parts-v62-meta-date">Data<input type="date" value={bulkDraft.orderDate} onChange={e=>setBulkDraft({...bulkDraft,orderDate:e.target.value})}/></label><label className="parts-v49-bulk-received parts-v62-meta-received"><input type="checkbox" checked={bulkDraft.receivedNow} onChange={e=>setBulkDraft({...bulkDraft,receivedNow:e.target.checked})}/><span>Já recebido</span></label><label className="parts-v62-meta-notes">Observações<input value={bulkDraft.notes} onChange={e=>setBulkDraft({...bulkDraft,notes:e.target.value})} placeholder="Opcional"/></label></section><section className="parts-v50-products parts-v10528-products"><header><div><b>Itens deste pedido</b><small>{bulkLinkedCount} BMCenter · {bulkExternalCount} avulso(s)</small></div><div className="parts-v10528-add-types"><button type="button" onClick={()=>addBulkProduct({focusName:true,type:'linked'})}><Plus size={12}/> BMCenter</button><button type="button" onClick={()=>addBulkProduct({focusName:true,type:'external'})}><Plus size={12}/> Avulso</button></div></header><div>{bulkProducts.map((rawProduct,index)=>{const product={type:'linked',quantity:'1',reference:'',...rawProduct},active=activeBulkProduct?.id===product.id,isExternal=product.type==='external';return <article className={`${active?'active':''} ${isExternal?'parts-v10528-external-product':''}`} key={product.id}><button type="button" className="parts-v50-product-select" onClick={()=>setBulkActiveProductId(product.id)}><span>{String(index+1).padStart(2,'0')}</span><div><b>{product.name||'Novo produto'}</b><small>{isExternal?`AVULSO · ${Math.max(1,Number(product.quantity||1)||1)} un.${product.reference?` · ${product.reference}`:''}`:`BMCENTER · ${(product.phoneIds||[]).length} aparelho(s)`}</small></div><ChevronRight size={14}/></button><div className={`parts-v50-product-fields ${isExternal?'parts-v10528-external-fields':''}`}><label>Peça<input data-bulk-product-name={product.id} value={product.name} onFocus={()=>setBulkActiveProductId(product.id)} onChange={e=>updateBulkProduct(product.id,{name:e.target.value})} placeholder={isExternal?'Ex.: Tela A14':'Ex.: Película'}/></label><label>Valor unitário<div className="money-prefix"><span>R$</span><input inputMode="decimal" value={product.unitPrice} onFocus={()=>setBulkActiveProductId(product.id)} onChange={e=>updateBulkProduct(product.id,{unitPrice:e.target.value.replace(/[^0-9,.-]/g,'')})} onKeyDown={e=>handleBulkUnitPriceTab(e,product,index)} placeholder="0,00"/></div></label>{isExternal&&<><label>Quantidade<input inputMode="numeric" value={product.quantity} onFocus={()=>setBulkActiveProductId(product.id)} onChange={e=>updateBulkProduct(product.id,{quantity:e.target.value.replace(/\D/g,'')})} placeholder="1"/></label><label className="parts-v10528-reference">Referência<input value={product.reference} onFocus={()=>setBulkActiveProductId(product.id)} onChange={e=>updateBulkProduct(product.id,{reference:e.target.value})} placeholder="Cliente João, uso pessoal..."/></label></>}</div><div className="parts-v50-product-actions">{isExternal?<button type="button" className={active?'primary':''} onClick={()=>setBulkActiveProductId(product.id)}><Bookmark size={12}/> Item avulso</button>:<button type="button" className={active?'primary':''} onClick={()=>setBulkActiveProductId(product.id)}><Smartphone size={12}/> Escolher aparelhos</button>}{bulkProducts.length>1&&<button type="button" className="danger icon-only" title="Remover produto" onClick={()=>removeBulkProduct(product.id)}><Trash2 size={12}/></button>}</div></article>})}</div></section></aside>{activeBulkProduct?.type==='external'?<section className="parts-v10528-external-editor"><div className="parts-v10528-external-hero"><span><Bookmark size={24}/></span><div><small>ITEM AVULSO</small><h3>{activeBulkProduct.name||'Informe a descrição da peça'}</h3><p>Este item pertence ao mesmo pedido do fornecedor, mas fica fora dos aparelhos, custos e relatórios operacionais do BMCenter.</p></div></div><div className="parts-v10528-external-summary"><div><small>Referência</small><b>{activeBulkProduct.reference||'Sem referência'}</b></div><div><small>Quantidade</small><b>{Math.max(1,Number(activeBulkProduct.quantity||1)||1)} un.</b></div><div><small>Valor unitário</small><b>{money(toNumber(activeBulkProduct.unitPrice))}</b></div><div><small>Total do item</small><strong>{money(toNumber(activeBulkProduct.unitPrice)*Math.max(1,Number(activeBulkProduct.quantity||1)||1))}</strong></div></div><div className="parts-v10528-external-note"><ShieldCheck size={16}/><span>Se precisar devolver depois, marque <b>Devolver</b> neste pedido recebido. O item aparecerá em Devoluções → Avulsas.</span></div></section>:<section className="parts-v50-bulk-select"><header><div><b>{activeBulkProduct?`Aparelhos para ${activeBulkProduct.name||'este produto'}`:'Selecione um produto'}</b><small>{activeBulkProduct?(activeBulkProduct.phoneIds||[]).length:0} selecionado(s) · {bulkVisiblePhones.length} nesta filtragem</small></div><div><button disabled={!activeBulkProduct} onClick={selectBulkFiltered}>Selecionar filtrados</button><button disabled={!activeBulkProduct} onClick={unselectBulkFiltered}>Desmarcar filtrados</button></div></header><div className="parts-v49-bulk-filters"><label><Search size={13}/><input value={bulkSearch} onChange={e=>setBulkSearch(e.target.value)} placeholder="Buscar modelo, código ou cor..."/></label><select value={bulkStatus} onChange={e=>setBulkStatus(e.target.value)}><option value="Todos">Todos os status</option>{statuses.filter(status=>!['Vendido','Descarte/Sucata'].includes(status)).map(status=><option key={status}>{status}</option>)}</select></div><div className="parts-v49-bulk-list">{activeBulkProduct&&bulkVisiblePhones.map(phone=>{const checked=(activeBulkProduct.phoneIds||[]).includes(phone.id),phoneProductCount=bulkProducts.filter(product=>product.type!=='external'&&(product.phoneIds||[]).includes(phone.id)).length,phoneTotal=bulkPhoneProductsTotal(bulkProducts.filter(product=>product.type!=='external'),phone.id);return <article className={checked?'selected':''} key={phone.id}><div className="parts-v64-bulk-card-line"><label className="parts-v49-bulk-check"><input type="checkbox" checked={checked} onChange={()=>toggleBulkPhone(phone.id)}/><div><b>{phoneDisplayName(phone,{includeCode:false})}</b><small>{phone.code} · {phone.status}{phoneProductCount>1?` · ${phoneProductCount} itens`:''}</small></div></label>{checked&&<label className="parts-v49-bulk-row-price parts-v65-bulk-total-price" title={`Total dos ${phoneProductCount||1} item(ns) deste pedido neste aparelho. Ao editar, o ajuste é aplicado ao produto selecionado.`}><span>R$</span><input inputMode="decimal" value={bulkPhoneTotalDisplay(phone.id)} onFocus={e=>{setBulkActiveProductId(activeBulkProduct.id);e.currentTarget.select()}} onChange={e=>updateBulkPhoneTotalDraft(phone.id,e.target.value.replace(/[^0-9,.-]/g,''))} onBlur={()=>commitBulkPhoneTotal(phone.id)} onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();commitBulkPhoneTotal(phone.id);e.currentTarget.blur()}}} placeholder="0,00"/></label>}</div></article>})}{!activeBulkProduct&&<div className="parts-v47-empty"><Package size={18}/><div><b>Selecione um produto</b><span>Escolha um produto à esquerda para vincular os aparelhos.</span></div></div>}{activeBulkProduct&&!bulkVisiblePhones.length&&<div className="parts-v47-empty"><Search size={18}/><div><b>Nenhum aparelho neste filtro</b><span>Altere a busca ou o status.</span></div></div>}</div></section>}</div><footer className="parts-v61-bulk-footer parts-v10528-bulk-footer"><div><span>{bulkOrderMode==='single'?`Pedido atual: ${bulkLinkedCount} BMCenter · ${bulkExternalCount} avulso(s)`:bulkStagedOrders.length?`${bulkStagedOrders.length} pedido(s) preparado(s) · ${stagedBulkLineCount} item(ns) já separados`:`Pedido atual: ${bulkLinkedCount} BMCenter · ${bulkExternalCount} avulso(s)`}</span><b>{bulkOrderMode==='single'?`BMCenter ${money(bulkLinkedSubtotal)} · Avulsos ${money(bulkExternalSubtotal)} · Frete ${money(toNumber(bulkDraft.freight))} · Total ${money(bulkSubtotal+toNumber(bulkDraft.freight))}`:bulkStagedOrders.length?`Preparados ${money(stagedBulkTotal)} · atual ${money(bulkSubtotal+toNumber(bulkDraft.freight))}`:`BMCenter ${money(bulkLinkedSubtotal)} · Avulsos ${money(bulkExternalSubtotal)} · Frete ${money(toNumber(bulkDraft.freight))} · Total ${money(bulkSubtotal+toNumber(bulkDraft.freight))}`}</b></div><div><button onClick={closeBulkOrder}>Cancelar</button><button className="primary" onClick={saveBulkOrder}><CheckSquare size={13}/> {bulkOrderMode==='single'?'Criar pedido':bulkStagedOrders.length?`Concluir ${bulkStagedOrders.length+(bulkCurrentHasSelection?1:0)} pedido(s)`:'Concluir pedido em massa'}</button></div></footer></section></div>}

  {directBuy&&<div className="parts-v47-backdrop" onMouseDown={e=>e.target===e.currentTarget&&setDirectBuy(null)}><section className="parts-v47-direct-dialog"><header><div><span>{directBuy.mode==='quote'?(directBuy.editingQuoteId?'EDITAR COTAÇÃO':'COTAÇÃO'):'COMPRA DIRETA DE PEÇA'}</span><h2>{directBuy.part.name}</h2><p>{phoneDisplayName(directBuy.phone,{includeCode:false})} · {directBuy.phone.code}</p></div><button className="icon-only" onClick={()=>setDirectBuy(null)}><X size={15}/></button></header><div className="parts-v47-direct-fields"><label>Fornecedor<input list="parts-v48-suppliers" value={directDraft.supplier} onChange={e=>setDirectDraft({...directDraft,supplier:e.target.value})} placeholder="Fornecedor"/><datalist id="parts-v48-suppliers">{activeSuppliers.map(s=><option key={s.id||s.name} value={s.name}/>)}</datalist></label><label>Valor da peça<div className="money-prefix"><span>R$</span><input autoFocus inputMode="decimal" value={directDraft.price} onChange={e=>setDirectDraft({...directDraft,price:e.target.value.replace(/[^0-9,.-]/g,'')})}/></div></label>{directBuy.mode==='buy'&&<><label>Frete deste pedido<div className="money-prefix"><span>R$</span><input inputMode="decimal" value={directDraft.freight} onChange={e=>setDirectDraft({...directDraft,freight:e.target.value.replace(/[^0-9,.-]/g,'')})}/></div></label><label>Data da compra<input type="date" value={directDraft.orderDate} onChange={e=>setDirectDraft({...directDraft,orderDate:e.target.value})}/></label></>}<label className="wide">Observação<input value={directDraft.notes} onChange={e=>setDirectDraft({...directDraft,notes:e.target.value})}/></label>{directBuy.mode==='buy'&&<label className="parts-v47-received-check"><input type="checkbox" checked={directDraft.receivedNow} onChange={e=>setDirectDraft({...directDraft,receivedNow:e.target.checked})}/><span>Já estou com a peça em mãos</span></label>}</div><footer><button onClick={()=>setDirectBuy(null)}>Cancelar</button><button className="primary" onClick={saveDirectPurchase}><CheckSquare size={13}/> {directBuy.mode==='quote'?(directBuy.editingQuoteId?'Salvar alterações':'Salvar cotação'):directDraft.receivedNow?'Salvar recebida':'Registrar compra'}</button></footer></section></div>}
  ...
  {refundEditor&&<div className="parts-v47-backdrop" onMouseDown={e=>e.target===e.currentTarget&&setRefundEditor(null)}><section className="parts-v84-refund-dialog"><header><div><span>DEVOLUÇÃO FINANCEIRA</span><h2>{refundEditor.partName}</h2><p>{refundEditor.phoneLabel} · {refundEditor.supplier}</p></div><button className="icon-only" onClick={()=>setRefundEditor(null)}><X size={15}/></button></header><div className="parts-v84-refund-original"><div><small>Peça original</small><b>{money(refundEditor.originalPart)}</b></div><div><small>Frete da compra</small><b>{money(refundEditor.originalFreight)}</b></div><div><small>Custo original</small><b>{money(Number(refundEditor.originalPart||0)+Number(refundEditor.originalFreight||0))}</b></div></div><div className="parts-v84-refund-fields"><label>Situação financeira<select value={refundEditor.financialStatus} onChange={e=>setRefundEditor({...refundEditor,financialStatus:e.target.value})}><option value="received">Reembolso recebido</option><option value="supplier_credit">Crédito confirmado no fornecedor</option><option value="pending">Reembolso/crédito pendente</option></select></label><label>Valor da peça recuperado<div className="money-prefix"><span>R$</span><input inputMode="decimal" value={refundEditor.partRefund} onChange={e=>setRefundEditor({...refundEditor,partRefund:e.target.value.replace(/[^0-9,.-]/g,'')})}/></div></label><label>Frete da compra recuperado<div className="money-prefix"><span>R$</span><input inputMode="decimal" value={refundEditor.freightRefund} onChange={e=>setRefundEditor({...refundEditor,freightRefund:e.target.value.replace(/[^0-9,.-]/g,'')})}/></div></label><label>Forma<select value={refundEditor.financialStatus==='supplier_credit'?'Crédito no fornecedor':refundEditor.method} disabled={refundEditor.financialStatus==='supplier_credit'} onChange={e=>setRefundEditor({...refundEditor,method:e.target.value})}><option value="">Selecione</option><option>Pix</option><option>Estorno no cartão</option><option>Dinheiro</option><option>Transferência</option><option>Crédito no fornecedor</option><option>Outro</option></select></label><label>Data do registro<input type="date" value={refundEditor.date} onChange={e=>setRefundEditor({...refundEditor,date:e.target.value})}/></label></div>{refundEditor.returnShipmentId&&<div className="parts-v10530-refund-shipping-note"><Package size={14}/><span>Frete para devolver: <b>{money(refundEditor.returnShippingFreightShare)}</b> · recuperado na remessa: <b>{money(refundEditor.returnShippingFreightRefundShare)}</b>. Esse frete é controlado pela remessa, separado do frete original da compra.</span></div>}{(()=>{const promised=Math.min(Number(refundEditor.originalPart||0)+Number(refundEditor.originalFreight||0),Math.max(0,toNumber(refundEditor.partRefund))+Math.max(0,toNumber(refundEditor.freightRefund))),recovered=refundEditor.financialStatus==='pending'?0:promised,residual=Math.max(0,Number(refundEditor.originalPart||0)+Number(refundEditor.originalFreight||0)-recovered);return <div className={`parts-v84-refund-impact ${refundEditor.financialStatus==='pending'?'pending':''}`}><div><small>{refundEditor.financialStatus==='pending'?'Valor aguardando fornecedor':'Redução imediata no custo do aparelho'}</small><strong>{money(refundEditor.financialStatus==='pending'?promised:recovered)}</strong></div><div><small>Custo que permanecerá no aparelho</small><strong>{money(residual)}</strong></div><p>{refundEditor.financialStatus==='pending'?'Enquanto estiver pendente, nenhum valor é retirado do custo do aparelho. Ao confirmar o recebimento ou crédito, o custo será recalculado automaticamente.':'O pedido original continua com o valor histórico integral. Somente o custo líquido do aparelho é reduzido pelo valor efetivamente recuperado.'}</p></div>})()}<footer><button onClick={()=>setRefundEditor(null)}>Cancelar</button><button className="primary" onClick={saveReturnSettlement}><CheckSquare size={13}/> Confirmar devolução</button></footer></section></div>}

  {orderEditor&&<div className="parts-v47-backdrop" onMouseDown={e=>e.target===e.currentTarget&&setOrderEditor(null)}><section className="parts-v48-order-editor parts-v10528-order-editor"><header><div><span>EDITAR PEDIDO</span><h2>{orderEditor.supplier}</h2><p>Itens avulsos continuam isolados dos custos dos aparelhos. O frete é recalculado pela quantidade total de unidades do pedido, incluindo os itens avulsos.</p></div><button className="icon-only" onClick={()=>setOrderEditor(null)}><X size={15}/></button></header><div className="parts-v48-edit-grid"><label>Fornecedor<input list="parts-v48-edit-suppliers" value={orderEditor.supplier} onChange={e=>setOrderEditor({...orderEditor,supplier:e.target.value})}/><datalist id="parts-v48-edit-suppliers">{activeSuppliers.map(s=><option key={s.id||s.name} value={s.name}/>)}</datalist></label><label>Data do pedido<input type="date" value={orderEditor.orderDate||''} onChange={e=>setOrderEditor({...orderEditor,orderDate:e.target.value})}/></label><label>Previsão de chegada<input type="date" value={orderEditor.expectedDate||''} onChange={e=>setOrderEditor({...orderEditor,expectedDate:e.target.value})}/></label><label>Frete do pedido<div className="money-prefix"><span>R$</span><input inputMode="decimal" value={orderEditor.freight??''} onChange={e=>setOrderEditor({...orderEditor,freight:e.target.value.replace(/[^0-9,.-]/g,'')})}/></div></label><label className="wide">Observações<textarea value={orderEditor.notes||''} onChange={e=>setOrderEditor({...orderEditor,notes:e.target.value})}/></label></div>{!!orderEditor.items?.length&&<section className="parts-v10528-edit-group"><header><b>Peças BMCenter</b><small>Vinculadas aos aparelhos cadastrados.</small></header><div className="parts-v48-edit-items">{orderEditor.items.map((item,index)=><label key={item.id}><div><b>{item.partName}</b><small>{item.phoneLabel}</small></div><div className="money-prefix"><span>R$</span><input inputMode="decimal" value={item.price} onChange={e=>setOrderEditor({...orderEditor,items:orderEditor.items.map((x,i)=>i===index?{...x,price:e.target.value.replace(/[^0-9,.-]/g,'')}:x)})}/></div><span>{item.returnStatus==='pending'?'Para devolver':item.returnStatus==='returned'?'Devolvida':item.receivedAt?'Recebida':item.confirmedAt?'Confirmada':'Rascunho'}</span></label>)}</div></section>}{!!orderEditor.externalItems?.length&&<section className="parts-v10528-edit-group external"><header><b>Itens avulsos</b><small>Não entram em aparelho, margem ou estoque.</small></header><div className="parts-v10528-edit-external">{orderEditor.externalItems.map((item,index)=><article key={item.id}><label>Peça<input value={item.partName} onChange={e=>setOrderEditor({...orderEditor,externalItems:orderEditor.externalItems.map((x,i)=>i===index?{...x,partName:e.target.value}:x)})}/></label><label>Referência<input value={item.reference||''} onChange={e=>setOrderEditor({...orderEditor,externalItems:orderEditor.externalItems.map((x,i)=>i===index?{...x,reference:e.target.value}:x)})} placeholder="Cliente, uso pessoal..."/></label><label>Qtd.<input inputMode="numeric" value={item.quantity||1} onChange={e=>setOrderEditor({...orderEditor,externalItems:orderEditor.externalItems.map((x,i)=>i===index?{...x,quantity:e.target.value.replace(/\D/g,'')}:x)})}/></label><label>Valor unitário<div className="money-prefix"><span>R$</span><input inputMode="decimal" value={item.unitPrice} onChange={e=>setOrderEditor({...orderEditor,externalItems:orderEditor.externalItems.map((x,i)=>i===index?{...x,unitPrice:e.target.value.replace(/[^0-9,.-]/g,'')}:x)})}/></div></label><span>{item.returnStatus==='pending'?'Separado para devolver':item.returnStatus==='returned'?'Devolvido':item.returnStatus==='resolved'?'Resolvido':item.receivedAt?'Recebido':item.confirmedAt?'Confirmado':'Rascunho'}</span></article>)}</div></section>}<div className="parts-v48-edit-summary"><span>O total pago ao fornecedor inclui BMCenter + avulsos + frete. Somente a parcela BMCenter entra nos custos dos aparelhos.</span><b>{money(normalizePartsOrder({...orderEditor,freight:toNumber(orderEditor.freight),items:orderEditor.items.map(item=>({...item,price:toNumber(item.price)})),externalItems:(orderEditor.externalItems||[]).map(item=>({...item,quantity:Math.max(1,Number(item.quantity||1)||1),unitPrice:toNumber(item.unitPrice)}))}).total)}</b></div><footer><button onClick={()=>setOrderEditor(null)}>Cancelar</button><button className="primary" onClick={saveOrderEdit}><Save size={13}/> Salvar e recalcular</button></footer></section></div>}
  </>
}
...
function Ads(){
 ...
 function setPublished(phoneId,profileId,active){
  ...
  const next=phones.map(phone=>{
   ...
   const profile=profiles.find(item=>item.id===profileId),base={...phone,marketplaceProfiles,lastActivityAt:stamp},synced=syncPhonePublicationStatus(base,marketplaceProfiles,{returnToReadyWhenEmpty:!active});
   return touchPhone(addTimeline(synced,`${active?'Publicado':'Removido'} no perfil ${profile?.name||'selecionado'}${synced.status!==phone.status?` · status ${synced.status}`:''}`));
  });persist(next)
 }
 function setPublishedDate(phoneId,profileId,publishedAt){
  if(!publishedAt)return;const stamp=new Date().toISOString();
  const next=phones.map(phone=>{if(phone.id!==phoneId)return phone;const map=normalizeMarketplaceProfiles(phone),current=map[profileId]||{};const marketplaceProfiles={...map,[profileId]:{...current,active:true,publishedAt,updatedAt:stamp}};const profile=profiles.find(item=>item.id===profileId),base={...phone,marketplaceProfiles,lastActivityAt:stamp},synced=syncPhonePublicationStatus(base,marketplaceProfiles);return touchPhone(addTimeline(synced,`Data de publicação ajustada para ${formatDate(publishedAt)} no perfil ${profile?.name||'selecionado'}${synced.status!==phone.status?` · status ${synced.status}`:''}`))});persist(next)
 }
...
function SaleModal({item,profiles,onClose,onSave}){
 ...
 const setSaleValue=v=>{const value=Math.max(0,Number(v)||0);setF(current=>({...current,value,receivedAmount:value}))};
 ...
 return <Modal title={`${item.sale?.soldAt?'Alterar Venda':'Registrar venda'} · ${showProductCode()?item.code:item.brand+' '+item.model}`} onClose={onClose} className="sale-register-modal">
  ...
  <div className="sale-data-grid">
   <Field className="sale-field-value" label="Valor vendido" type="number" value={f.value} onChange={setSaleValue}/>
   ...
   <label className="sale-field-delivery">Entrega<select value={f.deliveryType||''} onChange={e=>set('deliveryType',e.target.value)}><option>Retirada</option><option>Entrega local</option><option>Outro</option></select></label>
...
function repairSoldPublicationStates(phones,profiles=[]){return(Array.isArray(phones)?phones:[]).map(phone=>{const repaired=(soldPublicationStateNeedsRepair(phone)||soldSaleValueNeedsRepair(phone))?finalizeSoldPhonePublications(phone,profiles,phone.sale||{}):phone;return syncPhonePublicationStatus(repaired,normalizeMarketplaceProfiles(repaired))})}
...
function QuickReplyModal({phone,onClose}){
 const[settings,setSettings]=useState(()=>normalizeSalesReplySettings(load(SALESREPLYKEY)));
 const[copied,setCopied]=useState('');
 const[mode,setMode]=useState('phone');
 const[inventory]=useState(()=>load(SKEY));
 const[listBrand,setListBrand]=useState('Todas');
 const[listStorage,setListStorage]=useState('Todos');
 const[listMin,setListMin]=useState('');
 const[listMax,setListMax]=useState('');
 const[listStatuses,setListStatuses]=useState([]);
 const[listSelected,setListSelected]=useState([]);
 const[listFields,setListFields]=useState({storage:true,color:true,battery:false,condition:false});
 const replies=buildSalesQuickReplies(phone,settings);
 const persistSettings=next=>{const normalized=normalizeSalesReplySettings(next);setSettings(normalized);save(SALESREPLYKEY,normalized)};
 const availableInventory=(Array.isArray(inventory)?inventory:[]).filter(item=>!isClosedPhone(item)&&item.status!=='Reservado');
 const saleReadyStatuses=[...new Set(availableInventory.map(item=>String(item.status||'').trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'pt-BR',{sensitivity:'base'}));
 const listBrands=[...new Set(availableInventory.map(item=>String(item.brand||'').trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'pt-BR',{sensitivity:'base'}));
 const listStorages=[...new Set(availableInventory.map(item=>String(item.storage||'').trim()).filter(Boolean))].sort((a,b)=>(parseFloat(a)||0)-(parseFloat(b)||0));
 const filteredInventory=availableInventory.filter(item=>{
  const price=Number(phoneSaleDisplayValue(item)||0),min=Number(String(listMin||'').replace(',','.'))||0,max=Number(String(listMax||'').replace(',','.'))||0;
  return (listBrand==='Todas'||item.brand===listBrand)&&(listStorage==='Todos'||String(item.storage||'')===listStorage)&&(!listStatuses.length||listStatuses.includes(item.status))&&(!min||price>=min)&&(!max||price<=max)
 });
 const selectedPhones=availableInventory.filter(item=>listSelected.includes(item.id));
 const hiddenSelectedCount=Math.max(0,selectedPhones.length-filteredInventory.filter(item=>listSelected.includes(item.id)).length);
 const listText=buildAvailablePhonesReply(selectedPhones,{location:settings.location,showStorage:listFields.storage,showColor:listFields.color,showBattery:listFields.battery,showCondition:listFields.condition});
 function toggleListStatus(status){setListStatuses(current=>current.includes(status)?current.filter(item=>item!==status):[...current,status])}
 function toggleSelected(id){setListSelected(current=>current.includes(id)?current.filter(item=>item!==id):[...current,id])}
 function selectFiltered(){setListSelected(current=>[...new Set([...current,...filteredInventory.map(item=>item.id)])])}
 function clearFiltered(){const ids=new Set(filteredInventory.map(item=>item.id));setListSelected(current=>current.filter(id=>!ids.has(id)))}
 async function copyReply(reply){
  try{if(!navigator.clipboard?.writeText)throw new Error('clipboard indisponível');await navigator.clipboard.writeText(reply.text);setCopied(reply.id);setTimeout(()=>setCopied(current=>current===reply.id?'':current),1600)}
  catch{prompt('Copie a resposta abaixo:',reply.text)}
 }
 async function copyAvailableList(){
  if(!selectedPhones.length)return alert('Selecione pelo menos um aparelho para montar a relação.');
  try{if(!navigator.clipboard?.writeText)throw new Error('clipboard indisponível');await navigator.clipboard.writeText(listText);setCopied('available-list');setTimeout(()=>setCopied(current=>current==='available-list'?'':current),1600)}
  catch{prompt('Copie a relação abaixo:',listText)}
 }
 return <Modal className="v10527-quick-reply-modal v10538-quick-reply-modal" title="Resposta rápida" subtitle={phoneDisplayName(phone,{includeCode:false})} titleIcon={<MessageSquare size={18}/>} onClose={onClose}>
  <nav className="v10538-reply-tabs"><button type="button" className={mode==='phone'?'active':''} onClick={()=>setMode('phone')}><MessageSquare size={13}/> Respostas do aparelho</button><button type="button" className={mode==='list'?'active':''} onClick={()=>setMode('list')}><ListTodo size={13}/> Lista de disponíveis</button></nav>
  <div className="v10527-reply-settings"><label><span>Local padrão</span><input value={settings.location} onChange={e=>setSettings(current=>({...current,location:e.target.value}))} onBlur={()=>persistSettings(settings)} placeholder="Ex.: Maringá"/></label><small>Salvo automaticamente e usado nas respostas de disponibilidade, valor e na relação de aparelhos.</small></div>
  {mode==='phone'?<div className="v10527-reply-list">{replies.map(reply=><article key={reply.id}><header><div><b>{reply.label}</b><small>{reply.hint}</small></div><button type="button" className={copied===reply.id?'success-button':''} onClick={()=>copyReply(reply)}><Copy size={14}/>{copied===reply.id?' Copiado':' Copiar'}</button></header><p>{reply.text}</p></article>)}</div>:<div className="v10538-list-builder">
   <section className="v10538-list-filters"><header><div><b>Quais aparelhos entram na lista?</b><small>Filtre primeiro e depois marque exatamente os aparelhos que deseja enviar ao cliente.</small></div><span>{selectedPhones.length} selecionado(s)</span></header><div className="v10538-filter-grid"><label>Marca<select value={listBrand} onChange={e=>setListBrand(e.target.value)}><option>Todas</option>{listBrands.map(value=><option key={value}>{value}</option>)}</select></label><label>Armazenamento<select value={listStorage} onChange={e=>setListStorage(e.target.value)}><option>Todos</option>{listStorages.map(value=><option key={value}>{value}</option>)}</select></label><label>Preço mínimo<input inputMode="decimal" value={listMin} onChange={e=>setListMin(e.target.value.replace(/[^0-9,.-]/g,''))} placeholder="0,00"/></label><label>Preço máximo<input inputMode="decimal" value={listMax} onChange={e=>setListMax(e.target.value.replace(/[^0-9,.-]/g,''))} placeholder="Sem limite"/></label></div><div className="v10538-statuses"><span>Status incluídos</span><button type="button" className={!listStatuses.length?'active':''} onClick={()=>setListStatuses([])}>Todos</button>{saleReadyStatuses.map(status=><button type="button" key={status} className={listStatuses.includes(status)?'active':''} onClick={()=>toggleListStatus(status)}>{status}</button>)}</div></section>
   <section className="v10538-list-selection"><header><div><b>{filteredInventory.length} aparelho(s) neste filtro</b><small>Reservados, vendidos e sucata ficam fora automaticamente.{hiddenSelectedCount?` ${hiddenSelectedCount} selecionado(s) está(ão) fora do filtro atual.`:''}</small></div><div className="v10538-selection-actions"><button type="button" onClick={selectFiltered}>Selecionar filtrados</button><button type="button" onClick={clearFiltered}>Desmarcar filtrados</button></div></header><div className="v10538-phone-list">{filteredInventory.map(item=>{const checked=listSelected.includes(item.id);return <label key={item.id} className={checked?'selected':''}><input type="checkbox" checked={checked} onChange={()=>toggleSelected(item.id)}/><div><b>{item.brand} {item.model}</b><small>{[item.storage,item.color,item.status].filter(Boolean).join(' · ')}</small></div><strong>{money(phoneSaleDisplayValue(item))}</strong></label>})}{!filteredInventory.length&&<div className="v10538-empty">Nenhum aparelho encontrado com estes filtros.</div>}</div></section>
   <section className="v10538-list-options"><div><b>Informações na relação</b><label><input type="checkbox" checked={listFields.storage} onChange={e=>setListFields(current=>({...current,storage:e.target.checked}))}/> Armazenamento</label><label><input type="checkbox" checked={listFields.color} onChange={e=>setListFields(current=>({...current,color:e.target.checked}))}/> Cor</label><label><input type="checkbox" checked={listFields.battery} onChange={e=>setListFields(current=>({...current,battery:e.target.checked}))}/> Bateria</label><label><input type="checkbox" checked={listFields.condition} onChange={e=>setListFields(current=>({...current,condition:e.target.checked}))}/> Condição</label></div><article><header><div><b>Prévia da mensagem</b><small>{selectedPhones.length?`${selectedPhones.length} aparelho(s) serão enviados`:'Selecione os aparelhos ao lado'}</small></div><button type="button" className={copied==='available-list'?'success-button':'primary'} onClick={copyAvailableList} disabled={!selectedPhones.length}><Copy size={14}/>{copied==='available-list'?' Copiado':' Copiar lista'}</button></header><pre>{selectedPhones.length?listText:'A prévia da relação aparecerá aqui.'}</pre></article></section>
  </div>}
 </Modal>
}
...
function PhoneDetailModal({item,profiles,orders=[],onClose,onSave}){
 ...
 const[tab,setTab]=useState('summary'),[tag,setTag]=useState(''),[checkText,setCheckText]=useState(''),[commentText,setCommentText]=useState(''),[quickReplyOpen,setQuickReplyOpen]=useState(false);
 const set=(k,v)=>setF({...f,[k]:v});
 ...
 return <><Modal className="phone-detail-modal" title={`${showProductCode()?f.code+" · ":""}${f.brand} ${f.model}`} onClose={onClose}>
  <div className="phone-detail-hero">
   ...
   <div className="phone-detail-value"><span>Valor de venda</span><strong>{money(phoneSaleDisplayValue(f))}</strong><small>Custo estimado {money(phoneTotalCost(f))}</small><button type="button" className="v10527-reply-trigger" onClick={()=>setQuickReplyOpen(true)}><MessageSquare size={14}/> Resposta rápida</button></div>
  </div>
 ...
 </Modal>{quickReplyOpen&&<QuickReplyModal phone={f} onClose={()=>setQuickReplyOpen(false)}/>}</>
}
...
function BatchPhoneModal({existing,banks,onClose,onSave}){
 ...
 return <Modal className="batch-phone-modal" title="Cadastro em massa de aparelhos" onClose={onClose}>
  ...
  <section className="batch-shared-section">
   <header className="batch-shared-header">
    <div className="batch-shared-title"><h3>Dados compartilhados da compra</h3><p>Estas informações serão aplicadas a todos os aparelhos deste lote.</p></div>
    <span>{rows.filter(r=>r.brand||r.model).length} preenchido(s)</span>
   </header>
   <div className="grid">
   ...
   <div className={`batch-total-purchase ${shared.splitTotal?'active':''}`}>
    <Field className="batch-total-field" label="Valor total do lote" value={normalizeMoneyInput(shared.totalPurchase)} prefix="R$" inputMode="decimal" onChange={v=>setSharedField('totalPurchase',normalizeMoneyInput(v))}/>
    <label className="batch-total-inline-toggle"><input type="checkbox" checked={!!shared.splitTotal} onChange={e=>setSharedField('splitTotal',e.target.checked)}/><span>Dividir valor total</span></label>
    <div className="batch-split-result"><small>{rows.length} aparelho(s)</small><strong>{shared.splitTotal&&rows.length&&String(shared.totalPurchase??'').trim()?`${money(splitUnitValue)} cada`:'Divisão desativada'}</strong></div>
   </div>
...
function GlobalInlineSelectLayer(){
 ...
 const recentlyClosedRef=useRef({target:null,at:0});
 const closeMenu=targetOverride=>setMenu(current=>{const target=current?.target||targetOverride||null;try{target?.classList?.remove('global-inline-select-open')}catch{}recentlyClosedRef.current={target,at:Date.now()};return null});
 useEffect(()=>{
  const open=event=>{
   ...
   if(currentMenu?.target===target){event.preventDefault();event.stopPropagation();closeMenu(target);return}
   const lastClosed=recentlyClosedRef.current;
   if(lastClosed?.target===target&&Date.now()-Number(lastClosed.at||0)<350){event.preventDefault();event.stopPropagation();return}
   event.preventDefault();event.stopPropagation();
   try{menuRef.current?.target?.classList?.remove('global-inline-select-open')}catch{}
  ...
  const viewportClose=()=>closeMenu();
  document.addEventListener('pointerdown',open,true);document.addEventListener('click',block,true);document.addEventListener('keydown',escape,true);
  window.addEventListener('resize',viewportClose);window.addEventListener('scroll',viewportClose,true);window.visualViewport?.addEventListener?.('resize',viewportClose);window.visualViewport?.addEventListener?.('scroll',viewportClose);
  return()=>{document.removeEventListener('pointerdown',open,true);document.removeEventListener('click',block,true);document.removeEventListener('keydown',escape,true);window.removeEventListener('resize',viewportClose);window.removeEventListener('scroll',viewportClose,true);window.visualViewport?.removeEventListener?.('resize',viewportClose);window.visualViewport?.removeEventListener?.('scroll',viewportClose);try{menuRef.current?.target?.classList?.remove('global-inline-select-open')}catch{}};
 },[]);
 ...
 return <div className="global-inline-select-layer" onPointerDown={e=>{if(e.target!==e.currentTarget)return;e.stopPropagation();closeMenu();}}><div className={`global-inline-select-menu ${menu.dark?'theme-dark-menu':'theme-light-menu'} ${mobileStatusGrid?'status-grid-menu':''}`} role="listbox" style={{left,top,width,maxHeight:menuHeight}} onPointerDown={e=>e.stopPropagation()}>{menu.options.map(option=><button type="button" role="option" aria-selected={option.value===menu.value} key={`${option.index}-${option.value}`} className={option.value===menu.value?'selected':''} disabled={option.disabled} onClick={e=>{e.preventDefault();e.stopPropagation();choose(option)}}><span>{option.label}</span>{option.value===menu.value&&<Check size={15}/>}</button>)}</div></div>
}
...
function Field({label,value,onChange,type='text',prefix='',suffix='',inputMode,className=''}){const focus=e=>{if(type==='number'&&Number(value||0)===0)e.currentTarget.value=''};const blur=e=>{if(type==='number'&&e.currentTarget.value==='')onChange('0')};return <label className={className}>{label}<div className={`field-affix field-affix-inline ${prefix?'has-prefix':''} ${suffix?'has-suffix':''}`}>{prefix&&<span className="field-prefix">{prefix}</span>}<input type={type} inputMode={inputMode} value={value??''} onFocus={focus} onBlur={blur} onChange={e=>onChange(e.target.value)}/>{suffix&&<span className="field-suffix">{suffix}</span>}</div></label>}
