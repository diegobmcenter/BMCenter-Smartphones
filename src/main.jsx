...
function Parts(){
  ...
  const[bulkStagedOrders,setBulkStagedOrders]=useState([]);
  ...
  function resetCurrentBulkOrder(){
    ...
    setBulkDraft(emptyBulkDraft());setBulkProducts([first]);setBulkActiveProductId(first.id);setBulkSearch('');setBulkStatus('Todos')
  }
  ...
  function saveBulkOrder(){
    try{
      ...
      persistOrders(nextOrders,sourcePhones);setBulkOrderOpen(false);setBulkStagedOrders([]);setPartsView(receivedOrdersCount===createdOrders?'received':'orders');
