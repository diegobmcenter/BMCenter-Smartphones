...
import{effectivePartCost,returnRefundTotal,returnRecoveredAmount,normalizePartsOrder,normalizePartsOrders,syncOrdersIntoPhones,migrateLegacyPartsOrders,recoverLegacyPartOrderStatusMutations,isPartProcurementComplete,isPartOpenForProcurement,orderStatusLabel,createBulkPartsOrder,createMultiBulkPartsOrder,removePartsOrderLinks,bulkPhoneProductsTotal}from'./partsOrders.js';
...
function Parts(){
  ...
  function openReturnSettlement(row){
    ...
    setRefundEditor({orderId:row.order.id,itemId:item.id,supplier:row.order.supplier,partName:item.partName,phoneLabel:row.phone?phoneDisplayName(row.phone,{includeCode:false}):item.phoneLabel||'Aparelho',originalPart:Number(item.price||0),originalFreight:Number(item.freightShare||0),partRefund:numberText(item.returnPartRefund!==undefined&&item.returnPartRefund!==null&&item.returnPartRefund!==''?item.returnPartRefund:item.price||0),freightRefund:numberText(item.returnFreightRefund||0),financialStatus:item.returnFinancialStatus||'received',method:item.returnRefundMethod||'',date:item.returnRefundDate||today});
  }
  ...
  return <><div className="parts-command-page parts-v10447 parts-v10448 parts-v10450">
    ...
    <section className="parts-v47-workspace parts-v50-workspace"><nav className="parts-v47-tabs"><button className={partsView==='needs'?'active':''} onClick={()=>setPartsView('needs')}><Package size={14}/> Em aberto <b>{quoteableRows.length}</b></button><button className={partsView==='quotes'?'active':''} onClick={()=>setPartsView('quotes')}><Tags size={14}/> Cotações <b>{quoted.length}</b></button><button className={partsView==='orders'?'active':''} onClick={()=>setPartsView('orders')}><ShoppingCart size={14}/> Pedidos <b>{waitingOrders.length}</b></button><button className={partsView==='received'?'active':''} onClick={()=>setPartsView('received')}><CheckSquare size={14}/> Recebidos <b>{receivedOrders.length}</b></button><button className={partsView==='returns'?'active parts-v79-return-tab':''} onClick={()=>setPartsView('returns')}><RotateCcw size={14}/> Devoluções <b>{returnAttentionCount}</b></button></nav>
