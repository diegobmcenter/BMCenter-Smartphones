...
import{effectivePartCost,normalizePartsOrder,normalizePartsOrders,syncOrdersIntoPhones,migrateLegacyPartsOrders,orderStatusLabel,createBulkPartsOrder,createMultiBulkPartsOrder,removePartsOrderLinks}from'./partsOrders.js';
...
import BatchV102 from './v102/pages/BatchV102.jsx';import ActivityV102 from './v102/pages/ActivityV102.jsx';import ReportsV10 from './v10/pages/ReportsV10.jsx';import{cloudConfigured,getCloudSession,signInCloud,signUpCloud,signOutCloud,initializeCloudState,queueCloudSave,subscribeCloudState,getCloudStatus,clearCloudState,pushCloudStateNow,createCloudBackup,listCloudBackups,restoreCloudBackup,deleteCloudBackup}from'./cloud.js';import'./styles.css';import'./v10.css';import'./v102.css';import'./v1023.css';import'./v1024.css';import'./v1025.css';import'./v1026.css';import'./v1027.css';import'./v1028.css';import'./v1029.css';import'./v1030.css';import'./v1031.css';import'./v1033.css';import'./v1034.css';import'./v1038.css';import'./v1039.css';import'./v10311.css';import'./v10312.css';import'./v10313.css';import'./v10314.css';import'./v10315.css';import'./v1040.css';import'./v1041.css';import'./v1042.css';import'./v1043.css';import'./v1044.css';import'./v1046.css';import'./v1047.css';import'./v1048.css';import'./v1049.css';import'./v10410.css';import'./v10413.css';import'./v10414.css';import'./v10415.css';import'./v10416.css';import'./v10417.css';import'./v10418.css';import'./v10423.css';import'./v10424.css';import'./v10447.css';import'./v10448.css';import'./v10449.css';import'./v10450.css';
...
function Parts(){
  ...
  const[bulkActiveProductId,setBulkActiveProductId]=useState('');
  const[bulkDraft,setBulkDraft]=useState(()=>({supplier:'',freight:'',orderDate:new Date().toISOString().slice(0,10),expectedDate:'',notes:'',receivedNow:false}));
  ...
  function openBulkOrder(){
    ...
    setBulkOrderOpen(true);setBulkSearch('');setBulkStatus('Todos');setBulkProducts([first]);setBulkActiveProductId(first.id);
    setBulkDraft({supplier:'',freight:'',orderDate:new Date().toISOString().slice(0,10),expectedDate:'',notes:'',receivedNow:false})
  }
  ...
  function addBulkProduct(){const product=newBulkProduct('');setBulkProducts(current=>[...current,product]);setBulkActiveProductId(product.id)}
  ...
  function saveBulkOrder(){
    if(!bulkDraft.supplier.trim())return flash('Selecione o fornecedor do pedido em massa.');
    const selectedProducts=bulkProducts.filter(product=>String(product.name||'').trim()&&(product.phoneIds||[]).length);
    if(!selectedProducts.length)return flash('Adicione pelo menos um produto e selecione os aparelhos dele.');
    ...
    if(duplicateNames.length)return flash('Há produtos com o mesmo nome. Una os aparelhos no mesmo produto.');
    const missingPrice=selectedProducts.some(product=>(product.phoneIds||[]).some(id=>String(product.pricesByPhone?.[id]??product.unitPrice).trim()===''));
    if(missingPrice)return flash('Informe o valor de cada produto antes de concluir.');
    try{
      const products=selectedProducts.map(product=>({...product,name:product.name.trim(),unitPrice:toNumber(product.unitPrice),pricesByPhone:Object.fromEntries((product.phoneIds||[]).map(id=>[id,toNumber(product.pricesByPhone?.[id]??product.unitPrice)]))}));
      const result=createMultiBulkPartsOrder({phones,products,supplier:bulkDraft.supplier,freight:toNumber(bulkDraft.freight),orderDate:bulkDraft.orderDate,expectedDate:bulkDraft.expectedDate,notes:bulkDraft.notes,receivedNow:bulkDraft.receivedNow});
      if(!result.order)return flash(result.skipped.length?'As peças selecionadas já estão em pedidos ativos.':'Nenhum item pôde ser criado.');
      ...
      flash(`${total} item(ns) adicionados em massa${skipped?` · ${skipped} ignorado(s)`:''}.`)
