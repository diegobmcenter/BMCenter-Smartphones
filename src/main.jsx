...
import{buildSalesQuickReplies,defaultSalesReplySettings,normalizeSalesReplySettings}from'./salesQuickReplies.js';
...
Total: ${money(order.total)}`;
  ...
  return <><div className="parts-command-page parts-v10447 parts-v10448 parts-v10450">
    ...
    <div className="parts-v48-rule-banner"><ShieldCheck size={16}/><div><b>Pedidos continuam sendo somente de peças</b><small>Itens avulsos podem acompanhar o mesmo pedido do fornecedor, mas não criam aparelho, não entram no estoque e não alteram custos ou margem dos smartphones.</small></div></div>
...
function QuickReplyModal({phone,onClose}){
 ...
 return <Modal className="v10527-quick-reply-modal" title="Resposta rápida" subtitle={phoneDisplayName(phone,{includeCode:false})} titleIcon={<MessageSquare size={18}/>} onClose={onClose}>
  <div className="v10527-reply-settings"><label><span>Local padrão</span><input value={settings.location} onChange={e=>setSettings(current=>({...current,location:e.target.value}))} onBlur={()=>persistSettings(settings)} placeholder="Ex.: Maringá"/></label><small>Salvo automaticamente e usado nas respostas de disponibilidade e valor.</small></div>
  <div className="v10527-reply-list">{replies.map(reply=><article key={reply.id}><header><div><b>{reply.label}</b><small>{reply.hint}</small></div><button type="button" className={copied===reply.id?'success-button':''} onClick={()=>copyReply(reply)}><Copy size={14}/>{copied===reply.id?' Copiado':' Copiar'}</button></header><p>{reply.text}</p></article>)}</div>
  <div className="v10527-reply-footer"><small>Os textos usam os dados atuais do aparelho. Nenhuma mensagem é enviada automaticamente.</small><button type="button" onClick={onClose}>Fechar</button></div>
 </Modal>
}
