...
function Parts(){
  ...
  function undoOrderProgress(order,itemId=null){
    ...
    const step=targets.some(item=>item.receivedAt)?'receive':'confirm';
    if(!targets.some(item=>step==='receive'?item.receivedAt:(item.confirmedAt&&!item.receivedAt)))return flash('Não há uma etapa anterior para desfazer.');
