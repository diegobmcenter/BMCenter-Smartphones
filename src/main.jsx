...
const APP_VERSION='10.5.28';
const ALL_CLOUD_KEYS=[SKEY,ADSNOTEKEY,VKEY,BKEY,FKEY,QKEY,OKEY,UKEY,PKEY,TKEY,IKEY,MKEY,MENUKEY,CFGKEY,ATITLEKEY,ADESCKEY,VIEWKEY,CHECKKEY,GOALKEY,PHONECOLKEY,TABLELAYOUTKEY,SNAPKEY,PHONE_DRAFT_KEY,BATCH_DRAFT_KEY,STATUSKEY,'bmcenter-font-scales',SALESREPLYKEY];
...
function Parts(){
  ...
  const overviewCounts={
    ...
    returns:overviewReturnRows.length+overviewExternalReturnRows.length
  };
  const waitingItems=overviewWaitingRows.length;
  const receivedItems=overviewReceivedRows.length;
  const returnAttentionCount=overviewReturnRows.length+overviewExternalReturnRows.length;
  const overviewSuppliers=[...new Set([...quoteSuppliers,...orders.map(order=>order.supplier)].filter(Boolean))].sort((a,b)=>a.localeCompare(b,'pt-BR',{sensitivity:'base'}));
  const overviewDetailRows=overviewDetail==='open'?overviewOpenRows:overviewDetail==='waiting'?overviewWaitingRows:overviewDetail==='received'?overviewReceivedRows:overviewDetail==='returns'?[...overviewReturnRows,...overviewExternalReturnRows]:[];
  ...
  function saveBulkOrder(){
    try{
      ...
      for(const snapshot of snapshots){
        ...
        nextOrders.push(result.order);sourcePhones=syncOrdersIntoPhones(result.phones,nextOrders);createdOrders++;createdItems+=result.order.items.length+(result.order.externalItems||[]).reduce((sum,item)=>sum+Number(item.quantity||1),0);if(snapshot.draft.receivedNow)receivedOrdersCount++
      }
  ...
  return <><div className="parts-command-page parts-v10447 parts-v10448 parts-v10450">
    ...
    <section className="parts-v47-workspace parts-v50-workspace"><nav className="parts-v47-tabs"><button className={partsView==='needs'?'active':''} onClick={()=>setPartsView('needs')}><Package size={14}/> Em aberto <b>{overviewCounts.open}</b></button><button className={partsView==='quotes'?'active':''} onClick={()=>setPartsView('quotes')}><Tags size={14}/> Cotações <b>{overviewCounts.quotes}</b></button><button className={partsView==='orders'?'active':''} onClick={()=>setPartsView('orders')}><ShoppingCart size={14}/> Pedidos <b>{overviewCounts.orders}</b></button><button className={partsView==='received'?'active':''} onClick={()=>setPartsView('received')}><CheckSquare size={14}/> Recebidos <b>{overviewCounts.received}</b></button><button className={partsView==='returns'?'active parts-v79-return-tab':''} onClick={()=>setPartsView('returns')}><RotateCcw size={14}/> Devoluções <b>{overviewCounts.returns}</b></button></nav>
      ...
      {partsView==='returns'&&<div className="parts-v79-returns parts-v84-returns parts-v10528-returns">
        ...
        {!visibleReturnRows.length&&!visibleExternalReturnRows.length&&<div className="parts-v47-empty"><RotateCcw size={20}/><div><b>Nenhum item neste filtro</b><span>Nos pedidos recebidos, use “Devolver” na peça ou item avulso que ficou separado.</span></div></div>}
      </div>}
    </section>
  </div>
