...
function PhonePhotoStudio({phone,onPhoneChange,standalone=false}){
 ...
 const settings={style:'Automático',intensity:'Natural',keepScale:true,aiEngine:'magictouch',localFolderName:'',localFolderLinked:false,...(workPhone?.photoStudioSettings||{})};
 ...
 async function processAll({force=false,sceneOverride=null,sourcePhone=null}={}){
  ...
  for(let index=0;index<sourcePhotos.length;index++){
   ...
   try{
    ...
    await putPhotoAsset({id:processedAssetId,phoneId:current.id,role:'processed',dataUrl:resultDataUrl,createdAt:new Date().toISOString(),sceneId:chosenScene.id,name:`${current.code||'aparelho'}-${index+1}.jpg`,engine:'local-magictouch'});
 ...
 return <section className={`photo-studio ${standalone?'standalone':''}`}>
  <header className="photo-studio-head"><div><span><WandSparkles size={15}/></span><div><h3>Fotos para anúncio</h3><p>MagicTouch local · toque no aparelho para corrigir · pixels originais preservados.</p></div></div><div className="photo-studio-head-right"><div className="photo-metadata-badge"><ShieldCheck size={14}/><span><b>Sem metadados</b><small>EXIF · GPS · XMP · IPTC</small></span></div><div className="photo-count"><b>{ready}</b><span>prontas</span><em>/ {total}</em></div></div></header>
  <div className="photo-studio-controls">
   ...
   <label>Motor IA<select value="magictouch" disabled><option>MagicTouch · Google MediaPipe</option></select></label>
