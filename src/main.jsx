...
function Parts(){
  ...
  function changeReturnState(orderId,itemId,state){
    const stamp=new Date().toISOString();let affected=[];
    const next=orders.map(order=>{if(order.id!==orderId)return order;const items=order.items.map(item=>{if(item.id!==itemId)return item;affected.push(`${item.phoneId}::${item.partId}`);if(state==='pending')return{...item,returnStatus:'pending',returnMarkedAt:item.returnMarkedAt||stamp,returnedToSupplierAt:''};if(state==='returned')return{...item,returnStatus:'returned',returnMarkedAt:item.returnMarkedAt||stamp,returnedToSupplierAt:stamp};return{...item,returnStatus:'',returnMarkedAt:'',returnedToSupplierAt:''}});return normalizePartsOrder({...order,items,updatedAt:stamp})});
    const message=state==='pending'?'Peça separada para devolução':state==='returned'?'Peça devolvida ao fornecedor':'Devolução cancelada';
    const source=withTimeline(phones,affected,message);persistOrders(next,source);
    if(state==='pending'){setPartsView('returns');setReturnFilter('pending');flash('Item adicionado à lista de devoluções.')}else if(state==='returned')flash('Devolução marcada como concluída.');else flash('Item removido da lista de devoluções.')
  }
  ...
  const returnMatches=row=>{const text=`${row.order.supplier} ${row.item.partName} ${row.item.phoneLabel||''} ${row.phone?.code||''} ${row.phone?.brand||''} ${row.phone?.model||''}`.toLowerCase();return text.includes(query.toLowerCase())&&(supplierFilter==='Todos'||row.order.supplier===supplierFilter)&&(returnFilter==='all'||row.item.returnStatus===returnFilter)};
  ...
  return <><div className="parts-command-page parts-v10447 parts-v10448 parts-v10450">
    ...
    <section className="parts-v47-workspace parts-v50-workspace"><nav className="parts-v47-tabs"><button className={partsView==='needs'?'active':''} onClick={()=>setPartsView('needs')}><Package size={14}/> Em aberto <b>{quoteableRows.length}</b></button><button className={partsView==='quotes'?'active':''} onClick={()=>setPartsView('quotes')}><Tags size={14}/> Cotações <b>{quoted.length}</b></button><button className={partsView==='orders'?'active':''} onClick={()=>setPartsView('orders')}><ShoppingCart size={14}/> Pedidos <b>{waitingOrders.length}</b></button><button className={partsView==='received'?'active':''} onClick={()=>setPartsView('received')}><CheckSquare size={14}/> Recebidos <b>{receivedOrders.length}</b></button><button className={partsView==='returns'?'active parts-v79-return-tab':''} onClick={()=>setPartsView('returns')}><RotateCcw size={14}/> Devoluções <b>{pendingReturns.length}</b></button></nav>
      ...
      {partsView==='returns'&&<div className="parts-v79-returns">{visibleReturnRows.map(row=><article className={`parts-v79-return-card ${row.item.returnStatus}`} key={`${row.order.id}-${row.item.id}`}><span className="parts-v79-return-icon"><RotateCcw size={15}/></span><div className="parts-v79-return-identity"><small>{row.order.supplier}</small><b>{row.item.partName}</b><span>{row.phone?phoneDisplayName(row.phone,{includeCode:false}):row.item.phoneLabel||'Aparelho'}{row.phone?.code?` · ${row.phone.code}`:''}</span></div><div className="parts-v79-return-meta"><small>Pedido</small><b>{row.order.orderDate?formatDate(row.order.orderDate):'—'}</b></div><div className="parts-v79-return-meta"><small>Valor</small><b>{money(row.item.effectiveCost||row.item.price)}</b></div><div className="parts-v79-return-actions">{row.item.returnStatus==='pending'?<><span>Para devolver</span><button onClick={()=>changeReturnState(row.order.id,row.item.id,'cancel')}>Cancelar</button><button className="primary" onClick={()=>changeReturnState(row.order.id,row.item.id,'returned')}><CheckSquare size={12}/> Devolvido</button></>:<><span className="returned">Devolvida {row.item.returnedToSupplierAt?new Date(row.item.returnedToSupplierAt).toLocaleDateString('pt-BR'):''}</span><button onClick={()=>changeReturnState(row.order.id,row.item.id,'pending')}><RotateCcw size={12}/> Reabrir</button></>}</div></article>)}{!visibleReturnRows.length&&<div className="parts-v47-empty"><RotateCcw size={20}/><div><b>Nenhum item para devolução</b><span>Nos pedidos recebidos, use “Devolver” na peça que ficou sobrando.</span></div></div>}</div>}
    </section>
  </div>
