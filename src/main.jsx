...
function Parts(){
  ...
  const[overviewStatus,setOverviewStatus]=useState('Todos');
  ...
  const overviewNeedle=overviewSearch.trim().toLowerCase();
  const overviewPhoneMatches=phone=>(overviewStatus==='Todos'||phone?.status===overviewStatus);
  const overviewRowMatches=row=>{
    const text=`${phoneDisplayName(row.phone)} ${row.phone.code||''} ${row.phone.status||''} ${row.part.name} ${(row.quotes||[]).map(q=>q.supplier).join(' ')}`.toLowerCase();
    const supplierMatch=overviewSupplier==='Todos'||(row.quotes||[]).some(q=>q.supplier===overviewSupplier)||row.part.purchaseSupplier===overviewSupplier;
    return overviewPhoneMatches(row.phone)&&supplierMatch&&(!overviewNeedle||text.includes(overviewNeedle));
  };
  ...
  const overviewOrderMatches=order=>{
    const linkedPhones=(order.items||[]).map(item=>phoneById.get(item.phoneId)).filter(Boolean);
    const text=`${order.supplier||''} ${order.notes||''} ${(order.items||[]).map(item=>`${item.partName||''} ${item.phoneLabel||''}`).join(' ')} ${(order.externalItems||[]).map(item=>`${item.partName||''} ${item.reference||''}`).join(' ')} ${linkedPhones.map(phone=>`${phoneDisplayName(phone)} ${phone.code||''} ${phone.status||''}`).join(' ')}`.toLowerCase();
    const statusMatch=overviewStatus==='Todos'||linkedPhones.some(phone=>phone.status===overviewStatus);
    const supplierMatch=overviewSupplier==='Todos'||order.supplier===overviewSupplier;
    return statusMatch&&supplierMatch&&(!overviewNeedle||text.includes(overviewNeedle));
  };
  const overviewItemMatches=(order,item,phone)=>{
    const text=`${order.supplier||''} ${item.partName||''} ${item.phoneLabel||''} ${phone?phoneDisplayName(phone):''} ${phone?.code||''} ${phone?.status||''}`.toLowerCase();
    return (overviewStatus==='Todos'||phone?.status===overviewStatus)&&(overviewSupplier==='Todos'||order.supplier===overviewSupplier)&&(!overviewNeedle||text.includes(overviewNeedle));
  };
  ...
  const standaloneReturnRows=standaloneReturns.filter(Boolean).map(entry=>{const quantity=Math.max(1,Number(entry.quantity||1)||1),price=toNumber(entry.unitPrice)*quantity,freight=toNumber(entry.freight);return{order:{id:`standalone-${entry.id}`,supplier:entry.supplier||'Fornecedor não informado',source:'standalone'},item:{...entry,partName:entry.partName||'Item avulso',returnStatus:entry.status||'pending',price,freightShare:freight,effectiveCost:price+freight},phone:null,type:'standalone',standalone:entry}});
  ...
  const operationalCounts=partsOperationalCounters(phones,orders); // baseline global compartilhada com Relatórios; a UI usa overviewCounts quando há filtros gerais.
  const overviewOpenRows=quoteableRows.filter(overviewRowMatches);
  const overviewWaitingRows=orders.flatMap(order=>(order.items||[]).filter(item=>item.confirmedAt&&!item.receivedAt&&!item.returnStatus).map(item=>({order,item,phone:phoneById.get(item.phoneId)||null}))).filter(row=>overviewItemMatches(row.order,row.item,row.phone));
  const overviewReceivedRows=orders.flatMap(order=>(order.items||[]).filter(item=>item.receivedAt&&!item.returnStatus).map(item=>({order,item,phone:phoneById.get(item.phoneId)||null}))).filter(row=>overviewItemMatches(row.order,row.item,row.phone));
  const overviewReturnRows=[...pendingReturns,...pendingReturnFinancials].filter(row=>overviewItemMatches(row.order,row.item,row.phone));
  const overviewExternalReturnRows=activeExternalReturns.filter(row=>{const text=`${row.order.supplier||''} ${row.item.partName||''} ${row.item.reference||''}`.toLowerCase();return overviewStatus==='Todos'&&(overviewSupplier==='Todos'||row.order.supplier===overviewSupplier)&&(!overviewNeedle||text.includes(overviewNeedle))});
  const overviewStandaloneReturnRows=activeStandaloneReturns.filter(row=>{const text=`${row.order.supplier||''} ${row.item.partName||''} ${row.item.reference||''} ${row.item.reason||''}`.toLowerCase();return overviewStatus==='Todos'&&(overviewSupplier==='Todos'||row.order.supplier===overviewSupplier)&&(!overviewNeedle||text.includes(overviewNeedle))});
  const overviewCounts={
    open:overviewOpenRows.length,
    quotes:overviewOpenRows.filter(row=>row.quotes.length).length,
    orders:waitingOrders.filter(overviewOrderMatches).length,
    received:receivedOrders.filter(overviewOrderMatches).length,
    returns:overviewReturnRows.length+overviewExternalReturnRows.length+overviewStandaloneReturnRows.length
  };
  ...
  const overviewHasFilters=!!overviewSearch.trim()||overviewStatus!=='Todos'||overviewSupplier!=='Todos';
  const clearOverviewFilters=()=>{setOverviewSearch('');setOverviewStatus('Todos');setOverviewSupplier('Todos')};
...
Total: ${money(order.total)}`;
  ...
  function OrderCard({order,receivedHistory=false}){
    ...
    return <article className={`parts-v48-order-card status-${order.status} parts-v50-order-card ${receivedHistory?'parts-v49-received-card':''} ${expanded?'expanded':'collapsed'} ${externalItems.length?'parts-v10528-mixed-order':''}`}>
      <header className="parts-v50-order-summary"><div className="parts-v48-order-title"><span><Store size={14}/></span><div><small>{orderStatusLabel(order.status).toUpperCase()}</small><h3>{order.supplier}</h3><p>{order.items.length?`${order.items.length} BMCenter`:''}{order.items.length&&externalQty?' · ':''}{externalQty?`${externalQty} avulso(s)`:''}{order.orderDate?` · Pedido ${formatDate(order.orderDate)}`:''}</p></div></div><div className="parts-v48-order-total"><small>Total do pedido</small><strong>{money(order.total)}</strong><span>Frete {money(order.freight)}</span></div><div className="parts-v10514-summary-actions">{!expanded&&<button type="button" className="parts-v10514-undo-summary" title={receivedHistory&&allReceived?'Desfazer recebimento do pedido completo':anyReceived?'Desfazer último recebimento do pedido':anyConfirmed?'Desfazer confirmação do pedido':'Desfazer preparação do pedido'} onClick={()=>undoOrderProgress(order)}><RotateCcw size={13}/><span>Desfazer</span></button>}<button type="button" className="parts-v50-expand-button" title={expanded?'Fechar detalhes':'Ver detalhes'} onClick={toggle}><ChevronDown size={16}/></button></div></header>
  ...
  return <><div className="parts-command-page parts-v10447 parts-v10448 parts-v10450">
    ...
    <section className="parts-v10518-overview-tools"><div className="parts-v10518-overview-filters"><label><Search size={13}/><input value={overviewSearch} onChange={e=>setOverviewSearch(e.target.value)} placeholder="Filtrar resumo por aparelho, código ou peça..."/></label><select value={overviewStatus} onChange={e=>setOverviewStatus(e.target.value)}><option value="Todos">Todos os status</option>{statuses.filter(status=>!['Vendido','Descarte/Sucata'].includes(status)).map(status=><option key={status}>{status}</option>)}</select><select value={overviewSupplier} onChange={e=>setOverviewSupplier(e.target.value)}><option value="Todos">Todos os fornecedores</option>{overviewSuppliers.map(name=><option key={name}>{name}</option>)}</select><button type="button" className="parts-v10518-clear" disabled={!overviewHasFilters} onClick={clearOverviewFilters}><RotateCcw size={13}/> Limpar</button></div>{overviewHasFilters&&<small className="parts-v10518-filter-note">Os números acima e as listas abaixo estão considerando estes filtros.</small>}</section>
